#!/usr/bin/env python3
"""
Professional Adaptive Futures Signal Bot V20.4 PRO CONTINUATION.
...
APP_NAME = "Professional Adaptive Futures Bot V20.4.1 PRO CONTINUATION"
DEPLOY_MARKER = "V20_4_1_DUAL_START_TELEGRAM_FIX_2026_09_03"
...
TELEGRAM_SEND_ATTEMPTS = max(1, min(8, int(os.getenv("TELEGRAM_SEND_ATTEMPTS", "3"))))
TELEGRAM_STARTUP_ATTEMPTS = max(
    1, min(10, int(os.getenv("TELEGRAM_STARTUP_ATTEMPTS", "5")))
)
TELEGRAM_RETRY_BASE_SECONDS = max(
    0.2, float(os.getenv("TELEGRAM_RETRY_BASE_SECONDS", "0.8"))
)
STATE_FILE = Path(os.getenv("STATE_FILE", "bot_state_v20_4.json"))

SCAN_SECONDS = max(15, int(os.getenv("SCAN_SECONDS", "30")))
DIAGNOSTIC_SECONDS = max(1800, int(os.getenv("DIAGNOSTIC_SECONDS", "14400")))
UNIVERSE_SIZE = max(20, min(160, int(os.getenv("UNIVERSE_SIZE", "80"))))
DEEP_CANDIDATE_LIMIT = max(4, min(30, int(os.getenv("DEEP_CANDIDATE_LIMIT", "14"))))
MAX_ACTIVE_SIGNALS = max(1, min(4, int(os.getenv("MAX_ACTIVE_SIGNALS", "2"))))
HTTP_CONCURRENCY = max(2, min(20, int(os.getenv("HTTP_CONCURRENCY", "8"))))
HTTP_TIMEOUT_SECONDS = max(5, int(os.getenv("HTTP_TIMEOUT_SECONDS", "12")))
WATCH_MAX_MINUTES = max(30, int(os.getenv("WATCH_MAX_MINUTES", "90")))

MIN_QUOTE_TURNOVER_USDT = float(os.getenv("MIN_QUOTE_TURNOVER_USDT", "3000000"))
MAX_BOOK_SPREAD_BPS = float(os.getenv("MAX_BOOK_SPREAD_BPS", "8"))
MIN_BOOK_DEPTH_USDT = float(os.getenv("MIN_BOOK_DEPTH_USDT", "20000"))
MAX_EXPECTED_SLIPPAGE = float(os.getenv("MAX_EXPECTED_SLIPPAGE", "0.0008"))

MIN_SIGNAL_SCORE = int(os.getenv("MIN_SIGNAL_SCORE", "82"))
CONT_SCORE_MIN = int(os.getenv("CONT_SCORE_MIN", "7"))
FADE_SCORE_MAX = int(os.getenv("FADE_SCORE_MAX", "2"))
TF_AGREEMENT_MIN = int(os.getenv("TF_AGREEMENT_MIN", "2"))
PULLBACK_FRACTION_MIN = float(os.getenv("PULLBACK_FRACTION_MIN", "0.18"))
PULLBACK_FRACTION_MAX = float(os.getenv("PULLBACK_FRACTION_MAX", "0.45"))
PULLBACK_INVALIDATION = float(os.getenv("PULLBACK_INVALIDATION", "0.60"))
ANTI_CHASE_MAX = float(os.getenv("ANTI_CHASE_MAX", "0.004"))

TP_MOVES = (0.0080, 0.0140, 0.0200, 0.0300, 0.0440)
TP_SHARES = (0.15, 0.20, 0.30, 0.20, 0.15)
STOP_MOVE_MIN = float(os.getenv("STOP_MOVE_MIN", "0.0065"))
STOP_MOVE_MAX = float(os.getenv("STOP_MOVE_MAX", "0.0115"))
MIN_RR_TO_TP3 = float(os.getenv("MIN_RR_TO_TP3", "1.70"))
ESTIMATED_TAKER_FEE = float(os.getenv("ESTIMATED_TAKER_FEE", "0.0005"))
ESTIMATED_MAKER_FEE = float(os.getenv("ESTIMATED_MAKER_FEE", "0.0002"))
ACCOUNT_RISK_FRACTION = float(os.getenv("ACCOUNT_RISK_FRACTION", "0.0025"))
MAX_LEVERAGE = min(10, max(1, int(os.getenv("MAX_LEVERAGE", "10"))))

NO_PROGRESS_MINUTES = int(os.getenv("NO_PROGRESS_MINUTES", "6"))
NO_PROGRESS_MFE_R = float(os.getenv("NO_PROGRESS_MFE_R", "0.25"))
PRE_TP1_STALE_MINUTES = int(os.getenv("PRE_TP1_STALE_MINUTES", "12"))
HARD_EXPIRE_MINUTES = int(os.getenv("HARD_EXPIRE_MINUTES", "45"))
SYMBOL_QUARANTINE_HOURS = int(os.getenv("SYMBOL_QUARANTINE_HOURS", "24"))
SEND_WATCH_ALERTS = os.getenv("SEND_WATCH_ALERTS", "false").lower() in {
    "1",
    "true",
    "yes",
}
PAPER_MODE = (
    True  # Deliberately hard-coded. No real-order implementation in this version.
)
...
class BingXPublicClient:
    def __init__(self) -> None:
        if aiohttp is None:
            raise RuntimeError(
                "aiohttp is required; run: pip install -r requirements.txt"
            )
...
class TradingEngine:
    ...
    async def run(self) -> None:
        ...
        delivered = await self.telegram.send(
            ...
            f"Если это сообщение пришло — V20.4.1 запущен правильно.",
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if consecutive_errors in {1, 3, 10}:
                    await self.telegram.send(
                        f"⚠️ <b>V20.4 cycle error</b> · подряд {consecutive_errors}\n"
            ...
            try:
                ...
                pass
        await self.telegram.send("⏹ <b>V20.4 PRO stopped cleanly</b>")
...
class BotASGI:
    """Minimal ASGI wrapper for old Render uvicorn start commands."""
    ...
    async def start(self) -> None:
        ...
        self.engine = TradingEngine(self.client, telegram, StateStore(STATE_FILE))
        HEALTH.update(
            {
        ...
        self.engine_task = asyncio.create_task(
            self._run_engine(), name="v20-4-trading-engine"
        )
        LOG.info("ASGI startup complete; trading engine task created")
    ...
    async def _run_engine(self) -> None:
        ...
        try:
            ...
            LOG.exception("Trading engine task failed")
    ...
    async def stop(self) -> None:
        ...
        if self.engine_task is not None:
            try:
                await asyncio.wait_for(self.engine_task, timeout=20)
            except asyncio.TimeoutError:
        ...
        LOG.info("ASGI trading engine stopped cleanly")
        ...
        while True:
            ...
            if message["type"] == "lifespan.startup":
                try:
                    ...
                    LOG.exception("ASGI startup failed")
                    await send(
                        {
    ...
    async def _http(self, scope: dict[str, Any], send: Any) -> None:
        ...
        if path not in {"/", "/health"}:
            ...
            data = health_payload()
            status_code = 503 if data["status"] == "failed" else 200
...
async def async_main() -> None:
    ...
    for signal_name in (os_signal.SIGINT, os_signal.SIGTERM):
        try:
            loop.add_signal_handler(signal_name, engine.stop_event.set)
        except (NotImplementedError, RuntimeError):
            pass
    if not telegram.enabled:
        LOG.warning(
            "TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID missing; signals will only be logged"
        )
    try:
        await engine.run()
    finally:
