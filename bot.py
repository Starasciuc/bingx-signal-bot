# VERIFIED GITHUB DEPLOY FILE — V19.0 PRO MTF SPIKE FADE · ACTIVE MOVER
...
ADAPTIVE_TARGET_SUCCESS_RATE = float(os.getenv("ADAPTIVE_TARGET_SUCCESS_RATE", "0.50"))
ADAPTIVE_CONFIRMATION_PASSES = int(os.getenv("ADAPTIVE_CONFIRMATION_PASSES", "2"))
ADAPTIVE_CONFIRMATION_SELECTED = int(os.getenv("ADAPTIVE_CONFIRMATION_SELECTED", "20"))
...
FEATURE_NAMES: Tuple[str, ...] = (
    ...
    "edge_30m",
    "btc_alignment",
    # Non-linear quality markers retained in V16.6. _vector_from_dict derives
    # them for old V16.2 backups, so all 50 historical rows remain trainable.
    "score_below_guard",
    "vol1_below_guard",
    "evidence_quality_pass",
    "symbol_fail_streak",
    # Retained for feature-schema compatibility. V17 does not train on
    # ordinary diagnostic SHADOW rows; registered PAPER rows still carry 1.0.
    "is_shadow",
    # V17 records the actual post-WATCH path.  These fields did not exist in
    # the 400-row archive, so the archive remains audit-only and cannot train
    # the new model.
    "watch_impulse",
    "pullback_depth",
    "reclaim_recovery",
    "watch_age",
    # V17.2 liquidity-first and dual-lane identity.  The restored archive does
    # not contain these fields and is excluded by MODEL_DATA_POLICY.
    "is_continuation_lane",
    "is_sweep_reversal_lane",
    "liquidity_rank",
    "liquidity_log_turnover",
    "liquidity_active_fraction",
    "liquidity_unique_fraction",
    "atr1_pct",
    "normalized_directional_move",
    # V17.3 adds executable-liquidity evidence at the exact entry moment.
...
def maybe_retrain(force: bool = False) -> Dict[str, Any]:
    ...
    selected_forward_test_rows = [
        ...
        if str(test_rows[i]["decision_reason"] or "") == PAPER_VALIDATION_REASON
    ]
...
def format_training_attempt_message(report: Dict[str, Any]) -> str:
    ...
    lines = [
        title,
        f"Данных модели (только V17.3 direct-measured PAPER): {closed_count}",
    ...
    if paper_dataset:
        lines.append(f"FOLLOW-THROUGH PAPER V17.3.2: {_metrics_line(paper_dataset)}")
...
APP_NAME = "Professional Adaptive Futures Bot AUTO V19.0 PRO MTF SPIKE FADE + ACTIVE MOVER"
DEPLOY_MARKER = "V19_0_PRO_MTF_SPIKE_FADE_ACTIVE_MOVER_2026_08_30"
...
EXHAUSTION_PAPER_REASON = "mtf_spike_fade_paper_v19_0"

# V18.2.1: only these two strategies are user-facing.
...
# --- V19.0 PROFESSIONAL MULTI-TIMEFRAME SPIKE FADE ---
# Goal: catch CYS-like blow-off moves without requiring the setup to be 4H.
# Stage 1 watches abnormal UP/DOWN expansion on 5m / 15m / 1h / 4h.
# Stage 2 enters only after the spike stops extending and micro-structure turns.
#
# IMPORTANT: a vertical green candle is NOT automatically a SHORT.
# Continuing highs + strong closes + active volume => RUNAWAY WATCH, no entry.
SQUEEZE_4H_ENABLED = os.getenv("MTF_SPIKE_FADE_ENABLED", "true").lower() == "true"

# Absolute move thresholds by timeframe. A setup may also qualify through
# relative ATR/body acceleration, so a smaller but statistically extreme move
# is not automatically ignored.
SPIKE_MIN_MOVE_5M = float(os.getenv("SPIKE_MIN_MOVE_5M", "0.025"))
SPIKE_MIN_MOVE_15M = float(os.getenv("SPIKE_MIN_MOVE_15M", "0.040"))
SPIKE_MIN_MOVE_1H = float(os.getenv("SPIKE_MIN_MOVE_1H", "0.060"))
SPIKE_MIN_MOVE_4H = float(os.getenv("SPIKE_MIN_MOVE_4H", "0.080"))
SPIKE_MIN_ATR_MULT = float(os.getenv("SPIKE_MIN_ATR_MULT", "1.80"))
SPIKE_MIN_VOLUME_PACE = float(os.getenv("SPIKE_MIN_VOLUME_PACE", "1.55"))
SPIKE_MIN_BODY_ACCEL = float(os.getenv("SPIKE_MIN_BODY_ACCEL", "2.20"))
SPIKE_MIN_BODY_FRACTION = float(os.getenv("SPIKE_MIN_BODY_FRACTION", "0.48"))
SPIKE_CLOSE_EXTREME = float(os.getenv("SPIKE_CLOSE_EXTREME", "0.66"))
SPIKE_MIN_SCORE = float(os.getenv("SPIKE_MIN_SCORE", "68.0"))
SPIKE_WATCH_MAX = int(os.getenv("SPIKE_WATCH_MAX", "28"))
SPIKE_WATCH_SECONDS = int(os.getenv("SPIKE_WATCH_SECONDS", "5400"))

# Backward-compatible aliases used by the existing watch container.
SQUEEZE_4H_WATCH_MAX = SPIKE_WATCH_MAX
SQUEEZE_4H_WATCH_SECONDS = SPIKE_WATCH_SECONDS

# Entry confirmation after the blow-off.
# Slightly faster than old 4H-only lane so a CYS-like correction is not missed.
EXHAUST_MIN_WATCH_SECONDS = int(os.getenv("SPIKE_MIN_WATCH_SECONDS", "15"))
EXHAUST_MIN_RETRACE = float(os.getenv("SPIKE_MIN_RETRACE", "0.0045"))
EXHAUST_MAX_RETRACE_AT_ENTRY = float(os.getenv("SPIKE_MAX_RETRACE_AT_ENTRY", "0.0450"))
EXHAUST_MIN_REVERSAL_1M = float(os.getenv("SPIKE_MIN_REVERSAL_1M", "0.0009"))
EXHAUST_MIN_REVERSAL_3M = float(os.getenv("SPIKE_MIN_REVERSAL_3M", "0.0020"))
EXHAUST_MIN_CONFIRMATIONS = int(os.getenv("SPIKE_MIN_CONFIRMATIONS", "5"))
EXHAUST_RUNAWAY_RECENT_SECONDS = int(os.getenv("SPIKE_RUNAWAY_RECENT_SECONDS", "20"))
EXHAUST_RUNAWAY_MIN_1M = float(os.getenv("SPIKE_RUNAWAY_MIN_1M", "0.0014"))
EXHAUST_MIN_VOL1 = float(os.getenv("SPIKE_MIN_VOL1", "0.35"))

# Execution quality. We do not sacrifice executable liquidity merely to
...
# CYS-like correction geometry: roughly 0.8 / 1.4 / 2.1 / 3.1 / 4.2%.
# TP3 remains the minimum positive classification.
EXHAUST_TP1_MOVE = float(os.getenv("SPIKE_TP1_MOVE", "0.0080"))
EXHAUST_TP2_MOVE = float(os.getenv("SPIKE_TP2_MOVE", "0.0140"))
EXHAUST_TP3_MOVE = float(os.getenv("SPIKE_TP3_MOVE", "0.0210"))
EXHAUST_TP4_MOVE = float(os.getenv("SPIKE_TP4_MOVE", "0.0310"))
EXHAUST_TP5_MOVE = float(os.getenv("SPIKE_TP5_MOVE", "0.0420"))
EXHAUST_MIN_SL_MOVE = float(os.getenv("SPIKE_MIN_SL_MOVE", "0.0090"))
EXHAUST_MAX_SL_MOVE = float(os.getenv("SPIKE_MAX_SL_MOVE", "0.0220"))
EXHAUST_SOFT_EXPIRE_MINUTES = int(os.getenv("SPIKE_SOFT_EXPIRE_MINUTES", "35"))
EXHAUST_HARD_EXPIRE_MINUTES = int(os.getenv("SPIKE_HARD_EXPIRE_MINUTES", "150"))
...
def load_state() -> Dict[str, Any]:
    ...
    try:
        ...
        base["shadow_signals"] = [
            ...
            if str(item.get("shadow_reason", "")) in OFFICIAL_USER_PAPER_REASONS
        ]
...
def adaptive_model_data_count() -> int:
    """Rows allowed to influence V17.2: its own confirmed PAPER cohort only."""
    init_adaptive_db()
    with _LOCK, _connect() as conn:
        row = conn.execute(
            "SELECT COUNT(*) AS n FROM adaptive_trades "
...
def closed_outcome_progress_message(
    ...
    return (
        ...
        f"🚀 MTF SPIKE: {_metrics_line(exhaust)}\n"
...
def maybe_send_auto_backup() -> Dict[str, Any]:
    ...
    try:
        ...
        paper_count = int(paper_validation_metrics().get("n", 0) or 0)
        last_paper_checkpoint = int(
            STATE.get("last_paper_checkpoint_count", 0) or 0
        )
        total_due = bool(
            closed_count >= AUTO_BACKUP_EVERY_CLOSED
            and closed_count - last_count >= AUTO_BACKUP_EVERY_CLOSED
        )
        paper_due = bool(
            PAPER_CHECKPOINT_EVERY > 0
            and paper_count > 0
            and paper_count - last_paper_checkpoint >= PAPER_CHECKPOINT_EVERY
        )
        last_paper_report = int(
            STATE.get("last_paper_cohort_report_count", 0) or 0
        )
        report_milestones = {
            max(1, PAPER_PILOT_REQUIRED_OUTCOMES),
            max(1, PAPER_REVIEW_REQUIRED_OUTCOMES),
            max(1, PAPER_LANE_REQUIRED_OUTCOMES),
        }
        report_due = bool(
            paper_count > last_paper_report
            and (
                paper_count in report_milestones
                or (
                    paper_count > PAPER_LANE_REQUIRED_OUTCOMES
                    and paper_count % max(1, RETRAIN_EVERY) == 0
                )
            )
        )
        if not total_due and not paper_due and not report_due:
            return {
                "attempted": False,
                "sent": False,
                "closed_count": closed_count,
                ...
                "last_paper_checkpoint": last_paper_checkpoint,
            }
        last_audit_count = int(STATE.get("last_source_audit_closed_count", 0) or 0)
        if total_due and closed_count - last_audit_count >= AUTO_BACKUP_EVERY_CLOSED:
            if send_telegram(format_source_audit_message(AUTO_BACKUP_EVERY_CLOSED)):
                STATE["last_source_audit_closed_count"] = closed_count
                save_state()
        if report_due and send_telegram(format_paper_cohort_report(RETRAIN_EVERY)):
            STATE["last_paper_cohort_report_count"] = paper_count
            STATE["last_forward_report_count"] = paper_count
            save_state()

        filename_prefix = "adaptive_backup" if total_due else "adaptive_checkpoint"
        filename = (
            f"{filename_prefix}_{closed_count}_paper{paper_count}_{int(time.time())}.json"
        )
        caption_reason = (
            f"full {AUTO_BACKUP_EVERY_CLOSED}-outcome backup"
            if total_due
            else f"emergency PAPER checkpoint {paper_count}"
        )
        if send_telegram_document(
            build_export_bytes(),
            filename,
            f"🧠 V17.3 backup/checkpoint · {caption_reason} · total {closed_count} · PAPER {paper_count}",
        ):
            if total_due:
                STATE["last_backup_closed_count"] = closed_count
            if paper_due:
                STATE["last_paper_checkpoint_count"] = paper_count
            save_state()
            return {
                "attempted": True,
                "sent": True,
...
def _spike_tf_metrics(
    ...
    # Incomplete candle: compare projected volume pace with prior median.
    now_ms = int(time.time() * 1000)
    open_ms = int(cur.get("time", 0) or 0)
    elapsed_fraction = (now_ms - open_ms) / float(tf_ms) if open_ms > 0 else 1.0
    elapsed_fraction = max(0.18, min(1.0, elapsed_fraction))
    prev_vol_med = _median([float(x["volume"]) for x in prev])
    ...
    normal_body = max(_median(prev_bodies), 0.001)
    body_accel = body_move / normal_body

    # Recent impulse catches a "stick" spread across several candles instead of
    # demanding one perfect current candle.
    lookback = 3 if tf_name in {"5m", "15m"} else 2
    anchor = float(candles[-(lookback + 1)]["close"])
    recent_move = (c - anchor) / max(anchor, 1e-12)

    direction = 1 if (signed_body > 0 or recent_move > 0) else -1
    extreme_close = loc >= SPIKE_CLOSE_EXTREME if direction > 0 else loc <= (1.0 - SPIKE_CLOSE_EXTREME)

    abs_impulse = max(body_move, abs(recent_move))
    absolute_ok = abs_impulse >= absolute_min_move
    statistical_ok = atr_mult >= SPIKE_MIN_ATR_MULT and body_accel >= SPIKE_MIN_BODY_ACCEL

    if not (absolute_ok or statistical_ok):
        return None
    if volume_pace < SPIKE_MIN_VOLUME_PACE:
        return None
    if body_fraction < SPIKE_MIN_BODY_FRACTION and abs(recent_move) < absolute_min_move * 1.20:
        return None
    if not extreme_close and abs(recent_move) < absolute_min_move * 1.35:
        return None

    score = 48.0
    score += min(18.0, abs_impulse / max(absolute_min_move, 1e-9) * 8.0)
    score += min(12.0, max(0.0, (atr_mult - 1.2) * 4.0))
    score += min(12.0, max(0.0, (volume_pace - 1.0) * 3.5))
    score += min(10.0, max(0.0, (body_accel - 1.5) * 1.7))
    score = min(100.0, score)
...
def detect_mtf_spike_fade(
    ...
    """Detect a vertical high-volume impulse on ANY important timeframe.

    This is the CYS-style radar:
      strong UP expansion -> quiet SHORT watch
      strong DOWN expansion -> quiet LONG watch

    It does not enter merely because price moved a lot.
    """
    ...
    metrics = []
    for tf, candles, tf_ms, min_move in specs:
        m = _spike_tf_metrics(candles, tf, tf_ms, min_move)
        if m:
            metrics.append(m)

    if not metrics:
        return None, "no_mtf_spike"

    # Prefer the strongest standardized event. Multi-timeframe agreement boosts
    # conviction but is not mandatory.
    best = max(metrics, key=lambda x: float(x["score"]))
    direction = int(best["direction"])
    agreement = sum(1 for x in metrics if int(x["direction"]) == direction)
    opposite = sum(1 for x in metrics if int(x["direction"]) != direction)
    score = float(best["score"]) + max(0, agreement - 1) * 4.0 - opposite * 5.0
    score = min(100.0, score)
    if score < SPIKE_MIN_SCORE:
        return None, f"mtf_score_{score:.1f}"

    reversal_side = "SHORT" if direction > 0 else "LONG"
    event = "VERTICAL UP / BLOW-OFF" if reversal_side == "SHORT" else "VERTICAL DOWN / LIQUIDATION"
    tf_summary = ", ".join(
        f"{x['tf']} {x['recent_move']*100:+.1f}% Vx{x['volume_pace']:.1f}"
        for x in metrics
        if int(x["direction"]) == direction
    )

    reason = (
        f"🚀 MTF {event}: strongest {best['tf']} impulse {best['impulse_move']*100:.1f}%, "
        f"ATR x{best['atr_mult']:.2f}, volume pace x{best['volume_pace']:.2f}, "
        f"body acceleration x{best['body_acceleration']:.1f}; agreement {agreement}. "
        f"Context: {tf_summary}. NO ENTRY YET — waiting for spike failure."
    )

    # For SHORT use the highest observed blow-off point; for LONG the lowest.
    same = [x for x in metrics if int(x["direction"]) == direction]
    reference = max(float(x["high"]) for x in same) if reversal_side == "SHORT" else min(float(x["low"]) for x in same)

    return {
        "symbol": normalize_symbol(symbol),
        "side": reversal_side,
        "strategy": f"PRO_MTF_SPIKE_FADE_{reversal_side}",
        "trade_type": f"🚀 MTF SPIKE FADE {reversal_side}",
        "paper_style": "SQUEEZE_EXHAUSTION",
        "paper_setup_lane": "🚀 MTF SPIKE FADE",
        ...
        "spike_recent_move": float(best["recent_move"]),
...
def process_squeeze_exhaustion_watches() -> Dict[str, int]:
    """Stage 2: distinguish RUNAWAY spike from a real CYS-like fade.
    ...
    for item in snapshot:
        ...
        setup["paper_validation_origin"] = (
            f"🚀 V19 confirmed {side} correction after {item.get('spike_primary_tf', 'MTF')} vertical spike: "
...
def build_diagnostic(scan: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"🚀 MTF SPIKE FADE V19.0: {_metrics_line(exhaust_metrics)} · "
...
def add_shadow_signal(signal: Dict[str, Any], reason: str) -> bool:
    ...
    if reason not in OFFICIAL_USER_PAPER_REASONS:
        return False
...
async def scan_loop():
    ...
    send_telegram(
        ...
        f"Официальные PAPER-линии: 🧲 ACTIVE MOVER + 🚀 MTF SPIKE FADE V19.0. Legacy Follow-Through/SHADOW отключены.\n"
        f"Telegram CLEAN MODE: legacy SHADOW/near-miss/CONTROL уведомления жёстко отключены в коде; SHADOW_PROBE_ENABLED=False.\n"        f"Diagnostics: automatic summary every 12h (2 times/day); startup/manual scan can still send one on demand.\n"
        f"🧲 ACTIVE MOVER: HOT → quiet WATCH → pullback/reclaim → re-acceleration → PAPER.\n"        f"🧲 V18.2.3 gate fix: ACTIVE MOVER no longer passes through legacy PRO_DIRECT_MEASURED name filter; native quality + trigger + execution gates only.\n"
        f"🚀 MTF SPIKE FADE: вертикальный импульс на 5m/15m/1h/4h + аномальный объём → quiet WATCH → RUNAWAY guard → потеря micro-структуры → PAPER на коррекцию.\n"
        f"🚀 Важно: сильная зелёная палка сама по себе НЕ SHORT. Пока high обновляется, close держится у high и объём остаётся активным, бот ждёт и не шортит RUNAWAY.\n"
        f"Universe: MTF spike radar работает на deep-check ротации полного списка; 5m/15m/1h/4h проверяются для каждого deep-check контракта.\n"
