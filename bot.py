# VERIFIED GITHUB DEPLOY FILE — V20.3.6 MTF LEVEL RETEST · D1 → 4H → 1H → SCALP
...
APP_NAME = "Professional Adaptive Futures Bot AUTO V20.3.6 MTF LEVEL RETEST · D1 → 4H → 1H → SCALP"
DEPLOY_MARKER = "V20_3_6_MTF_LEVEL_RETEST_D1_4H_1H_SCALP_2026_09_04"
...
TRADER_STYLE_PAPER_REASON = "active_mover_paper_v20_3_6"
...
ACTIVE_MOVER_GUARD_REASON = "active_mover_strategy_guard_v20_3_6"
ACTIVE_RESEARCH_FILTER_REASON = "active_research_filter_v20_3_6"
ACTIVE_MTF_FILTER_REASON = "active_mtf_level_filter_v20_3_6"
...
# V20.3.6 — top-down technical-analysis hierarchy.
...
MTF_D1_TREND_SCORE_MIN = float(os.getenv("MTF_D1_TREND_SCORE_MIN", "52"))
...
MTF_LEVEL_MAX_DISTANCE_ATR = float(os.getenv("MTF_LEVEL_MAX_DISTANCE_ATR", "1.25"))
MTF_BREAKOUT_CLOSE_BUFFER_ATR = float(os.getenv("MTF_BREAKOUT_CLOSE_BUFFER_ATR", "0.08"))
MTF_BREAKOUT_MIN_VOL = float(os.getenv("MTF_BREAKOUT_MIN_VOL", "1.10"))
MTF_RETEST_MAX_AGE_HOURS = int(os.getenv("MTF_RETEST_MAX_AGE_HOURS", "10"))
MTF_RETEST_TOUCH_ATR = float(os.getenv("MTF_RETEST_TOUCH_ATR", "0.30"))
MTF_MIN_SCORE = float(os.getenv("MTF_MIN_SCORE", "56"))
...
def active_mover_setup(
    ...
    return {
        ...
        "trade_type": f"🧲 MTF LEVEL RETEST {side}",
...
def top_down_level_context(
    ...
    if d1_bias.get("bias") != wanted:
        return {
            "status": "D1_MISMATCH",
            "valid": False,
            "reason": f"D1 {d1_bias.get('bias')} != {wanted}",
            "d1_bias": d1_bias,
        }

    # 2) Only after D1 agrees: fetch 4H and 1H.
    ...
    mtf_score = (
        0.32 * d1_strength
        + 0.18 * (100.0 * four_alignment)
        + 0.50 * event_score
    )
    valid = bool(event.get("valid")) and mtf_score >= MTF_MIN_SCORE
    # A strongly opposite 4H correction is allowed only for the clearest level reaction.
    if four_alignment == 0.0 and str(event.get("state")) not in {"RETEST_CONFIRMED", "EMA_RETEST"}:
        valid = False

    return {
        "status": "OK" if valid else str(event.get("state", "WAIT")),
        "valid": valid,
        "reason": (
...
def add_active_mover_watch(setup: Dict[str, Any]) -> bool:
    ...
    V20.3.6 FAST START/FAST SCAN: the OI baseline is captured lazily only
...
def process_active_mover_watches() -> Dict[str, int]:
    ...
    for item in snapshot:
        ...
        # V20.3.6 mandatory top-down hierarchy:
        ...
        gate_reason += (
            f" · MTF {setup['mtf_d1_trend']}→{setup['mtf_4h_trend']}→{setup['mtf_1h_state']} "
        ...
        if float(trade.get("scalp_research_score",0.0) or 0.0) < ACTIVE_RESEARCH_SCORE_MIN:
            trade["paper_validation_origin"] += (
                f" · HIDDEN RESEARCH FILTER: score {float(trade.get('scalp_research_score',0.0)):.0f} "
                f"< {ACTIVE_RESEARCH_SCORE_MIN:.0f}; "
...
async def startup_event():
    """V20.3.6 instant Render startup.
