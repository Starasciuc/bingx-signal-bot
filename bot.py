...
A_PLUS_VOLUME_RATIO = float(os.getenv("A_PLUS_VOLUME_RATIO", "1.35"))
...
MIN_RR_TO_TP1 = float(os.getenv("MIN_RR_TO_TP1", "0.75"))
...
def nearest_resistance(price, candles_1h, candles_4h, max_distance=1.50):
    ...
    for level in highs:
        ...
        if distance <= max_distance and level >= price * 0.980:
            levels.append(level)
...
def nearest_support(price, candles_1h, candles_4h, max_distance=1.50):
    ...
    for level in lows:
        ...
        if distance <= max_distance and level <= price * 1.020:
            levels.append(level)
...
def build_signal_base(symbol, side, strategy, entry, sl, volume_ratio, quality):
    ...
    signal_id = f"{symbol}:V23_BALANCED:{strategy}:{side}:{round(entry, 6)}"
...
def build_breakout_strategy(symbol, side, candles_15m, candles_1m, candles_5m, candles_1h, candles_4h, btc_status):
    ...
    if side == "LONG":
        ...
        if recent_move_percent(candles_15m, 12) < 0.45:
            return None
        ...
        breakout = price > resistance * 1.0012 and prev["close"] <= resistance * 1.004
        ...
        sl = min(resistance - atr * 0.10, min(lows[-6:]) - atr * 0.04)
        quality = 84
        ...
        breakout = price < support * 0.9988 and prev["close"] >= support * 0.996
        ...
        sl = max(support + atr * 0.10, max(highs[-6:]) + atr * 0.04)
        quality = 84
    ...
    if trend_4h in ["BULLISH", "SOFT_BULLISH"] and side == "LONG":
        quality += 1

    if trend_4h in ["BEARISH", "SOFT_BEARISH"] and side == "SHORT":
        quality += 1
...
def build_sweep_retest_strategy(symbol, side, candles_15m, candles_1m, candles_5m, candles_1h, candles_4h, btc_status):
    ...
    if side == "LONG":
        ...
        swept = prev["low"] < support * 0.9985
        reclaimed = prev["close"] > support
        retest_hold = last["low"] >= support * 0.995 and last["close"] > support
        ...
        swept = prev["high"] > resistance * 1.0015
        reclaimed = prev["close"] < resistance
        retest_hold = last["high"] <= resistance * 1.005 and last["close"] < resistance
...
def build_reversal_strategy(symbol, side, candles_15m, candles_1m, candles_5m, candles_1h, candles_4h, btc_status):
    ...
    if side == "LONG":
        ...
        if recent_move_percent(candles_15m, 12) > -1.0:
            return None
        ...
        touch = last["high"] >= resistance * 0.994
...
def make_signal_message(signal):
    ...
    return f"""
🎯 <b>V23 Balanced Adaptive Strategy Manager</b> · <b>{mode_text}</b>
{strategy_name}
...
def apply_adaptive_rules(signal, result):
    ...
    if result == "SL" and not signal.get("counted_sl"):
        ...
        if result in ["TP1", "PROFIT_AFTER_TP1"]:
            STATS["side"][side]["tp1"] += 1
            STATS["strategy"][strategy]["tp1"] += 1

        elif result in ["TP2", "PROFIT_AFTER_TP2"]:
            STATS["side"][side]["tp2"] += 1
            STATS["strategy"][strategy]["tp2"] += 1

        elif result == "TP3":
            STATS["side"][side]["tp3"] += 1
            STATS["strategy"][strategy]["tp3"] += 1
