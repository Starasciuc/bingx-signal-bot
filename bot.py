# VERIFIED GITHUB DEPLOY FILE — V21.2 PRO CHAMPION · LONG CONTINUATION + CONFIRMED ENTRY
...
DB_PATH = os.getenv("ADAPTIVE_DB_PATH", "adaptive_bot_v21_2.sqlite3")
# Begin the first quality-model cycle only after a large fresh LONG cohort. The independent newest
# test still decides whether the challenger is strong enough to affect signals.
MIN_TRAIN_TRADES = max(150, int(os.getenv("ADAPTIVE_MIN_TRAIN_TRADES", "150")))
MIN_RESOLVED_MODEL_ROWS = max(80, int(os.getenv("ADAPTIVE_MIN_RESOLVED_MODEL_ROWS", "80")))
...
MIN_SELECTED_TEST_ROWS = max(8, int(os.getenv("ADAPTIVE_MIN_SELECTED_TEST_ROWS", "8")))
MIN_SELECTED_LIVE_TEST_ROWS = max(8, int(os.getenv("ADAPTIVE_MIN_SELECTED_LIVE_TEST_ROWS", "8")))
...
# V21 selection target: TP3+ rate must clear a meaningful floor while expectancy,
# coverage and independent forward validation remain positive. Risk/reward matters more than raw win rate.
ADAPTIVE_TARGET_SUCCESS_RATE = float(os.getenv("ADAPTIVE_TARGET_SUCCESS_RATE", "0.58"))
...
MODEL_ENABLED = True  # V21 self-learning enabled; real-money remains separately locked
MODEL_DATA_POLICY = "v21_2_long_first_confirmed_resolved_quality_v1"
...
SHADOW_ONLY = False  # V21: a validated champion may gate PAPER canary candidates after warm-up
...
FEATURE_NAMES: Tuple[str, ...] = (
    ...
    "scalp_cmf",
    "scalp_obv_pressure",
    "scalp_taker_imbalance",
    "scalp_oi_change",
    "scalp_funding_rate",
    "scalp_research_score",
    "mtf_d1_score",
    "mtf_score",
    "mtf_retest_quality",
    "mtf_role_reversal",
    "reference_match",
    "v21_level_retest_lane",
    "v21_followthrough_score",
)
...
def init_adaptive_db() -> None:
    with _LOCK, _connect() as conn:
        ...
        migrations = {
            ...
            "outcome_detail": "ALTER TABLE adaptive_trades ADD COLUMN outcome_detail TEXT",
        }
                ...
                if str(raw.get("data_policy", "legacy_all_sources")) != MODEL_DATA_POLICY:
                    eligible_count = int(
                        conn.execute(
                            ...
                            f"WHERE COALESCE(decision_reason, '') IN ({adaptive_training_reason_placeholders()})",
                            adaptive_training_reason_values(),
                        ).fetchone()["n"]
                        or 0
                    )
                    state.active = False
...
def build_feature_dict(trade: Dict[str, Any]) -> Dict[str, float]:
    ...
    features = {
        ...
        "scalp_cmf": _clip(trade.get("scalp_cmf"), -1.0, 1.0),
        "scalp_obv_pressure": _clip(trade.get("scalp_obv_pressure"), -1.0, 1.0),
        "scalp_taker_imbalance": _clip(trade.get("scalp_taker_imbalance"), -1.0, 1.0),
        "scalp_oi_change": _clip(trade.get("scalp_oi_change"), -0.20, 0.20) / 0.20,
        "scalp_funding_rate": _clip(trade.get("scalp_funding_rate"), -0.005, 0.005) / 0.005,
        "scalp_research_score": _clip(trade.get("scalp_research_score"), 0.0, 100.0) / 100.0,
        "mtf_d1_score": _clip(trade.get("mtf_d1_score"), -100.0, 100.0) / 100.0,
        "mtf_score": _clip(trade.get("mtf_score"), 0.0, 100.0) / 100.0,
        "mtf_retest_quality": _clip(trade.get("mtf_retest_quality"), 0.0, 1.0),
        "mtf_role_reversal": 1.0 if trade.get("mtf_role_reversal") else 0.0,
        "reference_match": _clip(trade.get("reference_match"), 0.0, 100.0) / 100.0,
        "v21_level_retest_lane": 1.0 if str(trade.get("v21_lane", "")).upper() == "LEVEL_RETEST" else 0.0,
    }
...
def predict_probability(trade: Dict[str, Any]) -> Tuple[Optional[float], ModelState]:
    ...
    if not MODEL_ENABLED or not state.active or state.trained_rows < MIN_RESOLVED_MODEL_ROWS:
        return None, state
...
def adaptive_gate(trade: Dict[str, Any]) -> Tuple[bool, str, Optional[float]]:
    strategy = str(trade.get("strategy", "")).upper()
    side = str(trade.get("side", "")).upper()
    if not (strategy.startswith("PRO_SPIKE_REGIME_") or strategy.startswith("PRO_ACTIVE_MOVER_")):
        return True, "adaptive model dedicated to V21.1 continuation lanes; bypass", None
    if side != "LONG":
        return True, "V21.2 SHORT is shadow-only; adaptive quality model is LONG-only", None

    probability, state = predict_probability(trade)
    if probability is None:
        return True, f"adaptive quality warm-up: {state.trained_rows}/{MIN_RESOLVED_MODEL_ROWS} resolved", None
...
def _classify_outcome_detail(signal: Dict[str, Any], result: str, pnl_r: float) -> str:
    """Separate dead expiries from useful partial follow-through.

    TP1/TP2 remain non-winning outcomes for the official TP3 statistic.  The
    subtype is diagnostic only and is used to understand why a continuation
    setup failed to finish.
    """
    if result == "profit":
        return "TP3_PLUS"
    if result == "sl":
        return "SL"
    tp1 = bool(signal.get("tp1_hit"))
    tp2 = bool(signal.get("tp2_hit"))
    mfe = float(signal.get("mfe_r", 0.0) or 0.0)
    if tp2:
        return "TP2_REVERSED" if pnl_r < 0 else "TP2_STALL"
    if tp1:
        return "TP1_REVERSED" if pnl_r < 0 else "TP1_STALL"
    if mfe >= 0.65:
        return "PARTIAL_MFE"
    if mfe < 0.20:
        return "DEAD"
...
def record_closed_trade(signal: Dict[str, Any], result: str, source: Optional[str] = None) -> Dict[str, Any]:
    ...
    outcome_detail = _classify_outcome_detail(signal, result, pnl_r)
    signal["outcome_detail"] = outcome_detail
    ...
    with _LOCK, _connect() as conn:
        cursor = conn.execute(
            ...
            INSERT OR IGNORE INTO adaptive_trades(
                ...
                decision_reason, outcome_detail
            ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """,
            (
                signal_id,
...
def _choose_threshold(labels: Sequence[int], probabilities: Sequence[float], pnl_r: Sequence[float]) -> Tuple[float, Dict[str, float]]:
    ...
    for raw in range(low, high + 1):
        ...
        # Do not optimize a candidate that cannot clear the V21 TP3+ target
        # even on calibration data. Coverage prevents a tiny cherry-picked sample.
...
def _collect_live_audit_metrics(state: ModelState) -> Dict[str, Any]:
    ...
    reasons = adaptive_training_reason_values()
    with _LOCK, _connect() as conn:
        ...
        post_rows = conn.execute(
            ...
            FROM adaptive_trades
            WHERE id > ?
              AND COALESCE(decision_reason,'') IN ({placeholders})
            ORDER BY id ASC
            """,
...
def _training_attempt_failed(
    ...
    payload: Dict[str, Any] = {
        ...
        "quality_model_data": _outcome_metrics(rows),
...
def maybe_retrain(force: bool = False) -> Dict[str, Any]:
    ...
    # V21.2 is a fresh LONG-first experiment. All old versions remain audit-only.
    # Expired outcomes are crucial telemetry for the follow-through gate, but
    # they are NOT treated as directional losses by the TP3-vs-SL quality model.
    lane_rows = [
        row for row in all_rows
        if is_adaptive_training_reason(row["decision_reason"])
        and str(row["side"] or "").upper() == "LONG"
    ]
    closed_count = len(lane_rows)
    quality_rows = [row for row in lane_rows if str(row["result"] or "") in {"profit", "sl"}]
    paper_rows = [
        row for row in all_rows
        if str(row["decision_reason"] or "") in OFFICIAL_USER_PAPER_REASONS
        and str(row["side"] or "").upper() == "LONG"
    ]
    if PRO_QUALITY_FORWARD_ENABLED and len(paper_rows) < PAPER_LANE_REQUIRED_OUTCOMES:
        ...
        return {
            ...
            "training_data": _outcome_metrics(lane_rows),
            "quality_model_data": _outcome_metrics(quality_rows),
            "resolved_rows": len(quality_rows),
            "all_data": _outcome_metrics(all_rows),
            "paper_validation_data": _outcome_metrics(paper_rows),
            "source_breakdown": _source_breakdown(lane_rows),
            "model_data_policy": MODEL_DATA_POLICY,
    ...
    if closed_count < MIN_TRAIN_TRADES:
        return {
            ...
            "resolved_rows": len(quality_rows),
            "resolved_needed": MIN_RESOLVED_MODEL_ROWS,
            "training_data": _outcome_metrics(lane_rows),
            "quality_model_data": _outcome_metrics(quality_rows),
            "source_breakdown": _source_breakdown(lane_rows),
            "all_closed_count": len(all_rows),
    ...
    if len(quality_rows) < MIN_RESOLVED_MODEL_ROWS:
        return _training_attempt_failed(
            state, "not_enough_resolved_quality_rows", closed_count, quality_rows,
            {
                "resolved_rows": len(quality_rows),
                "resolved_needed": MIN_RESOLVED_MODEL_ROWS,
                "continuation_data": _outcome_metrics(lane_rows),
            },
            all_rows,
        )
...
ADAPTIVE_REASON_RU: Dict[str, str] = {
    "warmup": "ещё не накоплено 150 чистых V21.1 LONG-наблюдений",
    "class_imbalance": "слишком мало положительных или отрицательных примеров",
    "not_enough_resolved_quality_rows": "ещё мало решённых TP3/SL исходов для quality-модели; expired анализируются отдельно",
    ...
    "coverage_too_low": "модель отбирает слишком узкую выборку; нужно сохранить рабочее покрытие",
    "tp3_majority_not_reached": "TP3+ ниже минимального V21 порога на независимой проверке",
    ...
    "forward_validation_freeze": "критерии зафиксированы до завершения независимой V21 forward-проверки",
...
def format_training_attempt_message(report: Dict[str, Any]) -> str:
    ...
    lines = [
        title,
        f"V21.1 LONG наблюдений: {closed_count}",
        f"ВСЕХ наблюдений, включая диагностический SHADOW: {all_closed_count}",
        f"LONG continuation telemetry: {_metrics_line(dataset)}",
        f"Причина: {_adaptive_reason_ru(reason)}",
    ]
    quality_dataset = report.get("quality_model_data") or {}
    if quality_dataset:
        lines.append(
            f"QUALITY MODEL · только TP3/SL: {_metrics_line(quality_dataset)} · "
            f"resolved={int(report.get('resolved_rows', quality_dataset.get('n', 0)) or 0)}"
        )
    if all_dataset:
        lines.append(f"ВСЯ ТЕЛЕМЕТРИЯ: {_metrics_line(all_dataset)}")
    if paper_dataset:
        lines.append(f"V21 continuation training cohort: {_metrics_line(paper_dataset)}")
...
APP_NAME = "Professional Adaptive Futures Bot AUTO V21.2 PRO CHAMPION · LONG CONTINUATION + CONFIRMED ENTRY"
DEPLOY_MARKER = "V21_2_PRO_CHAMPION_LONG_CONTINUATION_CONFIRMED_ENTRY_2026_09_12"
...
STATE_FILE = os.getenv("STATE_FILE", "bot_state_v21_2_long_first.json")
...
TRADER_STYLE_PAPER_REASON = "v21_2_level_retest_long_paper"
EXHAUSTION_PAPER_REASON = "v21_2_impulse_continuation_long_paper"  # compatibility name; exhaustion is disabled
V21_LEVEL_SHORT_SHADOW_REASON = "v21_2_level_retest_short_shadow"
V21_IMPULSE_SHORT_SHADOW_REASON = "v21_2_impulse_continuation_short_shadow"
V21_FOLLOWTHROUGH_FILTER_REASON = "v21_2_followthrough_filter"
...
SPIKE_ADAPTIVE_BLOCK_REASON = "v21_adaptive_block"
SPIKE_STRATEGY_GUARD_REASON = "v21_strategy_guard"
ACTIVE_MOVER_GUARD_REASON = "v21_active_strategy_guard"
ACTIVE_RESEARCH_FILTER_REASON = "v21_research_filter"
ACTIVE_MTF_FILTER_REASON = "v21_mtf_level_filter"

# V21: only two continuation lanes are user-facing; all reversal/exhaustion lanes are disabled.
...
TRACKABLE_SHADOW_REASONS = OFFICIAL_USER_PAPER_REASONS | {
    ...
    ACTIVE_RESEARCH_FILTER_REASON,
    ACTIVE_MTF_FILTER_REASON,
    V21_LEVEL_SHORT_SHADOW_REASON,
    V21_IMPULSE_SHORT_SHADOW_REASON,
    V21_FOLLOWTHROUGH_FILTER_REASON,
}
V21_PORTFOLIO_GUARD_REASON = "v21_portfolio_guard"
...
TRACKABLE_SHADOW_REASONS.add(V21_PORTFOLIO_GUARD_REASON)

def is_adaptive_training_reason(reason: Any) -> bool:
    return str(reason or "") in ADAPTIVE_TRAINING_REASONS


def adaptive_training_reason_values() -> Tuple[str, ...]:
    return tuple(sorted(str(x) for x in ADAPTIVE_TRAINING_REASONS))


def adaptive_training_reason_placeholders() -> str:
    return ",".join("?" for _ in adaptive_training_reason_values())
...
REAL_MONEY_LIVE_REASON = "v21_micro_live"
...
PAPER_REVIEW_REQUIRED_OUTCOMES = int(
    os.getenv("PAPER_REVIEW_REQUIRED_OUTCOMES", "100")
)
...
PAPER_LANE_REQUIRED_OUTCOMES = max(100, int(os.getenv("PAPER_LANE_REQUIRED_OUTCOMES", "100")))
...
ACTIVE_MOVER_MIN_VOL1 = float(os.getenv("ACTIVE_MOVER_MIN_VOL1", "0.25"))
ACTIVE_MOVER_MIN_RANGE1 = float(os.getenv("ACTIVE_MOVER_MIN_RANGE1", "0.55"))
...
ACTIVE_MOVER_SOFT_EXPIRE_MINUTES = int(os.getenv("ACTIVE_MOVER_SOFT_EXPIRE_MINUTES", "15"))
ACTIVE_MOVER_HARD_EXPIRE_MINUTES = int(os.getenv("ACTIVE_MOVER_HARD_EXPIRE_MINUTES", "60"))
ACTIVE_MOVER_MIN_PROGRESS_AT_SOFT = float(os.getenv("ACTIVE_MOVER_MIN_PROGRESS_AT_SOFT", "0.15"))
ACTIVE_MOVER_MAX_ACTIVE = int(os.getenv("ACTIVE_MOVER_MAX_ACTIVE", "2"))
...
ACTIVE_WATCH_MIN_PULLBACK = float(os.getenv("ACTIVE_WATCH_MIN_PULLBACK", "0.0012"))
ACTIVE_WATCH_MAX_PULLBACK = float(os.getenv("ACTIVE_WATCH_MAX_PULLBACK", "0.0160"))
ACTIVE_WATCH_RECLAIM_FRACTION = float(os.getenv("ACTIVE_WATCH_RECLAIM_FRACTION", "0.45"))
ACTIVE_WATCH_MIN_REACCEL_1M = float(os.getenv("ACTIVE_WATCH_MIN_REACCEL_1M", "0.0005"))
ACTIVE_WATCH_MAX_CHASE = float(os.getenv("ACTIVE_WATCH_MAX_CHASE", "0.0060"))
ACTIVE_WATCH_MIN_VOL1 = float(os.getenv("ACTIVE_WATCH_MIN_VOL1", "0.42"))
ACTIVE_WATCH_MIN_RANGE1 = float(os.getenv("ACTIVE_WATCH_MIN_RANGE1", "0.60"))

# V20.3.5 research-backed confirmation score.
ACTIVE_RESEARCH_SCORE_MIN = float(os.getenv("ACTIVE_RESEARCH_SCORE_MIN", "52"))
ACTIVE_RESEARCH_FLOW_STRONG = float(os.getenv("ACTIVE_RESEARCH_FLOW_STRONG", "0.08"))
ACTIVE_RESEARCH_CMF_STRONG = float(os.getenv("ACTIVE_RESEARCH_CMF_STRONG", "0.05"))
ACTIVE_RESEARCH_OI_STRONG = float(os.getenv("ACTIVE_RESEARCH_OI_STRONG", "0.003"))
ACTIVE_RESEARCH_FUNDING_CROWDED = float(os.getenv("ACTIVE_RESEARCH_FUNDING_CROWDED", "0.0010"))
ACTIVE_RESEARCH_BREAKOUT_BUFFER = float(os.getenv("ACTIVE_RESEARCH_BREAKOUT_BUFFER", "0.0005"))

# V20.3.6.1 — top-down technical-analysis hierarchy.
# D1 decides directional bias. 4H identifies the major level/zone. 1H must show
# a bounce or a breakout -> role reversal -> retest before the 1m/5m scalp trigger is admitted.
MTF_D1_MIN_CANDLES = int(os.getenv("MTF_D1_MIN_CANDLES", "35"))
MTF_4H_MIN_CANDLES = int(os.getenv("MTF_4H_MIN_CANDLES", "45"))
MTF_1H_MIN_CANDLES = int(os.getenv("MTF_1H_MIN_CANDLES", "55"))
MTF_D1_TREND_SCORE_MIN = float(os.getenv("MTF_D1_TREND_SCORE_MIN", "44"))
MTF_LEVEL_ZONE_ATR = float(os.getenv("MTF_LEVEL_ZONE_ATR", "0.32"))
MTF_LEVEL_ZONE_MIN_PCT = float(os.getenv("MTF_LEVEL_ZONE_MIN_PCT", "0.0022"))
MTF_LEVEL_MAX_DISTANCE_ATR = float(os.getenv("MTF_LEVEL_MAX_DISTANCE_ATR", "1.60"))
MTF_BREAKOUT_CLOSE_BUFFER_ATR = float(os.getenv("MTF_BREAKOUT_CLOSE_BUFFER_ATR", "0.08"))
MTF_BREAKOUT_MIN_VOL = float(os.getenv("MTF_BREAKOUT_MIN_VOL", "1.00"))
MTF_RETEST_MAX_AGE_HOURS = int(os.getenv("MTF_RETEST_MAX_AGE_HOURS", "14"))
MTF_RETEST_TOUCH_ATR = float(os.getenv("MTF_RETEST_TOUCH_ATR", "0.38"))
MTF_MIN_SCORE = float(os.getenv("MTF_MIN_SCORE", "52"))
MTF_ALLOW_DYNAMIC_EMA_RETEST = os.getenv("MTF_ALLOW_DYNAMIC_EMA_RETEST", "true").lower() == "true"

# V20.3.6.1 BALANCED MTF: hard-block only strong higher-TF conflict.
MTF_STRONG_CONFLICT_SCORE = float(os.getenv("MTF_STRONG_CONFLICT_SCORE", "72"))
MTF_SIDEWAYS_D1_ALLOWED = os.getenv("MTF_SIDEWAYS_D1_ALLOWED", "true").lower() == "true"
MTF_A_TIER_SCORE = float(os.getenv("MTF_A_TIER_SCORE", "68"))
MTF_B_TIER_SCORE = float(os.getenv("MTF_B_TIER_SCORE", "54"))
MTF_A_RESEARCH_MIN = float(os.getenv("MTF_A_RESEARCH_MIN", "48"))
MTF_B_RESEARCH_MIN = float(os.getenv("MTF_B_RESEARCH_MIN", "55"))
MTF_ALLOW_BREAKOUT_HOLD = os.getenv("MTF_ALLOW_BREAKOUT_HOLD", "true").lower() == "true"
MTF_BREAKOUT_HOLD_MIN_SCORE = float(os.getenv("MTF_BREAKOUT_HOLD_MIN_SCORE", "66"))
...
# V20.3.2 professional candle/squeeze intelligence.
...
# --- V21 REFERENCE SCALP GEOMETRY ---
# Exact user-provided successful examples with complete target ladders cluster near:
# TP1 0.8-0.9%, TP2 1.4-1.5%, TP3 2.0-2.1%, TP4 ~3.0%, TP5 ~4.0-4.4%.
# This is a target/quality reference only. The distant ~8.5% area is REASSESSMENT,
# never an automatic averaging order.
REFERENCE_SCALP_ENABLED = os.getenv("REFERENCE_SCALP_ENABLED", "true").lower() == "true"
REFERENCE_SCALP_STRONG_SCORE = float(os.getenv("REFERENCE_SCALP_STRONG_SCORE", "66"))
REFERENCE_SCALP_SCORE_BONUS_MAX = float(os.getenv("REFERENCE_SCALP_SCORE_BONUS_MAX", "4"))
REFERENCE_TP1_MOVE = float(os.getenv("REFERENCE_TP1_MOVE", "0.0085"))
REFERENCE_TP2_MOVE = float(os.getenv("REFERENCE_TP2_MOVE", "0.0145"))
REFERENCE_TP3_MOVE = float(os.getenv("REFERENCE_TP3_MOVE", "0.0205"))
REFERENCE_TP4_MOVE = float(os.getenv("REFERENCE_TP4_MOVE", "0.0300"))
REFERENCE_TP5_MOVE = float(os.getenv("REFERENCE_TP5_MOVE", "0.0410"))
REFERENCE_REASSESS_MOVE = float(os.getenv("REFERENCE_REASSESS_MOVE", "0.0850"))
REFERENCE_ATR_BASE = float(os.getenv("REFERENCE_ATR_BASE", "0.0045"))
REFERENCE_TARGET_SCALE_MIN = float(os.getenv("REFERENCE_TARGET_SCALE_MIN", "0.92"))
REFERENCE_TARGET_SCALE_MAX = float(os.getenv("REFERENCE_TARGET_SCALE_MAX", "1.08"))


def reference_scalp_context(setup: Dict[str, Any], c1: List[Dict[str, float]], c5: List[Dict[str, float]]) -> Dict[str, Any]:
    side = str(setup.get("side", "")).upper()
    entry = float(setup.get("entry", 0.0) or 0.0)
    defaults = (REFERENCE_TP1_MOVE, REFERENCE_TP2_MOVE, REFERENCE_TP3_MOVE, REFERENCE_TP4_MOVE, REFERENCE_TP5_MOVE)
    if not REFERENCE_SCALP_ENABLED or side not in {"LONG", "SHORT"} or entry <= 0 or len(c5) < 16:
        return {"score":0.0,"strong":False,"target_moves":defaults,"reassess_price":0.0,"reason":"reference unavailable"}
    direction = 1.0 if side == "LONG" else -1.0
    a5 = max(atr(c5, 14), entry * 1e-8)
    atr_pct = a5 / entry
    scale = _clip(0.90 + 0.10 * (atr_pct / max(REFERENCE_ATR_BASE, 1e-9)), REFERENCE_TARGET_SCALE_MIN, REFERENCE_TARGET_SCALE_MAX, 1.0)
    target_moves = tuple(x * scale for x in defaults)
    score = 18.0
    reasons: List[str] = []
    intel = _clip(setup.get("intel_setup_quality"), 0.0, 100.0)
    score += intel * 0.18
    vol1 = _clip(setup.get("vol1"), 0.0, 8.0)
    range1 = _clip(setup.get("range1"), 0.0, 8.0)
    score += min(9.0, vol1 / 1.20 * 9.0) + min(7.0, range1 / 1.40 * 7.0)
    directional_3m = direction * float(setup.get("ch3m_1m", 0.0) or 0.0)
    if directional_3m > 0:
        score += min(9.0, directional_3m / 0.012 * 9.0); reasons.append(f"3m {directional_3m*100:.2f}%")
    else:
        score -= min(10.0, abs(directional_3m) / 0.010 * 10.0)
    pullback = float(setup.get("active_watch_pullback", setup.get("current_retrace", 0.0)) or 0.0)
    recovery = float(setup.get("active_watch_recovery", 0.0) or 0.0)
    if 0.0012 <= pullback <= 0.0200:
        score += 8.0; reasons.append(f"pullback {pullback*100:.2f}%")
    elif pullback > 0.035:
        score -= 7.0
    if recovery >= 0.50: score += min(5.0, recovery * 5.0)
    if bool(setup.get("mtf_role_reversal")): score += 7.0; reasons.append("role reversal")
    mtf_q = _clip(setup.get("mtf_score"), 0.0, 100.0)
    score += min(8.0, max(0.0, mtf_q - 45.0) * 0.16)
    flow = direction * float(setup.get("scalp_taker_imbalance", 0.0) or 0.0)
    if flow >= 0.08: score += 5.0; reasons.append(f"flow {flow:+.2f}")
    elif flow <= -0.25: score -= 7.0
    recent = c5[-24:]
    recent_hi = max(float(x["high"]) for x in recent); recent_lo = min(float(x["low"]) for x in recent)
    viability = ((recent_hi - recent_lo) / entry) / max(target_moves[-1], 1e-9)
    if viability >= 0.85: score += 8.0; reasons.append(f"range {viability:.1f}x")
    elif viability < 0.50: score -= 5.0
    score = _clip(score, 0.0, 100.0)
    reassess = entry * (1.0 - REFERENCE_REASSESS_MOVE) if side == "LONG" else entry * (1.0 + REFERENCE_REASSESS_MOVE)
    return {"score":score,"strong":score >= REFERENCE_SCALP_STRONG_SCORE,"target_moves":target_moves,"target_scale":scale,
            "reassess_price":reassess,"reason":f"reference {score:.0f}/100 · " + (", ".join(reasons[:6]) if reasons else "neutral")}


def _attach_reference_context(setup: Dict[str, Any], context: Dict[str, Any]) -> None:
    setup["reference_match"] = float(context.get("score", 0.0) or 0.0)
    setup["reference_strong"] = bool(context.get("strong", False))
    setup["reference_target_moves"] = list(context.get("target_moves") or ())
    setup["reference_reassess_price"] = float(context.get("reassess_price", 0.0) or 0.0)
    setup["reference_reason"] = str(context.get("reason", ""))


def _reference_target_moves(setup: Dict[str, Any], fallback: Tuple[float,float,float,float,float], blend: float) -> Tuple[float,float,float,float,float]:
    if not REFERENCE_SCALP_ENABLED or not bool(setup.get("reference_strong")):
        return fallback
    raw = setup.get("reference_target_moves") or ()
    if len(raw) != 5: return fallback
    b = _clip(blend, 0.0, 1.0, 0.0)
    blended = tuple((1.0-b)*float(old) + b*float(new) for old,new in zip(fallback,raw))
    floors=(0.0065,0.0115,0.0180,0.0260,0.0350); caps=(0.0105,0.0165,0.0225,0.0320,0.0450)
    clean=tuple(_clip(x,lo,hi,old) for x,lo,hi,old in zip(blended,floors,caps,fallback))
    return clean if all(clean[i] < clean[i+1] for i in range(4)) else fallback

SPIKE_EXHAUSTION_ENABLED = False  # V21 hard lock: reversal/exhaustion never becomes an official trade
...
EXHAUST_TP3_MOVE = float(os.getenv("SPIKE_TP3_MOVE", "0.0200"))
EXHAUST_TP4_MOVE = float(os.getenv("SPIKE_TP4_MOVE", "0.0300"))
EXHAUST_TP5_MOVE = float(os.getenv("SPIKE_TP5_MOVE", "0.0400"))
EXHAUST_MIN_SL_MOVE = float(os.getenv("SPIKE_MIN_SL_MOVE", "0.0090"))
EXHAUST_MAX_SL_MOVE = float(os.getenv("SPIKE_MAX_SL_MOVE", "0.0220"))
EXHAUST_SOFT_EXPIRE_MINUTES = int(os.getenv("SPIKE_SOFT_EXPIRE_MINUTES", "15"))
EXHAUST_HARD_EXPIRE_MINUTES = int(os.getenv("SPIKE_HARD_EXPIRE_MINUTES", "60"))
...
def load_state() -> Dict[str, Any]:
    ...
    try:
        ...
        base.setdefault("v21_entry_confirmations", {})
...
def real_money_readiness() -> Dict[str, Any]:
    ...
    gross_profit = sum(max(0.0, float(row["pnl_r"] or 0.0)) for row in rows)
    gross_loss = sum(max(0.0, -float(row["pnl_r"] or 0.0)) for row in rows)
    profit_factor = gross_profit / max(gross_loss, 1e-12) if rows else 0.0
    recent_gross_profit = sum(max(0.0, float(row["pnl_r"] or 0.0)) for row in recent_rows)
    recent_gross_loss = sum(max(0.0, -float(row["pnl_r"] or 0.0)) for row in recent_rows)
    recent_profit_factor = recent_gross_profit / max(recent_gross_loss, 1e-12) if recent_rows else 0.0
    symbol_counts: Dict[str, int] = {}
    for row in rows:
        sym = normalize_symbol(str(row["symbol"] or "?"))
        symbol_counts[sym] = symbol_counts.get(sym, 0) + 1
    max_symbol_share = (max(symbol_counts.values()) / len(rows)) if rows and symbol_counts else 0.0
    ...
    for side in ("LONG", "SHORT"):
        ...
        side_checks = {
            ...
            "side_tp3_success_rate": float(side_metrics.get("success_rate", 0.0) or 0.0)
            ...
            "side_recent_tp3_success_rate": float(
    ...
    checks = {
        ...
        "tp3_success_rate": success >= REAL_MONEY_MIN_SUCCESS_RATE,
        "positive_expectancy": expectancy >= REAL_MONEY_MIN_EXPECTANCY_R,
        "recent_block_complete": len(recent_rows) >= recent_window,
        "recent_tp3_success_rate": recent_success >= REAL_MONEY_MIN_SUCCESS_RATE,
        "recent_positive_expectancy": recent_expectancy > 0.0,
        "profit_factor": profit_factor >= 1.30,
        "recent_profit_factor": recent_profit_factor >= 1.10,
        "symbol_concentration": max_symbol_share <= 0.10,
...
def closed_outcome_progress_message(
    ...
    """Post-close stats for the two official V21 continuation lanes."""
    ...
    return (
        "📊 V21.1 LONG-FIRST · ЧИСТАЯ СТАТИСТИКА\n"
        f"Последний исход: {labels.get(result, result)} · {lane} · "
        f"{display_symbol(signal.get('symbol', '?'))}\n"
        f"🎯 LEVEL RETEST: {_metrics_line(active)}\n"
        f"🚀 IMPULSE CONTINUATION: {_metrics_line(exhaust)}\n"
        f"🎯 LONG-FIRST ИТОГО: {_metrics_line(total)}\n"
        f"{v21_expired_breakdown_text()}\n"
        f"SHORT SHADOW: {_metrics_line(v21_short_shadow_metrics())}\n"
...
def maybe_send_auto_backup() -> Dict[str, Any]:
    ...
    V21 milestones are based only on the two clean LONG continuation lanes,
    not on legacy/control/diagnostic observations in the database.
    """
    ...
    try:
        ...
        filename = (
            ...
            f"impulse{spike_count}_level{active_count}_{int(time.time())}.json"
        )
        caption = (
            f"🧠 V21 clean backup · official {official_count} · "
            f"IMPULSE {spike_count} · LEVEL {active_count} · total DB {total_closed}"
        )
...
def get_open_interest_value(symbol: str) -> Optional[float]:
    """Current BingX perpetual open interest in base asset."""
    data = get_json(
    ...
    if not isinstance(payload, dict):
        return None
    try:
        value = float(payload.get("openInterest") or 0.0)
        return value if value > 0 else None
    except Exception:
        return None


def get_funding_rate_value(symbol: str) -> Optional[float]:
    """Latest public perpetual funding rate. Context only; never a stand-alone signal."""
    symbol = normalize_symbol(symbol)
    for endpoint in (
        "/openApi/swap/v2/quote/fundingRate",
        "/openApi/swap/v2/quote/premiumIndex",
    ):
        data = get_json(endpoint, {"symbol": symbol, "limit": 1})
        payload: Any = data.get("data") if isinstance(data, dict) else None
        if isinstance(payload, list):
...
def recent_taker_imbalance(symbol: str, limit: int = 120) -> Optional[float]:
    """Aggressive buy-vs-sell flow from public recent trades.

    BingX `buyerMaker=true` means the buyer was passive maker, therefore the
    seller was the taker. `buyerMaker=false` means aggressive/taker buying.
    Output is bounded [-1,+1].
    """
    data = get_json(
        "/openApi/swap/v2/quote/trades",
        {"symbol": normalize_symbol(symbol), "limit": max(20, min(300, int(limit)))},
    )
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        try:
            quote = float(row.get("quoteQty") or 0.0)
            if quote <= 0:
                quote = float(row.get("price") or 0.0) * float(row.get("qty") or 0.0)
            if quote <= 0:
                continue
            maker_raw = row.get("isBuyerMaker")
            if maker_raw is None:
                maker_raw = row.get("buyerMaker")  # compatibility with older payload aliases
            if isinstance(maker_raw, str):
                buyer_is_maker = maker_raw.strip().lower() in {"true", "1", "yes"}
            else:
                buyer_is_maker = bool(maker_raw)
            if buyer_is_maker:
                taker_sell += quote
            else:
                taker_buy += quote
    ...
    if total <= 0:
        return None
    return max(-1.0, min(1.0, (taker_buy - taker_sell) / total))


def chaikin_money_flow(candles: List[Dict[str, float]], n: int = 20) -> float:
    """CMF-style close-location weighted volume, bounded about [-1,+1]."""
    if len(candles) < max(5, n):
        return 0.0
    rows = candles[-n:]
    weighted = 0.0
    volume = 0.0
    for c in rows:
        ...
        if rng <= 1e-12 or v <= 0:
            continue
        multiplier = ((cl - l) - (h - cl)) / rng
        weighted += multiplier * v
        volume += v
    return max(-1.0, min(1.0, weighted / max(volume, 1e-12)))


def obv_pressure(candles: List[Dict[str, float]], n: int = 20) -> float:
    """Normalized recent OBV direction, used as confirmation rather than prediction."""
    if len(candles) < max(5, n + 1):
        return 0.0
    rows = candles[-(n + 1):]
    signed = 0.0
    total = 0.0
    for prev, cur in zip(rows[:-1], rows[1:]):
        v = max(0.0, float(cur.get("volume", 0.0) or 0.0))
        total += v
        if float(cur["close"]) > float(prev["close"]):
            signed += v
        elif float(cur["close"]) < float(prev["close"]):
            signed -= v
    return max(-1.0, min(1.0, signed / max(total, 1e-12)))


def directional_reversal_candle_bonus(
    c1: List[Dict[str, float]], side: str
) -> float:
    """Small context bonus for engulf/rejection candles; never a stand-alone gate."""
    if len(c1) < 3:
        return 0.0
    side = str(side).upper()
    prev = c1[-2]
    ...
    upper = (float(last["high"]) - max(float(last["open"]), float(last["close"]))) / rng
    lower = (min(float(last["open"]), float(last["close"])) - float(last["low"])) / rng
    if side == "LONG":
        engulf = (
        ...
        rejection = lower >= 0.35 and body >= 0.25
    else:
        engulf = (
            float(last["close"]) < float(last["open"])
            and float(prev["close"]) > float(prev["open"])
        ...
        rejection = upper >= 0.35 and body >= 0.25
    return 1.0 if engulf else (0.6 if rejection else 0.0)


def scalp_research_score(
    setup: Dict[str, Any],
    c1: List[Dict[str, float]],
    c5: List[Dict[str, float]],
    c15: List[Dict[str, float]],
    *,
    watch_oi: Optional[float],
    current_oi: Optional[float],
    funding: Optional[float],
    taker_imbalance: Optional[float],
) -> Dict[str, Any]:
    """Score a pullback-continuation scalp without turning every indicator into a hard gate.

    Native HOT->pullback->reclaim->reacceleration remains the core setup.
    Research evidence only upgrades/downgrades quality, preserving frequency.
    """
    side = str(setup.get("side", "")).upper()
    direction = 1.0 if side == "LONG" else -1.0
    price = float(setup.get("entry", c1[-1]["close"]) or c1[-1]["close"])

    cmf = chaikin_money_flow(c5, 20)
    obv = obv_pressure(c5, 20)
    d_cmf = direction * cmf
    d_obv = direction * obv

    pullback = float(setup.get("active_watch_pullback", 0.0) or 0.0)
    recovery = float(setup.get("active_watch_recovery", 0.0) or 0.0)
    one = direction * spike_bar_return(c1, 1)
    three = direction * spike_bar_return(c1, 3)
    fifteen = direction * spike_bar_return(c5, 3)

    # Micro breakout/reclaim relative to the bars immediately before the trigger.
    prior = c1[-7:-1]
    if side == "LONG":
        swing = max(float(x["high"]) for x in prior) if prior else price
        breakout_distance = (price - swing) / max(swing, 1e-12)
    else:
        swing = min(float(x["low"]) for x in prior) if prior else price
        breakout_distance = (swing - price) / max(swing, 1e-12)

    # Pullback should normally cool participation, followed by re-expansion at trigger.
    prior_pause = c1[-4:-1]
    older = c1[-14:-4]
    pause_vol = (
        sum(float(x.get("volume", 0.0) or 0.0) for x in prior_pause) / len(prior_pause)
        if prior_pause else 0.0
    )
    older_vol = (
        sum(float(x.get("volume", 0.0) or 0.0) for x in older) / len(older)
        if older else max(pause_vol, 1.0)
    )
    pause_contraction = pause_vol / max(older_vol, 1e-12)

    oi_change: Optional[float] = None
    if watch_oi is not None and current_oi is not None and watch_oi > 0:
        oi_change = (current_oi - watch_oi) / watch_oi
    ...
    # Structure / momentum.
    if fifteen >= 0.004:
        score += 7; notes.append("15m trend")
    elif fifteen < -0.002:
        score -= 7; notes.append("15m conflict")

    if 0.0020 <= pullback <= 0.0090:
        score += 7; notes.append("healthy pullback")
    elif pullback > 0.014:
        score -= 5; notes.append("deep pullback")

    if 0.45 <= recovery <= 0.88:
        score += 5; notes.append("controlled reclaim")
    elif recovery > 0.96:
        score -= 2

    if one >= 0.0005 and three > 0:
        score += 7; notes.append("reacceleration")
    if breakout_distance >= ACTIVE_RESEARCH_BREAKOUT_BUFFER:
        score += 7; notes.append("micro breakout")
    elif breakout_distance < -0.0025:
        score -= 4

    # Volume/price confirmation.
    if d_cmf >= ACTIVE_RESEARCH_CMF_STRONG:
        score += 7; notes.append("CMF confirms")
    elif d_cmf < -0.08:
        score -= 6; notes.append("CMF diverges")

    if d_obv >= 0.08:
        score += 5; notes.append("OBV confirms")
    elif d_obv < -0.10:
        score -= 4

    if pause_contraction <= 0.85:
        score += 4; notes.append("pullback volume cooled")
    if directional_reversal_candle_bonus(c1, side) > 0:
        score += 3; notes.append("trigger candle")

    # Public trade flow. Unknown data is neutral, never an automatic rejection.
    if taker_imbalance is not None:
        d_flow = direction * taker_imbalance
        if d_flow >= ACTIVE_RESEARCH_FLOW_STRONG:
            score += 7; notes.append("taker flow confirms")
        elif d_flow <= -ACTIVE_RESEARCH_FLOW_STRONG:
            score -= 7; notes.append("taker flow opposes")

    # OI confirms trend participation when rising. Falling OI is not always bad:
    # during a squeeze it can represent forced/voluntary covering, so it is only a soft downgrade.
    if oi_change is not None:
        if oi_change >= ACTIVE_RESEARCH_OI_STRONG:
            score += 6; notes.append("OI expanding")
        elif oi_change <= -0.008:
            score -= 2; notes.append("OI contracting")

    # Funding is a crowding warning, not a direction signal.
    if funding is not None:
        if side == "LONG" and funding >= ACTIVE_RESEARCH_FUNDING_CROWDED:
            score -= 4; notes.append("crowded long funding")
        elif side == "SHORT" and funding <= -ACTIVE_RESEARCH_FUNDING_CROWDED:
            score -= 4; notes.append("crowded short funding")
        elif (side == "LONG" and funding < 0) or (side == "SHORT" and funding > 0):
            score += 2; notes.append("funding not crowded")

    # Existing candle-intelligence context: only a small penalty for climax.
    intel_quality = float(setup.get("intel_setup_quality", 0.0) or 0.0)
    intel_climax = float(setup.get("intel_climax", 0.0) or 0.0)
    if intel_quality >= 60:
        score += 4
    if intel_climax >= 82:
        score -= 6; notes.append("late climax")
    ...
    return {
        ...
        "pause_volume_ratio": pause_contraction,
        "notes": notes,
    }
...
# --- V21 PORTFOLIO RISK CIRCUIT ---
V21_MAX_CONSECUTIVE_SL = int(os.getenv("V21_MAX_CONSECUTIVE_SL", "3"))
V21_DAILY_LOSS_LIMIT_R = float(os.getenv("V21_DAILY_LOSS_LIMIT_R", "-3.0"))
V21_CIRCUIT_COOLDOWN_SECONDS = int(os.getenv("V21_CIRCUIT_COOLDOWN_SECONDS", "43200"))


def v21_portfolio_circuit_breaker() -> Tuple[bool, str]:
    now = now_ts()
    until = int(STATE.get("v21_circuit_until", 0) or 0)
    if until > now:
        return False, f"V21 circuit active for {(until-now)/60.0:.0f}m"
    try:
        init_adaptive_db()
        with _LOCK, _connect() as conn:
            rows = conn.execute(
                "SELECT result,pnl_r,closed_at FROM adaptive_trades WHERE COALESCE(decision_reason,'') IN (?,?) ORDER BY closed_at DESC,id DESC LIMIT 40",
                (TRADER_STYLE_PAPER_REASON, EXHAUSTION_PAPER_REASON),
            ).fetchall()
        consecutive = 0
        for row in rows:
            if str(row["result"]) == "sl": consecutive += 1
            else: break
        day_start = now - (now % 86400)
        daily_r = sum(float(row["pnl_r"] or 0.0) for row in rows if int(row["closed_at"] or 0) >= day_start)
        if consecutive >= max(1, V21_MAX_CONSECUTIVE_SL):
            with STATE_IO_LOCK:
                STATE["v21_circuit_until"] = now + max(3600, V21_CIRCUIT_COOLDOWN_SECONDS)
                STATE["v21_circuit_reason"] = f"{consecutive} consecutive SL"
                save_state()
            return False, STATE["v21_circuit_reason"]
        if daily_r <= V21_DAILY_LOSS_LIMIT_R:
            until = day_start + 86400
            with STATE_IO_LOCK:
                STATE["v21_circuit_until"] = until
                STATE["v21_circuit_reason"] = f"daily {daily_r:+.2f}R <= {V21_DAILY_LOSS_LIMIT_R:+.2f}R"
                save_state()
            return False, STATE["v21_circuit_reason"]
        return True, f"portfolio guard ok: streak={consecutive}, day={daily_r:+.2f}R"
    except Exception as exc:
        return True, f"portfolio guard fail-open PAPER: {repr(exc)}"


V21_FOLLOWTHROUGH_MIN_A = float(os.getenv("V21_FOLLOWTHROUGH_MIN_A", "56"))
V21_FOLLOWTHROUGH_MIN_B = float(os.getenv("V21_FOLLOWTHROUGH_MIN_B", "61"))
V21_FOLLOWTHROUGH_MIN_IMPULSE = float(os.getenv("V21_FOLLOWTHROUGH_MIN_IMPULSE", "58"))


def v21_followthrough_score(setup: Dict[str, Any]) -> Tuple[float, str]:
    """Estimate whether a valid continuation has enough *current* energy.

    This is deliberately a soft composite. Missing OI/flow is neutral; only the
    existing hard safety gates can veto a trade. The goal is to reduce DEAD
    expiries without recreating the silent-bot problem.
    """
    side = str(setup.get("side", "")).upper()
    direction = 1.0 if side == "LONG" else -1.0
    lane = str(setup.get("v21_lane", ""))
    score = 34.0
    notes = []

    # Entry is after pullback/reclaim; reward decisive recovery, not raw chase.
    recovery = float(setup.get("active_watch_recovery", 0.0) or 0.0)
    if recovery > 0:
        score += min(9.0, max(0.0, (recovery - 0.35) * 22.0))
        notes.append(f"recovery={recovery:.2f}")

    mom1 = direction * float(setup.get("entry_mom1", 0.0) or 0.0)
    mom3 = direction * float(setup.get("entry_mom3", setup.get("ch3m_1m", 0.0)) or 0.0)
    score += min(8.0, max(0.0, mom1) * 4000.0)
    score += min(7.0, max(0.0, mom3) * 1300.0)

    vol1 = float(setup.get("entry_vol1_raw", setup.get("vol1", 0.0)) or 0.0)
    range1 = float(setup.get("range1", 0.0) or 0.0)
    score += min(7.0, max(0.0, vol1 - 0.35) * 6.0)
    score += min(6.0, max(0.0, range1 - 0.55) * 4.5)

    intel = float(setup.get("intel_setup_quality", 0.0) or 0.0)
    score += min(6.0, max(0.0, intel - 45.0) * 0.12)

    # Top-down level quality matters most for the LEVEL_RETEST lane.
    mtf = float(setup.get("mtf_score", 0.0) or 0.0)
    retest = float(setup.get("mtf_retest_quality", 0.0) or 0.0)
    if lane == "LEVEL_RETEST":
        score += min(8.0, max(0.0, mtf - 50.0) * 0.18)
        score += min(6.0, max(0.0, retest) * 6.0)
        if bool(setup.get("mtf_role_reversal")):
            score += 4.0
    else:
        # Impulse lane earns its context from the spike itself.
        score += min(7.0, max(0.0, float(setup.get("spike_volume_pace", 1.0) or 1.0) - 1.0) * 2.5)
        score += min(6.0, max(0.0, float(setup.get("spike_tf_agreement", 1.0) or 1.0) - 1.0) * 2.5)
        score += min(6.0, max(0.0, float(setup.get("regime_cont_score", 0.0) or 0.0) - 3.0) * 1.6)

    research = float(setup.get("scalp_research_score", 50.0) or 50.0)
    score += min(6.0, max(0.0, research - 50.0) * 0.12)

    # Independent order-flow confirmations are bonuses/penalties, not mandatory.
    flow_terms = [
        direction * float(setup.get("scalp_taker_imbalance", 0.0) or 0.0),
        direction * float(setup.get("scalp_cmf", 0.0) or 0.0),
        direction * float(setup.get("scalp_obv_pressure", 0.0) or 0.0),
    ]
    aligned = sum(1 for x in flow_terms if x >= 0.04)
    opposed = sum(1 for x in flow_terms if x <= -0.08)
    score += aligned * 2.0
    score -= opposed * 2.5

    oi = float(setup.get("scalp_oi_change", 0.0) or 0.0)
    if oi > 0.002:
        score += min(3.0, oi * 300.0)

    # Penalize late/chasing contexts without hard-blocking a high-quality retest.
    overheat = float(setup.get("intel_overheat", 0.0) or 0.0)
    if overheat > 75:
        score -= min(8.0, (overheat - 75.0) * 0.22)
    chase = abs(float(setup.get("scalp_breakout_distance", 0.0) or 0.0))
    if chase > 0.008:
        score -= min(8.0, (chase - 0.008) * 500.0)

    score = max(0.0, min(100.0, score))
    notes.append(f"mom1={mom1*100:+.2f}%")
    notes.append(f"mom3={mom3*100:+.2f}%")
    notes.append(f"vol1={vol1:.2f}")
    notes.append(f"flow={aligned}-{opposed}")
    return score, ", ".join(notes)



# --- V21.2 EMPIRICAL CONTINUATION QUALITY -----------------------------------
# Historical evidence is strongest for LONG continuation. The best old
# continuation-LONG outcomes had multi-TF agreement and were not the most
# vertically stretched/late spikes. This guard is intentionally multi-factor:
# one large volume reading alone never rejects a trade.
V21_IMPULSE_REQUIRE_TF_AGREEMENT = int(os.getenv("V21_IMPULSE_REQUIRE_TF_AGREEMENT", "2"))
V21_IMPULSE_LATE_VOLUME_PACE = float(os.getenv("V21_IMPULSE_LATE_VOLUME_PACE", "7.0"))
V21_IMPULSE_LATE_BASE_DISTANCE = float(os.getenv("V21_IMPULSE_LATE_BASE_DISTANCE", "0.10"))
V21_IMPULSE_LATE_OVERHEAT = float(os.getenv("V21_IMPULSE_LATE_OVERHEAT", "84"))


def v21_empirical_impulse_guard(setup: Dict[str, Any]) -> Tuple[bool, str]:
    if str(setup.get("v21_lane", "")) != "IMPULSE_CONTINUATION":
        return True, "empirical guard n/a"

    side = str(setup.get("side", "")).upper()
    if side != "LONG":
        # SHORT is shadow-only anyway; keep collecting unbiased evidence.
        return True, "SHORT shadow evidence retained"

    tf = int(setup.get("spike_tf_agreement", 0) or 0)
    pace = float(setup.get("spike_volume_pace", 0.0) or 0.0)
    base_dist = float(setup.get("spike_base_distance", 0.0) or 0.0)
    overheat = float(setup.get("intel_overheat", 0.0) or 0.0)

    if tf < V21_IMPULSE_REQUIRE_TF_AGREEMENT:
        return False, f"empirical guard: TF agreement {tf} < {V21_IMPULSE_REQUIRE_TF_AGREEMENT}"

    late_blowoff = (
        pace >= V21_IMPULSE_LATE_VOLUME_PACE
        and base_dist >= V21_IMPULSE_LATE_BASE_DISTANCE
    )
    late_overheat = (
        overheat >= V21_IMPULSE_LATE_OVERHEAT
        and base_dist >= V21_IMPULSE_LATE_BASE_DISTANCE * 0.80
    )
    if late_blowoff or late_overheat:
        return False, (
            f"empirical anti-chase: volpace={pace:.1f}x · "
            f"baseDist={base_dist*100:.1f}% · overheat={overheat:.0f}"
        )

    return True, (
        f"empirical continuation ok: TF={tf} · volpace={pace:.1f}x · "
        f"baseDist={base_dist*100:.1f}% · overheat={overheat:.0f}"
    )


def v21_followthrough_gate(setup: Dict[str, Any]) -> Tuple[bool, str, float]:
    score, detail = v21_followthrough_score(setup)
    lane = str(setup.get("v21_lane", ""))
    if lane == "LEVEL_RETEST":
        threshold = V21_FOLLOWTHROUGH_MIN_A if str(setup.get("mtf_tier", "B")) == "A" else V21_FOLLOWTHROUGH_MIN_B
    else:
        threshold = V21_FOLLOWTHROUGH_MIN_IMPULSE
    return score >= threshold, f"follow-through {score:.0f}/{threshold:.0f}: {detail}", score


def v21_flow_gate(setup: Dict[str, Any]) -> Tuple[bool, str]:
    """V21.2 order-flow conflict gate.

    V21.1 blocked only when taker+CMF+OBV were *all* strongly opposite. That was
    too permissive for a scalp entry. V21.2 blocks a candidate when at least two
    genuinely available flow dimensions are materially opposed. Missing public
    data remain neutral, so this does not recreate the silent-bot problem.
    """
    side = str(setup.get("side", "")).upper()
    direction = 1.0 if side == "LONG" else -1.0
    taker_raw = float(setup.get("scalp_taker_imbalance", 0.0) or 0.0)
    cmf_raw = float(setup.get("scalp_cmf", 0.0) or 0.0)
    obv_raw = float(setup.get("scalp_obv_pressure", 0.0) or 0.0)
    taker = direction * taker_raw
    cmf = direction * cmf_raw
    obv = direction * obv_raw

    available = [
        ("taker", taker, abs(taker_raw) > 1e-6, -0.18, 0.06),
        ("cmf", cmf, abs(cmf_raw) > 1e-6, -0.06, 0.03),
        ("obv", obv, abs(obv_raw) > 1e-6, -0.08, 0.04),
    ]
    opposed = [name for name, value, present, neg, pos in available if present and value <= neg]
    aligned = [name for name, value, present, neg, pos in available if present and value >= pos]
    hard = len(opposed) >= 2
    if hard:
        return False, (
            f"flow conflict 2-of-3: opposed={','.join(opposed)} · "
            f"taker={taker:+.2f} cmf={cmf:+.2f} obv={obv:+.2f}"
        )
    return True, (
        f"flow ok aligned={','.join(aligned) or 'neutral'} · "
        f"taker={taker:+.2f} cmf={cmf:+.2f} obv={obv:+.2f}"
    )


# --- V21.2 TEMPORAL ENTRY CONFIRMATION ---------------------------------------
# V21.1 already finds the desired setup. The remaining failure mode was timing:
# a candidate could satisfy the final gates on one transient market snapshot.
# V21.2 therefore requires the *same qualified setup* to survive a second scan.
V21_ENTRY_CONFIRM_ENABLED = os.getenv("V21_ENTRY_CONFIRM_ENABLED", "true").lower() == "true"
V21_ENTRY_CONFIRM_MIN_SECONDS = float(os.getenv("V21_ENTRY_CONFIRM_MIN_SECONDS", "8"))
V21_ENTRY_CONFIRM_MAX_SECONDS = float(os.getenv("V21_ENTRY_CONFIRM_MAX_SECONDS", "50"))
V21_ENTRY_CONFIRM_MAX_CHASE = float(os.getenv("V21_ENTRY_CONFIRM_MAX_CHASE", "0.0040"))
V21_ENTRY_CONFIRM_MAX_SCORE_DROP = float(os.getenv("V21_ENTRY_CONFIRM_MAX_SCORE_DROP", "6"))
V21_ENTRY_CONFIRM_MIN_MOM1 = float(os.getenv("V21_ENTRY_CONFIRM_MIN_MOM1", "-0.0002"))
V21_ENTRY_CONFIRM_MIN_MOM3 = float(os.getenv("V21_ENTRY_CONFIRM_MIN_MOM3", "0.0000"))


def _v21_entry_confirmation_key(trade: Dict[str, Any], lane: str) -> str:
    return (
        f"{normalize_symbol(str(trade.get('symbol','?')))}:"
        f"{str(trade.get('side','?')).upper()}:{str(lane).upper()}"
    )


def _v21_clear_stale_entry_confirmations(now: Optional[int] = None) -> None:
    now = int(now or now_ts())
    cache = STATE.setdefault("v21_entry_confirmations", {})
    stale = [
        key for key, row in cache.items()
        if now - int((row or {}).get("armed_at", now) or now)
        > max(60, int(V21_ENTRY_CONFIRM_MAX_SECONDS * 3))
    ]
    for key in stale[:200]:
        cache.pop(key, None)


def v21_temporal_entry_confirmation(
    trade: Dict[str, Any],
    lane: str,
) -> Tuple[bool, str]:
    """Require a second independent V21.1-quality observation before PAPER entry.

    This does not replace MTF/retest/impulse/follow-through logic. It only stops
    a one-snapshot burst from becoming an official trade. SHORT remains shadow
    and therefore does not need this official-risk confirmation layer.
    """
    if not V21_ENTRY_CONFIRM_ENABLED:
        return True, "V21.2 temporal confirmation disabled"

    side = str(trade.get("side", "")).upper()
    if side != "LONG":
        return True, "V21.2 temporal confirmation applies to official LONG only"

    now = now_ts()
    _v21_clear_stale_entry_confirmations(now)
    cache = STATE.setdefault("v21_entry_confirmations", {})
    key = _v21_entry_confirmation_key(trade, lane)

    entry = float(trade.get("entry", 0.0) or 0.0)
    follow = float(trade.get("v21_followthrough_score", 0.0) or 0.0)
    mom1 = float(trade.get("entry_mom1", trade.get("ch2m", 0.0)) or 0.0)
    mom3 = float(trade.get("entry_mom3", trade.get("ch3m_1m", 0.0)) or 0.0)
    level = float(trade.get("mtf_level", trade.get("level", 0.0)) or 0.0)
    flow = float(trade.get("scalp_taker_imbalance", 0.0) or 0.0)
    cmf = float(trade.get("scalp_cmf", 0.0) or 0.0)
    obv = float(trade.get("scalp_obv_pressure", 0.0) or 0.0)
    ...
    if not isinstance(prior, dict):
        ...
        with STATE_IO_LOCK:
            STATE["v21_entry_confirmations"] = cache
            save_state()
        return False, (
            f"V21.2 confirmation armed; wait ≥{V21_ENTRY_CONFIRM_MIN_SECONDS:.0f}s "
            f"for independent follow-through"
        )

    age = max(0.0, now - int(prior.get("armed_at", now) or now))
    if age < V21_ENTRY_CONFIRM_MIN_SECONDS:
        return False, f"V21.2 confirmation wait {age:.0f}/{V21_ENTRY_CONFIRM_MIN_SECONDS:.0f}s"

    if age > V21_ENTRY_CONFIRM_MAX_SECONDS:
        cache.pop(key, None)
        with STATE_IO_LOCK:
            STATE["v21_entry_confirmations"] = cache
            save_state()
        return False, "V21.2 confirmation expired; setup must re-arm"

    first_entry = max(float(prior.get("entry", entry) or entry), 1e-12)
    chase = (entry / first_entry - 1.0) if entry > 0 else 0.0
    if chase > V21_ENTRY_CONFIRM_MAX_CHASE:
        cache.pop(key, None)
        with STATE_IO_LOCK:
            STATE["v21_entry_confirmations"] = cache
            save_state()
        return False, f"V21.2 confirmation rejected: chase {chase*100:.2f}%"

    first_follow = float(prior.get("follow", follow) or follow)
    if follow + V21_ENTRY_CONFIRM_MAX_SCORE_DROP < first_follow:
        cache.pop(key, None)
        with STATE_IO_LOCK:
            STATE["v21_entry_confirmations"] = cache
            save_state()
        return False, f"V21.2 confirmation rejected: follow-through fell {first_follow:.0f}→{follow:.0f}"

    # The price may retest the level, but the second observation must not show
    # renewed downside momentum. A tiny negative 1m print is tolerated.
    if mom1 < V21_ENTRY_CONFIRM_MIN_MOM1 or mom3 < V21_ENTRY_CONFIRM_MIN_MOM3:
        return False, (
            f"V21.2 waiting: momentum not stable "
            f"mom1={mom1*100:+.2f}% mom3={mom3*100:+.2f}%"
        )

    if level > 0 and entry < level * 0.998:
        cache.pop(key, None)
        with STATE_IO_LOCK:
            STATE["v21_entry_confirmations"] = cache
            save_state()
        return False, "V21.2 confirmation rejected: reclaimed level failed"

    # At least one current participation measure should be supportive, unless
    # this is a very strong A-tier level retest. Missing feeds remain neutral.
    participation = sum((
        flow >= 0.06,
        cmf >= 0.03,
        obv >= 0.04,
    ...
    strong_a = (
        str(trade.get("mtf_tier", "")).upper() == "A"
        and follow >= max(V21_FOLLOWTHROUGH_MIN_A + 8.0, 64.0)
    )
    if participation < 1 and not strong_a:
        return False, (
            f"V21.2 waiting: no fresh participation "
            f"flow={flow:+.2f} cmf={cmf:+.2f} obv={obv:+.2f}"
        )
    ...
    with STATE_IO_LOCK:
        ...
        save_state()
    return True, (
        f"V21.2 confirmed on second scan after {age:.0f}s · "
        f"follow={follow:.0f} · participation={participation}"
    )
...
def active_mover_setup(
    ...
    return {
        ...
        "trade_type": f"🎯 V21 LEVEL RETEST {side}",
        "v21_lane": "LEVEL_RETEST",
...
def _closed_tf_candles(
    candles: List[Dict[str, float]],
    tf_seconds: int,
) -> List[Dict[str, float]]:
    """Return only completed candles when the latest bar is still forming."""
    if not candles:
        return []
    rows = list(candles)
    try:
        last_open_ms = int(rows[-1].get("time", 0) or 0)
        now_ms = int(time.time() * 1000)
        if last_open_ms > 0 and now_ms < last_open_ms + int(tf_seconds * 1000):
            rows = rows[:-1]
    except Exception:
        pass
    return rows
...
def _trend_structure_score(candles: List[Dict[str, float]], direction: int) -> float:
    """Score HH/HL (bull) or LH/LL (bear) structure on completed candles."""
    if len(candles) < 8:
        return 0.0
    rows = candles[-10:]
    hits = 0
    ...
    for prev, cur in zip(rows[:-1], rows[1:]):
        ...
        if direction > 0:
            ...
            hits += int(cl <= pl)
        total += 2
    return 100.0 * hits / max(1, total)


def _tf_bias(candles: List[Dict[str, float]]) -> Dict[str, Any]:
    """EMA + price structure bias, intentionally simple and interpretable."""
    if len(candles) < 30:
        return {"bias": "UNKNOWN", "score": 0.0, "ema20": 0.0, "ema50": 0.0}
    cs = closes(candles)
    price = float(cs[-1])
    e20 = ema(cs[-80:], 20)
    e50 = ema(cs[-120:], 50)
    prior_e20 = ema(cs[-80:-3], 20) if len(cs) >= 35 else e20
    bull_structure = _trend_structure_score(candles, 1)
    bear_structure = _trend_structure_score(candles, -1)

    bull = 0.0
    bull += 24.0 if price > e20 else 0.0
    bull += 26.0 if e20 > e50 else 0.0
    bull += 14.0 if e20 > prior_e20 else 0.0
    bull += 0.36 * bull_structure

    bear = 0.0
    bear += 24.0 if price < e20 else 0.0
    bear += 26.0 if e20 < e50 else 0.0
    bear += 14.0 if e20 < prior_e20 else 0.0
    bear += 0.36 * bear_structure

    net = bull - bear
    if net >= MTF_D1_TREND_SCORE_MIN:
        bias = "BULL"
    elif net <= -MTF_D1_TREND_SCORE_MIN:
        bias = "BEAR"
    else:
        bias = "SIDEWAYS"
    return {
        ...
        "bull_score": min(100.0, bull),
        "bear_score": min(100.0, bear),
        "ema20": e20,
        "ema50": e50,
        "price": price,
...
def _pivot_points(
    ...
    """Local pivot highs/lows used to create support/resistance zones."""
    out: List[Dict[str, Any]] = []
    ...
    for i in range(left, len(candles) - right):
        ...
        hi = float(row["high"])
        lo = float(row["low"])
        left_rows = candles[i-left:i]
        right_rows = candles[i+1:i+1+right]
        if hi >= max(float(x["high"]) for x in left_rows + right_rows):
            out.append({"price": hi, "kind": "R", "index": i})
        if lo <= min(float(x["low"]) for x in left_rows + right_rows):
            out.append({"price": lo, "kind": "S", "index": i})
    return out


def _cluster_level_zones(
    c4: List[Dict[str, float]],
    c1h: List[Dict[str, float]],
    current_price: float,
) -> List[Dict[str, Any]]:
    """4H levels are primary; nearby 1H pivots reinforce them.

    Support/resistance is represented as a zone rather than a single exact price.
    """
    a4 = max(atr(c4, 14), current_price * 1e-6)
    half = max(a4 * MTF_LEVEL_ZONE_ATR, current_price * MTF_LEVEL_ZONE_MIN_PCT)
    piv4 = _pivot_points(c4[-140:], 2, 2)
    piv1 = _pivot_points(c1h[-180:], 2, 2)
    ...
    # Fallback for newer coins: recent 4H swing extrema still form candidate zones.
    if not raw and len(c4) >= 12:
        recent = c4[-24:]
        raw.extend([
            {"price": max(float(x["high"]) for x in recent), "weight": 2.5, "source": "4H", "kind": "R"},
            {"price": min(float(x["low"]) for x in recent), "weight": 2.5, "source": "4H", "kind": "S"},
        ])
    ...
    for p in sorted(raw, key=lambda x: x["price"]):
        matched = None
        for z in clusters:
            if abs(float(p["price"]) - float(z["center"])) <= half:
                matched = z
                break
        if matched is None:
            matched = {
                "center": float(p["price"]),
                "weighted_sum": float(p["price"]) * float(p["weight"]),
                "weight": float(p["weight"]),
                "touches4h": 1 if p["source"] == "4H" else 0,
                "touches1h": 1 if p["source"] == "1H" else 0,
                "support_marks": 1 if p["kind"] == "S" else 0,
                "resistance_marks": 1 if p["kind"] == "R" else 0,
            }
            clusters.append(matched)
        else:
            matched["weighted_sum"] += float(p["price"]) * float(p["weight"])
            matched["weight"] += float(p["weight"])
            matched["center"] = matched["weighted_sum"] / max(matched["weight"], 1e-12)
            matched["touches4h"] += 1 if p["source"] == "4H" else 0
            matched["touches1h"] += 1 if p["source"] == "1H" else 0
            matched["support_marks"] += 1 if p["kind"] == "S" else 0
            matched["resistance_marks"] += 1 if p["kind"] == "R" else 0

    result: List[Dict[str, Any]] = []
    for z in clusters:
        center = float(z["center"])
        # Require the level to be relevant to the current area, not ancient/far away.
        distance_atr = abs(current_price - center) / max(a4, 1e-12)
        if distance_atr > 8.0:
            continue
        strength = min(
            100.0,
            18.0 * float(z["touches4h"])
            + 5.0 * float(z["touches1h"])
            + 3.0 * min(5.0, float(z["weight"])),
        )
        result.append({
            **z,
            "center": center,
    ...
    return sorted(result, key=lambda z: abs(current_price - float(z["center"])))


def _relative_volume_at(candles: List[Dict[str, float]], index: int, lookback: int = 20) -> float:
    if index <= 0 or index >= len(candles):
        return 0.0
    start = max(0, index - lookback)
    prior = [
        float(x.get("volume", 0.0) or 0.0)
        for x in candles[start:index]
        if float(x.get("volume", 0.0) or 0.0) > 0
    ]
    if not prior:
        return 0.0
    med = _median(prior)
    return float(candles[index].get("volume", 0.0) or 0.0) / max(med, 1e-12)


def _directional_candle_confirmation(candle: Dict[str, float], side: str) -> float:
    """Pin/rejection/strong body confirmation, used only as one component."""
    o = float(candle["open"]); h = float(candle["high"])
    l = float(candle["low"]); cl = float(candle["close"])
    rng = max(h - l, 1e-12)
    body = abs(cl - o) / rng
    ...
    if side == "LONG":
        score = 0.0
        score += 0.55 if cl > o and body >= 0.45 else 0.0
        score += 0.45 if lower >= 0.32 and cl >= l + 0.62 * rng else 0.0
    else:
        score = 0.0
        score += 0.55 if cl < o and body >= 0.45 else 0.0
        score += 0.45 if upper >= 0.32 and cl <= l + 0.38 * rng else 0.0
    return min(1.0, score)


def _evaluate_level_event(
    c1h: List[Dict[str, float]],
    zones: List[Dict[str, Any]],
    side: str,
    current_price: float,
) -> Dict[str, Any]:
    """Classify current 1H relationship to 4H/1H zones.

    Valid entry contexts:
      - RETEST_CONFIRMED: resistance/support broke earlier, changed role, then retested.
      - LEVEL_BOUNCE: direct reaction from an established level in D1 trend direction.
      - EMA_RETEST: dynamic level fallback handled outside this function.

    A fresh BREAKOUT is explicitly distinguished from a RETEST and is not an entry.
    """
    side = side.upper()
    if len(c1h) < 25 or not zones:
        return {"state": "NO_LEVEL", "valid": False, "score": 0.0}

    a1 = max(atr(c1h, 14), current_price * 1e-6)
    max_dist = MTF_LEVEL_MAX_DISTANCE_ATR * a1
    recent = c1h[-min(len(c1h), 28):]
    ...
    for z in zones[:12]:
        ...
        lower = float(z["lower"])
        upper = float(z["upper"])
        if abs(current_price - center) > max_dist * 2.5:
            continue
        ...
        # Search completed recent 1H candles for a true close beyond the zone.
        for i in range(1, len(recent)):
            prev = recent[i - 1]
            cur = recent[i]
            prev_close = float(prev["close"])
            close = float(cur["close"])
            buffer = max(a1 * MTF_BREAKOUT_CLOSE_BUFFER_ATR, current_price * 0.0005)
            vol_ratio = _relative_volume_at(recent, i, 16)
            rng = max(float(cur["high"]) - float(cur["low"]), 1e-12)
            body = abs(float(cur["close"]) - float(cur["open"])) / rng

            if side == "LONG":
                crossed = prev_close <= upper and close > upper + buffer
            else:
                crossed = prev_close >= lower and close < lower - buffer

            if crossed:
                breakout_idx = i
                breakout_vol = vol_ratio
                breakout_quality = (
                    0.55 * min(1.5, vol_ratio / max(MTF_BREAKOUT_MIN_VOL, 1e-12))
                    + 0.45 * min(1.0, body / 0.55)
                )
                # A wick-only / low-volume push is a weak breakout.
                if vol_ratio < MTF_BREAKOUT_MIN_VOL or body < 0.35:
                    breakout_quality *= 0.55

        if breakout_idx is not None:
            # Role reversal: after the break, the former resistance/support is
            # expected to act as the opposite level on a retest.
            later = recent[breakout_idx + 1:]
            max_age = min(len(later), max(1, MTF_RETEST_MAX_AGE_HOURS))
            later = later[:max_age]
            for j, cur in enumerate(later, start=breakout_idx + 1):
                close = float(cur["close"])
                low = float(cur["low"]); high = float(cur["high"])
                touch_pad = a1 * MTF_RETEST_TOUCH_ATR
                if side == "LONG":
                    touched = low <= upper + touch_pad and high >= lower - touch_pad
                    held = close >= center
                    invalid = close < lower - touch_pad
                else:
                    touched = high >= lower - touch_pad and low <= upper + touch_pad
                    held = close <= center
                    invalid = close > upper + touch_pad
                if invalid:
                    false_break = True
                if touched and held:
                    candle_conf = _directional_candle_confirmation(cur, side)
                    rv = _relative_volume_at(recent, j, 16)
                    # Retest does not need huge volume; controlled volume plus a
                    # directional rejection is usually cleaner.
                    volume_calm = 1.0 if rv <= max(1.25, breakout_vol * 0.95) else 0.55
                    retest_quality = min(
                        1.0,
                        0.45 * candle_conf
                        + 0.30 * volume_calm
                        + 0.25 * min(1.0, float(z["strength"]) / 70.0),
                    )
                    retest_idx = j

        # Direct bounce from established support/resistance.
        last = recent[-1]
        low = float(last["low"]); high = float(last["high"]); close = float(last["close"])
        touch_pad = a1 * MTF_RETEST_TOUCH_ATR
        if side == "LONG":
            direct_touch = low <= upper + touch_pad and close >= center
        else:
            direct_touch = high >= lower - touch_pad and close <= center
        direct_conf = _directional_candle_confirmation(last, side) if direct_touch else 0.0

        if false_break:
            state = "FALSE_BREAKOUT"
            valid = False
            event_score = 20.0
        elif retest_idx is not None and breakout_quality >= 0.55:
            state = "RETEST_CONFIRMED"
            valid = True
            event_score = 72.0 + 18.0 * retest_quality
        elif breakout_idx is not None:
            # Balanced mode: normally wait for the retest. But if the breakout
            # closed strongly beyond the zone, volume confirmed, and price is
            # still holding beyond the broken level, admit it as a lower-tier
            # breakout-hold candidate. The 1m/5m pullback/reclaim gate still
            # has to confirm later, so this is not a blind chase entry.
            if side == "LONG":
                still_holding = current_price >= upper
            else:
                still_holding = current_price <= lower
            if (
                MTF_ALLOW_BREAKOUT_HOLD
                and breakout_quality >= 0.78
                and breakout_vol >= MTF_BREAKOUT_MIN_VOL
                and still_holding
            ):
                state = "BREAKOUT_HOLD"
                valid = True
                event_score = max(MTF_BREAKOUT_HOLD_MIN_SCORE, 60.0 + 20.0 * breakout_quality)
            else:
                state = "BREAKOUT_WAIT_RETEST"
                valid = False
                event_score = 48.0 + 18.0 * breakout_quality
        elif direct_touch and direct_conf >= 0.45:
            state = "LEVEL_BOUNCE"
            valid = True
            event_score = 62.0 + 18.0 * direct_conf
        else:
            state = "NEAR_LEVEL_WAIT"
            valid = False
            event_score = 35.0

        distance_score = max(0.0, 100.0 - abs(current_price - center) / max(a1, 1e-12) * 35.0)
        combined = (
            0.55 * event_score
            + 0.25 * float(z["strength"])
            + 0.20 * distance_score
        )
        candidate = {
            "state": state,
            ...
            "zone_low": lower,
            "zone_high": upper,
            "level_strength": float(z["strength"]),
            "breakout_volume": breakout_vol,
            "breakout_quality": breakout_quality,
            "retest_quality": retest_quality,
            "distance_atr1h": abs(current_price - center) / max(a1, 1e-12),
            "role_reversal": bool(retest_idx is not None),
        }
        if best is None or float(candidate["score"]) > float(best["score"]):
            best = candidate

    return best or {"state": "NO_LEVEL", "valid": False, "score": 0.0}


def top_down_level_context(
    symbol: str,
    side: str,
    current_price: float,
) -> Dict[str, Any]:
    ...
    Network usage is lazy: this is called only after the native 1m/5m
    pullback/reclaim/reacceleration trigger exists, so FAST START/full-universe
    scanning is not slowed down.
    """
    side = side.upper()

    # 1) D1 first. If the global trend disagrees, we do not even fetch 4H/1H.
    d1_raw = get_klines(symbol, "1d", 120, cache_seconds=900)
    d1 = _closed_tf_candles(d1_raw or [], 86400)
    if len(d1) < MTF_D1_MIN_CANDLES:
        return {"status": "DATA_WAIT", "valid": False, "reason": "not enough D1 candles"}

    d1_bias = _tf_bias(d1)
    wanted = "BULL" if side == "LONG" else "BEAR"
    d1_name = str(d1_bias.get("bias", "UNKNOWN"))
    d1_score = float(d1_bias.get("score", 0.0) or 0.0)

    # Balanced rule:
    # - matching D1 = best;
    # - SIDEWAYS D1 may proceed to 4H/1H if enabled;
    # - opposite D1 blocks only when the conflict is strong.
    strong_conflict = (
        (side == "LONG" and d1_score <= -MTF_STRONG_CONFLICT_SCORE)
        or (side == "SHORT" and d1_score >= MTF_STRONG_CONFLICT_SCORE)
    )
    if strong_conflict:
        return {
            "status": "D1_STRONG_CONFLICT",
            "valid": False,
            "reason": f"D1 strong conflict {d1_name}({d1_score:+.0f}) vs {wanted}",
            "d1_bias": d1_bias,
        }
    if d1_name == "SIDEWAYS" and not MTF_SIDEWAYS_D1_ALLOWED:
        return {
            "status": "D1_SIDEWAYS_BLOCK",
            "valid": False,
            "reason": "D1 sideways",
            "d1_bias": d1_bias,
        }

    # 2) After D1 is not strongly conflicting: fetch 4H and 1H.
    c4_raw = get_klines(symbol, "4h", 180, cache_seconds=300)
    c1h_raw = get_klines(symbol, "1h", 220, cache_seconds=120)
    c4 = _closed_tf_candles(c4_raw or [], 4 * 3600)
    c1h = _closed_tf_candles(c1h_raw or [], 3600)
    if len(c4) < MTF_4H_MIN_CANDLES or len(c1h) < MTF_1H_MIN_CANDLES:
        return {
            "status": "DATA_WAIT",
            "valid": False,
            "reason": "not enough 4H/1H candles",
            "d1_bias": d1_bias,
        }

    four = _tf_bias(c4)
    zones = _cluster_level_zones(c4, c1h, current_price)
    event = _evaluate_level_event(c1h, zones, side, current_price)

    # Dynamic level fallback from the video chapter on moving averages:
    # if no clean horizontal event exists, allow a confirmed retest of 4H EMA20
    # in the D1 trend direction. This keeps the system from becoming silent.
    dynamic = None
    if MTF_ALLOW_DYNAMIC_EMA_RETEST and not bool(event.get("valid")):
        a1 = max(atr(c1h, 14), current_price * 1e-6)
        e4 = float(four.get("ema20", 0.0) or 0.0)
        if e4 > 0 and abs(current_price - e4) <= 0.65 * a1:
            last = c1h[-1]
            conf = _directional_candle_confirmation(last, side)
            if side == "LONG":
                touched = float(last["low"]) <= e4 + 0.30 * a1 and float(last["close"]) >= e4
            else:
                touched = float(last["high"]) >= e4 - 0.30 * a1 and float(last["close"]) <= e4
            if touched and conf >= 0.45:
                dynamic = {
                    "state": "EMA_RETEST",
                    "valid": True,
                    "score": 64.0 + 18.0 * conf,
                    "level": e4,
                    "zone_low": e4 - 0.25 * a1,
                    "zone_high": e4 + 0.25 * a1,
                    "level_strength": 55.0,
                    "breakout_volume": 0.0,
                    "breakout_quality": 0.0,
                    "retest_quality": conf,
                    "distance_atr1h": abs(current_price - e4) / max(a1, 1e-12),
                    "role_reversal": False,
                }
    if dynamic and float(dynamic["score"]) > float(event.get("score", 0.0) or 0.0):
        event = dynamic

    # 4H may be in a correction while D1 remains intact. We don't require a
    # perfect EMA alignment, but an explicitly opposite 4H bias needs a strong
    # 1H retest/bounce to compensate.
    four_wanted = wanted
    four_bias_name = str(four.get("bias", "UNKNOWN"))
    four_alignment = 1.0 if four_bias_name == four_wanted else (0.5 if four_bias_name == "SIDEWAYS" else 0.0)

    d1_strength = min(100.0, abs(float(d1_bias.get("score", 0.0) or 0.0)))
    # Matching D1 gets full directional credit; weak opposite/sideways gets less.
    if d1_name == wanted:
        d1_direction_factor = 1.0
    elif d1_name == "SIDEWAYS":
        d1_direction_factor = 0.55
    else:
        d1_direction_factor = 0.30

    event_score = float(event.get("score", 0.0) or 0.0)
    mtf_score = (
        0.20 * d1_strength * d1_direction_factor
        + 0.20 * (100.0 * four_alignment)
        + 0.60 * event_score
    )

    event_state = str(event.get("state", "WAIT"))
    valid = bool(event.get("valid")) and mtf_score >= MTF_MIN_SCORE

    # Opposite 4H is no longer an automatic block if the 1H level event itself
    # is strong. This is useful when D1 trend is intact but 4H is correcting.
    if four_alignment == 0.0 and event_state not in {"RETEST_CONFIRMED", "EMA_RETEST", "BREAKOUT_HOLD"}:
        valid = False

    if valid and mtf_score >= MTF_A_TIER_SCORE:
        tier = "A"
    elif valid and mtf_score >= MTF_B_TIER_SCORE:
        tier = "B"
    else:
        tier = "WAIT"
        valid = False
    ...
    return {
        "status": f"OK_{tier}" if valid else event_state,
        "valid": valid,
        "reason": (
            f"D1={d1_bias.get('bias')}({float(d1_bias.get('score',0.0)):+.0f}) · "
            f"4H={four_bias_name} · 1H={event.get('state')} · score={mtf_score:.0f}"
        ),
        "score": mtf_score,
        "tier": tier,
        "d1_bias": d1_bias,
        "four_h_bias": four,
        "event": event,
        "level": float(event.get("level", 0.0) or 0.0),
        "zone_low": float(event.get("zone_low", 0.0) or 0.0),
        "zone_high": float(event.get("zone_high", 0.0) or 0.0),
    }
...
def calculate_active_mover_trade(
    ...
    target_moves = _reference_target_moves(
        setup, (ACTIVE_TP1_MOVE, ACTIVE_TP2_MOVE, ACTIVE_TP3_MOVE, ACTIVE_TP4_MOVE, ACTIVE_TP5_MOVE), 0.45
    )
    if side == "LONG":
        sl = entry * (1 - sl_move)
        tp1, tp2, tp3, tp4, tp5 = [entry * (1 + m) for m in target_moves]
    else:
        sl = entry * (1 + sl_move)
        tp1, tp2, tp3, tp4, tp5 = [entry * (1 - m) for m in target_moves]
    ...
    trade.update({
        ...
        "roi_tp1": target_moves[0] * LEVERAGE * 100,
        "target_moves": list(target_moves),
        "target_profile": "V21_REFERENCE_BLEND" if bool(setup.get("reference_strong")) else "V21_NATIVE",
...
def add_active_mover_watch(setup: Dict[str, Any]) -> bool:
    ...
    V20.3.6.1 FAST START/FAST SCAN: the OI baseline is captured lazily only
    ...
    with STATE_IO_LOCK:
        ...
        item["watch_open_interest"] = watch_oi
        watches.append(item)
        save_state()
    # V18.1.1 QUIET WATCH:
...
def process_active_mover_watches() -> Dict[str, int]:
    ...
    """
    stats = {"checked": 0, "triggered": 0, "short_shadow": 0, "expired": 0, "rejected": 0, "research_blocked": 0, "mtf_blocked": 0, "mtf_valid": 0, "followthrough_blocked": 0, "confirmation_wait": 0}
    if not ACTIVE_MOVER_ENABLED:
        return stats
    ...
    for item in snapshot:
        ...
        # V20.3.6.1 mandatory top-down hierarchy:
        # D1 direction -> 4H level/context -> 1H breakout/retest/bounce ->
        # only then lower-TF scalp confirmation and derivatives context.
        mtf = top_down_level_context(symbol, side, price)
        if str(mtf.get("status")) == "DATA_WAIT":
            # Infrastructure/data delay is not a trading rejection. Keep the watch.
            remaining.append(item)
            continue

        setup["mtf_status"] = str(mtf.get("status", "UNKNOWN"))
        setup["mtf_tier"] = str(mtf.get("tier", "WAIT"))
        setup["mtf_score"] = float(mtf.get("score", 0.0) or 0.0)
        setup["mtf_reason"] = str(mtf.get("reason", ""))
        setup["mtf_level"] = float(mtf.get("level", 0.0) or 0.0)
        setup["mtf_zone_low"] = float(mtf.get("zone_low", 0.0) or 0.0)
        setup["mtf_zone_high"] = float(mtf.get("zone_high", 0.0) or 0.0)
        setup["mtf_d1_trend"] = str((mtf.get("d1_bias") or {}).get("bias", "UNKNOWN"))
        setup["mtf_d1_score"] = float((mtf.get("d1_bias") or {}).get("score", 0.0) or 0.0)
        setup["mtf_4h_trend"] = str((mtf.get("four_h_bias") or {}).get("bias", "UNKNOWN"))
        setup["mtf_1h_state"] = str((mtf.get("event") or {}).get("state", "UNKNOWN"))
        setup["mtf_level_strength"] = float((mtf.get("event") or {}).get("level_strength", 0.0) or 0.0)
        setup["mtf_breakout_volume"] = float((mtf.get("event") or {}).get("breakout_volume", 0.0) or 0.0)
        setup["mtf_retest_quality"] = float((mtf.get("event") or {}).get("retest_quality", 0.0) or 0.0)
        setup["mtf_role_reversal"] = bool((mtf.get("event") or {}).get("role_reversal", False))

        # Make the actual higher-timeframe level the technical reference for SL geometry.
        if setup["mtf_level"] > 0:
            setup["level"] = setup["mtf_level"]

        if not bool(mtf.get("valid")):
            blocked_trade = calculate_active_mover_trade(setup, c1, c5)
            if blocked_trade:
                blocked_trade["paper_validation_origin"] += (
                    f" · HIDDEN MTF BLOCK: {setup['mtf_reason']}"
                )
                add_shadow_signal(blocked_trade, ACTIVE_MTF_FILTER_REASON)
            stats["mtf_blocked"] += 1
            stats["rejected"] += 1
            continue
        ...
        research = scalp_research_score(
            ...
            funding=funding,
            taker_imbalance=taker_flow,
        )
        # Strong top-down confluence may upgrade an otherwise borderline scalp,
        # but cannot rescue an invalid D1/4H/1H setup because that was gated above.
        mtf_bonus = max(0.0, min(8.0, (setup["mtf_score"] - MTF_MIN_SCORE) * 0.22))
        research["score"] = min(100.0, float(research.get("score", 0.0) or 0.0) + mtf_bonus)
        setup["scalp_cmf"] = float(research.get("cmf",0.0) or 0.0)
        setup["scalp_obv_pressure"] = float(research.get("obv_pressure",0.0) or 0.0)
        setup["scalp_taker_imbalance"] = float(research.get("taker_imbalance",0.0) or 0.0) if research.get("taker_imbalance") is not None else 0.0
        setup["scalp_oi_change"] = float(research.get("oi_change",0.0) or 0.0) if research.get("oi_change") is not None else 0.0
        setup["scalp_funding_rate"] = float(research.get("funding_rate",0.0) or 0.0) if research.get("funding_rate") is not None else 0.0
        setup["scalp_research_score"] = float(research.get("score",0.0) or 0.0)
        setup["scalp_breakout_distance"] = float(research.get("breakout_distance",0.0) or 0.0)
        setup["scalp_pause_volume_ratio"] = float(research.get("pause_volume_ratio",1.0) or 1.0)
        setup["entry_vol1_raw"] = vol1
        setup["entry_mom1"] = raw_one_min
        setup["entry_mom3"] = raw_three_min
        setup["entry_mom5"] = spike_bar_return(c1, 5)
        setup["entry_mom15"] = spike_bar_return(c5, 3)

        reference_ctx = reference_scalp_context(setup, c1, c5)
        _attach_reference_context(setup, reference_ctx)
        ref_bonus = max(0.0, (float(reference_ctx.get("score", 0.0) or 0.0) - 50.0) / 50.0 * REFERENCE_SCALP_SCORE_BONUS_MAX)
        setup["score"] = min(100, int(float(setup.get("score", 0) or 0) + ref_bonus))
        gate_reason += f" · {reference_ctx.get('reason','')}"

        flow_ok, flow_reason = v21_flow_gate(setup)
        if not flow_ok:
            blocked_trade = calculate_active_mover_trade(setup, c1, c5)
            if blocked_trade:
                blocked_trade["paper_validation_origin"] += f" · HIDDEN V21 FLOW BLOCK: {flow_reason}"
                add_shadow_signal(blocked_trade, ACTIVE_RESEARCH_FILTER_REASON)
            stats["research_blocked"] += 1
            stats["rejected"] += 1
            continue

        follow_ok, follow_reason, follow_score = v21_followthrough_gate(setup)
        setup["v21_followthrough_score"] = follow_score
        if not follow_ok:
            blocked_trade = calculate_active_mover_trade(setup, c1, c5)
            if blocked_trade:
                blocked_trade["paper_validation_origin"] += f" · HIDDEN FOLLOW-THROUGH BLOCK: {follow_reason}"
                add_shadow_signal(blocked_trade, V21_FOLLOWTHROUGH_FILTER_REASON)
            stats["followthrough_blocked"] += 1
            stats["rejected"] += 1
            continue

        gate_reason += f" · {follow_reason}"
        gate_reason += (
            f" · MTF-{setup['mtf_tier']} {setup['mtf_d1_trend']}→{setup['mtf_4h_trend']}→{setup['mtf_1h_state']} "
            f"score={setup['mtf_score']:.0f} level={setup['mtf_level']:.8g} · "
            f"RESEARCH SCALP score={setup['scalp_research_score']:.0f} "
            f"CMF={setup['scalp_cmf']:+.2f} OBV={setup['scalp_obv_pressure']:+.2f} "
            f"flow={setup['scalp_taker_imbalance']:+.2f} OIΔ={setup['scalp_oi_change']*100:+.2f}% "
            f"funding={setup['scalp_funding_rate']*100:+.3f}%"
        )
        ...
        mtf_tier = str(trade.get("mtf_tier", "WAIT"))
        if mtf_tier == "A":
            required_research = min(ACTIVE_RESEARCH_SCORE_MIN, MTF_A_RESEARCH_MIN)
        else:
            required_research = max(52.0, MTF_B_RESEARCH_MIN)

        if float(trade.get("scalp_research_score",0.0) or 0.0) < required_research:
            trade["paper_validation_origin"] += (
                f" · HIDDEN RESEARCH FILTER: score {float(trade.get('scalp_research_score',0.0)):.0f} "
                f"< {required_research:.0f} for MTF tier {mtf_tier}; "
                + ", ".join(str(x) for x in research.get("notes", [])[:6])
            )
            if add_shadow_signal(trade, ACTIVE_RESEARCH_FILTER_REASON):
                stats["research_blocked"] += 1
            stats["rejected"] += 1
            continue
        ...
        if side == "SHORT":
            trade["paper_validation_origin"] += " · V21.2 SHORT SHADOW ONLY"
            if add_shadow_signal(trade, V21_LEVEL_SHORT_SHADOW_REASON):
                stats["short_shadow"] += 1
            continue
        confirm_ok, confirm_reason = v21_temporal_entry_confirmation(trade, "LEVEL_RETEST")
        trade["paper_validation_origin"] += f" · {confirm_reason}"
        if not confirm_ok:
            remaining.append(item)
            stats["confirmation_wait"] += 1
            continue

        try:
            adaptive_ok, adaptive_reason, adaptive_probability = adaptive_gate(trade)
            trade["adaptive_reason"] = adaptive_reason
            if adaptive_probability is not None: trade["adaptive_probability"] = adaptive_probability
        except Exception as exc:
            adaptive_ok = True; adaptive_reason = f"adaptive fail-open PAPER: {repr(exc)}"; trade["adaptive_reason"] = adaptive_reason
        if not adaptive_ok:
            trade["paper_validation_origin"] += f" · HIDDEN ADAPTIVE BLOCK: {adaptive_reason}"
            add_shadow_signal(trade, SPIKE_ADAPTIVE_BLOCK_REASON)
            stats["rejected"] += 1
            continue
        portfolio_ok, portfolio_reason = v21_portfolio_circuit_breaker()
        trade["paper_validation_origin"] += f" · {adaptive_reason} · {portfolio_reason}"
        if not portfolio_ok:
            add_shadow_signal(trade, V21_PORTFOLIO_GUARD_REASON)
            stats["rejected"] += 1
            continue
...
def v21_short_shadow_metrics() -> Dict[str, Any]:
    init_adaptive_db()
    with _LOCK, _connect() as conn:
        rows = conn.execute(
            "SELECT result,pnl_r,symbol FROM adaptive_trades WHERE COALESCE(decision_reason,'') IN (?,?) ORDER BY closed_at ASC,id ASC",
            (V21_LEVEL_SHORT_SHADOW_REASON, V21_IMPULSE_SHORT_SHADOW_REASON),
        ).fetchall()
    metrics = _outcome_metrics(rows)
    metrics["unique_symbols"] = len({normalize_symbol(str(r["symbol"] or "?")) for r in rows})
    return metrics


def v21_expired_breakdown() -> Dict[str, int]:
    init_adaptive_db()
    with _LOCK, _connect() as conn:
        rows = conn.execute(
            "SELECT COALESCE(outcome_detail,'UNKNOWN') AS detail, COUNT(*) AS n "
            "FROM adaptive_trades WHERE result='expired' AND COALESCE(decision_reason,'') IN (?,?) "
            "GROUP BY COALESCE(outcome_detail,'UNKNOWN')",
            (TRADER_STYLE_PAPER_REASON, EXHAUSTION_PAPER_REASON),
        ).fetchall()
    return {str(r["detail"]): int(r["n"] or 0) for r in rows}


def v21_expired_breakdown_text() -> str:
    d = v21_expired_breakdown()
    if not d:
        return "expired detail: none"
    order = ["DEAD", "WEAK", "PARTIAL_MFE", "TP1_STALL", "TP1_REVERSED", "TP2_STALL", "TP2_REVERSED", "UNKNOWN"]
    parts = [f"{k}={d[k]}" for k in order if d.get(k)]
    parts.extend(f"{k}={v}" for k,v in sorted(d.items()) if k not in order and v)
    return "expired detail: " + " · ".join(parts)
...
def professional_candle_intelligence(
    ...
    """Price-action classifier for squeeze, overheat and exhaustion.

    It uses only observable price/volume data. Without OI/liquidation feeds it
    cannot prove forced liquidations, so `squeeze` is a price/volume proxy.
    """
    if direction not in {-1,1} or not c1 or not c5:
        return {"state":"UNKNOWN","overheat":0.0,"climax":0.0,"squeeze":0.0,
    ...
    e9=ema(closes(c5[-60:]),9); e21=ema(closes(c5[-80:]),21); vw=vwap(c5,36)
    ...
    rsi1=rsi_context(c1,14); rsi5=rsi_context(c5,14)

    streak=0
    for candle in reversed(c5[-8:]):
        with_dir=(float(candle["close"])>float(candle["open"])) if direction>0 else (float(candle["close"])<float(candle["open"]))
        if with_dir: streak+=1
        else: break

    hist=c5[-26:-1] if len(c5)>=26 else c5[:-1]
    typical_range=_median([candle_range(x)/max(float(x["open"]),1e-12) for x in hist]) if hist else 0.0
    ...
    prior=c5[-8:-2]
    prior_vol=(sum(float(x["volume"]) for x in prior)/len(prior)) if prior else med_vol
    push_volume_ratio=recent_vol/max(prior_vol,1e-12)

    trend_alignment=(price>e9>e21 and price>vw) if direction>0 else (price<e9<e21 and price<vw)
    mom3=direction*spike_bar_return(c1,3)
    mom15=direction*spike_bar_return(c5,3)
    mom30=direction*spike_bar_return(c5,6)

    squeeze=0.0
    squeeze+=18 if trend_alignment else 0
    squeeze+=min(18,max(0,mom3)/0.006*9)
    squeeze+=min(16,max(0,mom15)/0.020*10)
    squeeze+=12 if shape5["body"]>=0.55 and shape5["close_extreme"]>=0.65 else 0
    squeeze+=min(12,max(0,volume_health-0.8)*8)
    squeeze+=min(10,max(0,range_expansion-1.0)*5)
    squeeze+=min(8,streak*2)
    squeeze=min(100.0,squeeze)

    rsi_extreme=max(0,(rsi5-50)/22) if direction>0 else max(0,(50-rsi5)/22)
    overheat=0.0
    overheat+=min(24,distance_atr/4*24)
    overheat+=min(16,vwap_distance_atr/4.5*16)
    overheat+=min(18,min(1.5,rsi_extreme)*12)
    overheat+=min(16,streak*3)
    overheat+=min(14,max(0,range_expansion-1)*7)
    overheat+=min(12,max(0,mom30)/0.05*12)
    ...
    climax=0.0
    climax+=min(28,overheat*0.28)
    climax+=min(24,rejection*55)
    climax+=18*volume_weakening
    climax+=12 if shape5["body"]<0.45 else 0
    climax+=10 if signed_distance>2.5 and shape5["close_extreme"]<0.45 else 0
    climax+=8 if mom3<0 else 0
    climax=min(100.0,climax)

    if squeeze>=68 and climax<58: state="SQUEEZE_CONTINUATION"
    elif overheat>=72 and climax<60: state="OVERHEATED_TRENDING"
    elif overheat>=58 and climax>=58: state="EXHAUSTION_PRESSURE"
    elif squeeze>=50: state="MOMENTUM"
    else: state="NORMAL"

    setup_quality=0.44*squeeze+0.20*min(100,max(0,volume_health)*50)+0.18*min(100,max(0,range_expansion)*35)+0.18*(100 if trend_alignment else 35)
    setup_quality=max(0.0,min(100.0,setup_quality))
    return {"state":state,"overheat":overheat,"climax":climax,"squeeze":squeeze,
            "rejection_wick":rejection,"distance_atr":distance_atr,
            "volume_health":volume_health,"push_volume_ratio":push_volume_ratio,
            "range_expansion":range_expansion,"rsi_fast":rsi1,"rsi_slow":rsi5,
            "setup_quality":setup_quality,"trend_alignment":bool(trend_alignment),
            "streak":streak,"mom3_with_direction":mom3,"mom15_with_direction":mom15}
...
def detect_mtf_spike_fade(
    ...
    return {
        ...
        "trade_type": "🚀 V21 IMPULSE CONTINUATION WATCH",
        "paper_style": "IMPULSE_CONTINUATION",
        "paper_setup_lane": "CONTINUATION",
...
def calculate_exhaustion_trade(setup: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Build a V21 spike-continuation trade. Exhaustion path is retained only for legacy compatibility and is hard-disabled upstream."""
    side = str(setup.get("side", "")).upper()
    ...
    target_moves = _reference_target_moves(
        setup, (EXHAUST_TP1_MOVE, EXHAUST_TP2_MOVE, EXHAUST_TP3_MOVE, EXHAUST_TP4_MOVE, EXHAUST_TP5_MOVE), 0.50
    )
    if side == "SHORT":
        sl = entry * (1 + sl_move)
        tps = [entry * (1 - m) for m in target_moves]
    else:
        sl = entry * (1 - sl_move)
        tps = [entry * (1 + m) for m in target_moves]
...
def process_squeeze_exhaustion_watches() -> Dict[str, int]:
    """V21 continuation-only regime classifier.

    Real spike is detected first. V21 accepts only healthy continuation:
      UP spike + controlled pullback/recovery -> LONG
      DOWN spike + controlled pullback/recovery -> SHORT

    Exhaustion/reversal is hard-disabled. The watch is quiet; Telegram receives
    only confirmed continuation PAPER entries.
    """
    stats = {
        "checked": 0, "continuation": 0, "exhaustion": 0,
        "runaway_watch": 0, "triggered": 0, "short_shadow": 0, "followthrough_blocked": 0,
        "confirmation_wait": 0, "expired": 0,
    ...
    for item in snapshot:
        ...
        if SPIKE_EXHAUSTION_ENABLED and (
            failed_reclaim
        ...
        setup["trade_type"] = f"🚀 V21 IMPULSE CONTINUATION {trade_side}"
        setup["paper_setup_lane"] = "CONTINUATION"
        setup["v21_lane"] = "IMPULSE_CONTINUATION"
        ...
        # V21: optional order-flow/derivatives context is soft; only a three-factor
        # opposing flow conflict is a hard block. Missing endpoints are neutral.
        try:
            with ThreadPoolExecutor(max_workers=3, thread_name_prefix="v21-spike-flow") as pool:
                ff = pool.submit(recent_taker_imbalance, symbol, 120)
                fo = pool.submit(get_open_interest_value, symbol)
                fu = pool.submit(get_funding_rate_value, symbol)
                taker_flow = ff.result(); current_oi = fo.result(); funding = fu.result()
        except Exception:
            taker_flow = None; current_oi = None; funding = None
        setup["scalp_cmf"] = chaikin_money_flow(c1, 20)
        setup["scalp_obv_pressure"] = obv_pressure(c1, 24)
        setup["scalp_taker_imbalance"] = float(taker_flow or 0.0)
        setup["scalp_funding_rate"] = float(funding or 0.0)
        setup["scalp_oi_change"] = 0.0
        setup["scalp_research_score"] = 50.0 + 20.0 * (1 if trade_side == "LONG" else -1) * float(taker_flow or 0.0)
        reference_ctx = reference_scalp_context(setup, c1, c5)
        _attach_reference_context(setup, reference_ctx)
        ref_bonus = max(0.0, (float(reference_ctx.get("score", 0.0) or 0.0) - 50.0) / 50.0 * REFERENCE_SCALP_SCORE_BONUS_MAX)
        setup["score"] = min(100, int(float(setup.get("score", 0) or 0) + ref_bonus))
        setup["paper_validation_origin"] += f" REFERENCE: {reference_ctx.get('reason','')}."
        flow_ok, flow_reason = v21_flow_gate(setup)
        if not flow_ok:
            stats["execution_rejected"] += 1
            remaining.append(item)
        ...
        setup["v21_followthrough_score"] = follow_score
        if not follow_ok:
            blocked_trade = calculate_exhaustion_trade(setup)
            if blocked_trade:
                blocked_trade["paper_validation_origin"] += f" · HIDDEN FOLLOW-THROUGH BLOCK: {follow_reason}"
                add_shadow_signal(blocked_trade, V21_FOLLOWTHROUGH_FILTER_REASON)
            stats["followthrough_blocked"] += 1
            continue
        setup["paper_validation_origin"] += f" · {follow_reason}"

        empirical_ok, empirical_reason = v21_empirical_impulse_guard(setup)
        setup["paper_validation_origin"] += f" · {empirical_reason}"
        if not empirical_ok:
            blocked_trade = calculate_exhaustion_trade(setup)
            if blocked_trade:
                blocked_trade["paper_validation_origin"] += f" · HIDDEN EMPIRICAL BLOCK: {empirical_reason}"
                add_shadow_signal(blocked_trade, V21_FOLLOWTHROUGH_FILTER_REASON)
            stats["followthrough_blocked"] += 1
            continue
        ...
        if trade_side == "SHORT":
            trade["paper_validation_origin"] += " · V21.2 SHORT SHADOW ONLY"
            if add_shadow_signal(trade, V21_IMPULSE_SHORT_SHADOW_REASON):
                stats["short_shadow"] += 1
            continue

        confirm_ok, confirm_reason = v21_temporal_entry_confirmation(trade, "IMPULSE_CONTINUATION")
        trade["paper_validation_origin"] += f" · {confirm_reason}"
        if not confirm_ok:
            remaining.append(item)
            stats["confirmation_wait"] += 1
        ...
        portfolio_ok, portfolio_reason = v21_portfolio_circuit_breaker()
        trade["paper_validation_origin"] += f" · {strategy_reason} · {adaptive_reason} · {portfolio_reason}"
        if not portfolio_ok:
            add_shadow_signal(trade, V21_PORTFOLIO_GUARD_REASON)
            continue
...
def analyze_symbol(
    ...
    # Only confirmed continuation after a controlled pullback becomes a V21 PAPER trade.
...
def build_paper_signal_message(s: Dict[str, Any]) -> str:
    ...
    is_exhaust = str(s.get("paper_style", "")) == "IMPULSE_CONTINUATION" or str(s.get("paper_validation_lane", "")) == EXHAUSTION_PAPER_REASON
    if is_exhaust:
        badge = "🚀 V21 IMPULSE CONTINUATION"
        horizon = f"до {EXHAUST_HARD_EXPIRE_MINUTES} мин; мягкая проверка после {EXHAUST_SOFT_EXPIRE_MINUTES} мин"
    elif is_active:
        badge = "🎯 V21 LEVEL RETEST"
...
def build_paper_result_message(
    ...
    is_exhaust = str(signal.get("paper_style", "")) == "IMPULSE_CONTINUATION" or str(signal.get("shadow_reason", "")) == EXHAUSTION_PAPER_REASON
    badge = "🚀 V21 IMPULSE CONTINUATION" if is_exhaust else ("🎯 V21 LEVEL RETEST" if is_active else "⚡🟡 LEGACY FOLLOW-THROUGH")
    labels = {
        "profit": "✅ TP3+",
        "sl": "❌ STOP LOSS",
        "expired": "⏱ EXPIRED",
    ...
    stats_name = "V21 IMPULSE CONTINUATION" if is_exhaust else ("V21 LEVEL RETEST" if is_active else "LEGACY FOLLOW-THROUGH")
    return (
...
def _diagnostic_block_label(key: str) -> str:
    """Map legacy internal counter keys to current V21 user-facing names."""
    mapping = {
...
def build_diagnostic(scan: Dict[str, Any]) -> str:
    ...
    return (
        f"🧪 Диагностика V21.1 · LONG-FIRST + FOLLOW-THROUGH\n"
        ...
        f"🎯 LEVEL RETEST LONG · V21.2: {_metrics_line(active_metrics)} · "
        f"n={int(active_metrics.get('n', 0) or 0)}\n"
        f"🚀 V21.1 IMPULSE LONG cohort: {_metrics_line(spike_metrics)} · "
        ...
        f"MTF-valid={int(active_watch.get('mtf_valid',0) or 0)} "
        f"MTF-blocked={int(active_watch.get('mtf_blocked',0) or 0)} "
        f"research-blocked={int(active_watch.get('research_blocked',0) or 0)} "
        f"followthrough-blocked={int(active_watch.get('followthrough_blocked',0) or 0)} "
        f"short-shadow={int(active_watch.get('short_shadow',0) or 0)} · "
        ...
        f"followthrough-blocked={int(spike_watch.get('followthrough_blocked',0) or 0)} "
        f"short-shadow={int(spike_watch.get('short_shadow',0) or 0)} "
        f"triggered={int(spike_watch.get('triggered',0) or 0)}\n"
        f"{learning_line}\n"
        f"Runtime: bootstrap_ready={RUNTIME_BOOTSTRAP_READY} · "
        f"bootstrap_error={RUNTIME_BOOTSTRAP_ERROR or 'none'}\n"
...
def track_shadow_signals() -> bool:
    ...
    for signal in shadows:
        ...
        is_squeeze_exhaustion = (
            str(signal.get("paper_style", "")) == "IMPULSE_CONTINUATION"
...
async def scan_loop():
    ...
    send_telegram(
        ...
        f"🎯 LEVEL RETEST: D1 context → 4H/1H level → breakout/role reversal → pullback/retest → 1m/5m re-acceleration.\n"
        f"🚀 IMPULSE CONTINUATION: real displacement → controlled pullback → recovery; EXHAUSTION is hard-OFF.\n"
        f"📊 ORDER FLOW: CMF/OBV + optional taker flow/OI/funding are confirmations; only multi-factor opposite flow hard-blocks.\n"
        f"🧠 SELF-LEARNING: fresh V21.2-only cohort · warm-up {MIN_TRAIN_TRADES} · retrain every {RETRAIN_EVERY} · "
        f"chronological 2-pass validation · canary {ADAPTIVE_INITIAL_LIVE_FRACTION*100:.0f}%.\n"
        f"🎯 V21 TP: {ACTIVE_TP1_MOVE*100:.1f}/{ACTIVE_TP2_MOVE*100:.1f}/"
...
async def bootstrap_runtime_loop() -> None:
    ...
    while not RUNTIME_BOOTSTRAP_READY:
        ...
        try:
            ...
            RUNTIME_BOOTSTRAP_ERROR = ""
            if str(STATE.get("last_error", "")).startswith("bootstrap_runtime_loop:"):
                STATE["last_error"] = ""
                save_state()
            return
...
async def _supervise_background_task(name: str, worker_factory) -> None:
    ...
    while True:
        try:
            ...
            # bootstrap_runtime_loop is intentionally one-shot: after storage
            # has been restored successfully it MUST return and must not be
            # restarted every 5 seconds by the generic supervisor.
            if name == "bootstrap_runtime_loop" and RUNTIME_BOOTSTRAP_READY:
                return
...
async def startup_event():
    """V20.3.6.1 instant Render startup.
