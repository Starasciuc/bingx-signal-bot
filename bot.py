# VERIFIED GITHUB DEPLOY FILE — V21.0 PRO CONTINUATION · LEVEL RETEST + ORDER FLOW + SAFE LEARNING
...
MODEL_DATA_POLICY = "v21_0_continuation_forward_only_v1"
...
def adaptive_gate(trade: Dict[str, Any]) -> Tuple[bool, str, Optional[float]]:
    ...
    if not (strategy.startswith("PRO_SPIKE_REGIME_") or strategy.startswith("PRO_ACTIVE_MOVER_")):
        return True, "adaptive model dedicated to V21 continuation lanes; bypass", None
...
def format_training_attempt_message(report: Dict[str, Any]) -> str:
    ...
    lines = [
        title,
        f"Данных модели (V21 continuation candidates): {closed_count}",
...
APP_NAME = "Professional Adaptive Futures Bot AUTO V21.0 PRO CONTINUATION · LEVEL RETEST + ORDER FLOW + SAFE LEARNING"
DEPLOY_MARKER = "V21_0_PRO_CONTINUATION_LEVEL_RETEST_ORDER_FLOW_SAFE_LEARNING_2026_09_10"
...
STATE_FILE = os.getenv("STATE_FILE", "bot_state_v21_0_pro_continuation.json")
...
TRADER_STYLE_PAPER_REASON = "v21_level_retest_continuation_paper"
EXHAUSTION_PAPER_REASON = "v21_spike_continuation_paper"  # compatibility name; exhaustion is disabled
...
def closed_outcome_progress_message(
    ...
    return (
        "📊 V21 CONTINUATION · ЧИСТАЯ СТАТИСТИКА\n"
        ...
        f"🎯 ОБЕ ЛИНИИ CONTINUATION: {_metrics_line(total)}\n"
...
def maybe_send_auto_backup() -> Dict[str, Any]:
    ...
    V21 milestones are based only on the two clean continuation lanes,
...
def process_active_mover_watches() -> Dict[str, int]:
    ...
    """
    stats = {"checked": 0, "triggered": 0, "expired": 0, "rejected": 0, "research_blocked": 0, "mtf_blocked": 0, "mtf_valid": 0}
    if not ACTIVE_MOVER_ENABLED:
        return stats
...
def build_diagnostic(scan: Dict[str, Any]) -> str:
    ...
    return (
        f"🧪 Диагностика V21.0 · LEVEL RETEST + IMPULSE CONTINUATION\n"
        ...
        f"🚀 V21 IMPULSE CONTINUATION cohort: {_metrics_line(spike_metrics)} · "
