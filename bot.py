# VERIFIED GITHUB DEPLOY FILE — V20.3.3.1 LEVEL RECLAIM · UB-STYLE CONTINUATION
...
MODEL_DATA_POLICY = "v20_3_3_trading_intelligence_forward_only_v1"
...
APP_NAME = "Professional Adaptive Futures Bot AUTO V20.3.3.1 LEVEL RECLAIM · UB-STYLE CONTINUATION"
DEPLOY_MARKER = "V20_3_3_1_LEVEL_RECLAIM_UB_STYLE_CONTINUATION_2026_09_04"
...
TRADER_STYLE_PAPER_REASON = "active_mover_paper_v20_3_3"
...
ACTIVE_MOVER_GUARD_REASON = "active_mover_strategy_guard_v20_3_3"
...
# V20.3.3.1 — UB-style resistance/support flip continuation.
# Core sequence:
# level -> true close through level -> acceptance -> retest/hold -> lower-TF reacceleration.
LEVEL_RECLAIM_ENABLED = os.getenv("LEVEL_RECLAIM_ENABLED", "true").lower() == "true"
LEVEL_RECLAIM_ZONE_PCT = float(os.getenv("LEVEL_RECLAIM_ZONE_PCT", "0.0028"))
LEVEL_RECLAIM_ZONE_ATR15 = float(os.getenv("LEVEL_RECLAIM_ZONE_ATR15", "0.22"))
LEVEL_RECLAIM_MAX_DISTANCE_PCT = float(os.getenv("LEVEL_RECLAIM_MAX_DISTANCE_PCT", "0.018"))
LEVEL_RECLAIM_BREAKOUT_BUFFER_PCT = float(os.getenv("LEVEL_RECLAIM_BREAKOUT_BUFFER_PCT", "0.0005"))
LEVEL_RECLAIM_MIN_BREAKOUT_VOL = float(os.getenv("LEVEL_RECLAIM_MIN_BREAKOUT_VOL", "0.90"))
LEVEL_RECLAIM_MIN_BREAKOUT_BODY = float(os.getenv("LEVEL_RECLAIM_MIN_BREAKOUT_BODY", "0.34"))
LEVEL_RECLAIM_MIN_ACCEPT_CLOSES = int(os.getenv("LEVEL_RECLAIM_MIN_ACCEPT_CLOSES", "2"))
LEVEL_RECLAIM_MIN_SCORE = float(os.getenv("LEVEL_RECLAIM_MIN_SCORE", "62"))
LEVEL_RECLAIM_MIN_RECOVERY = float(os.getenv("LEVEL_RECLAIM_MIN_RECOVERY", "0.30"))
LEVEL_RECLAIM_MIN_REACCEL_1M = float(os.getenv("LEVEL_RECLAIM_MIN_REACCEL_1M", "0.0004"))
LEVEL_RECLAIM_MIN_VOL1 = float(os.getenv("LEVEL_RECLAIM_MIN_VOL1", "0.45"))
LEVEL_RECLAIM_MIN_RANGE1 = float(os.getenv("LEVEL_RECLAIM_MIN_RANGE1", "0.60"))
LEVEL_RECLAIM_MAX_CHASE = float(os.getenv("LEVEL_RECLAIM_MAX_CHASE", "0.0070"))
LEVEL_RECLAIM_ALLOW_BREAKOUT_HOLD = os.getenv("LEVEL_RECLAIM_ALLOW_BREAKOUT_HOLD", "true").lower() == "true"
...
def _lr_median(values: List[float]) -> float:
...
def _lr_pivots(candles: List[Dict[str, float]], side: str) -> List[float]:
    """Extract local resistance pivots for LONG and support pivots for SHORT."""
    rows = candles[-120:]
    out: List[float] = []
    if len(rows) < 8:
        return out
    for i in range(2, len(rows) - 2):
        if side == "LONG":
            value = float(rows[i]["high"])
            if value >= max(float(x["high"]) for x in rows[i-2:i] + rows[i+1:i+3]):
                out.append(value)
        else:
            value = float(rows[i]["low"])
            if value <= min(float(x["low"]) for x in rows[i-2:i] + rows[i+1:i+3]):
                out.append(value)
    return out


def find_level_reclaim_zone(
    c15: List[Dict[str, float]],
    c1h: List[Dict[str, float]],
    side: str,
    price: float,
) -> Optional[Dict[str, Any]]:
    """Find the nearest repeated 15m/1h level that can flip role after a break."""
    if price <= 0 or len(c15) < 24 or len(c1h) < 20:
        return None
    side = side.upper()
    a15 = max(atr(c15, 14), price * 1e-6)
    half = max(price * LEVEL_RECLAIM_ZONE_PCT, a15 * LEVEL_RECLAIM_ZONE_ATR15)

    raw: List[Tuple[float, float, str]] = []
    for value in _lr_pivots(c1h, side):
        raw.append((value, 2.5, "1H"))
    for value in _lr_pivots(c15, side):
        raw.append((value, 1.0, "15M"))
    if not raw:
        return None

    clusters: List[Dict[str, Any]] = []
    for value, weight, source in sorted(raw, key=lambda x: x[0]):
        match = None
        for z in clusters:
            if abs(value - float(z["center"])) <= half:
                match = z
                break
        if match is None:
            match = {
                ...
                "weight": weight,
                "touches": 1,
                "touches_1h": 1 if source == "1H" else 0,
                "touches_15m": 1 if source == "15M" else 0,
            }
            clusters.append(match)
        else:
            match["weighted"] += value * weight
            match["weight"] += weight
            match["center"] = match["weighted"] / max(match["weight"], 1e-12)
            match["touches"] += 1
            match["touches_1h"] += 1 if source == "1H" else 0
            match["touches_15m"] += 1 if source == "15M" else 0

    candidates: List[Dict[str, Any]] = []
    for z in clusters:
        center = float(z["center"])
        dist = abs(price - center) / price
        if dist > LEVEL_RECLAIM_MAX_DISTANCE_PCT:
            continue
        # LONG wants old resistance at/just below current price after a break.
        # SHORT wants old support at/just above current price after a break.
        if side == "LONG" and center > price * 1.006:
            continue
        if side == "SHORT" and center < price * 0.994:
            continue
        strength = min(
            100.0,
            20.0 * float(z["touches_1h"])
            + 8.0 * float(z["touches_15m"])
            + 4.0 * min(5.0, float(z["touches"])),
        )
        candidates.append({
            **z,
            "center": center,
    ...
    return max(
        candidates,
        key=lambda z: float(z["strength"]) - 1200.0 * float(z["distance_pct"]),
    )


def _lr_relative_volume_at(candles: List[Dict[str, float]], idx: int, lookback: int = 20) -> float:
    if idx <= 0 or idx >= len(candles):
        return 0.0
    prior = [
        float(x.get("volume", 0.0) or 0.0)
        for x in candles[max(0, idx-lookback):idx]
        if float(x.get("volume", 0.0) or 0.0) > 0
    ]
    if not prior:
        return 0.0
    med = _lr_median(prior)
    return float(candles[idx].get("volume", 0.0) or 0.0) / max(med, 1e-12)


def level_reclaim_context(
    c1: List[Dict[str, float]],
    c5: List[Dict[str, float]],
    c15: List[Dict[str, float]],
    c1h: List[Dict[str, float]],
    ...
    """Classify UB-style level reclaim continuation.

    Valid LONG:
      old resistance -> 15m close above zone -> 5m acceptance/retest ->
      former resistance holds as support -> 1m reacceleration.
    ...
    side = side.upper()
    price = float(c1[-1]["close"])
    zone = find_level_reclaim_zone(c15, c1h, side, price)
    if not zone:
        return neutral
    ...
    breakout_vol = 0.0
    breakout_body = 0.0
    buffer = price * LEVEL_RECLAIM_BREAKOUT_BUFFER_PCT

    rows15 = c15[-12:]
    for i in range(1, len(rows15)):
        prev_close = float(rows15[i-1]["close"])
        row = rows15[i]
        close = float(row["close"])
        rng = max(float(row["high"]) - float(row["low"]), 1e-12)
        body = abs(close - float(row["open"])) / rng
        vol = _lr_relative_volume_at(rows15, i, 8)
        crossed = (
            prev_close <= upper and close > upper + buffer
            if side == "LONG"
            else prev_close >= lower and close < lower - buffer
        )
        if crossed:
            breakout_idx = i
            breakout_vol = vol
    ...
    if breakout_idx is None:
        return {**neutral, "state": "LEVEL_NEAR_NO_BREAK", "level": center,
                "zone_low": lower, "zone_high": upper}

    breakout_time = int(rows15[breakout_idx].get("time", 0) or 0)
    after5 = [x for x in c5[-18:] if int(x.get("time", 0) or 0) >= breakout_time]
    if not after5:
        after5 = c5[-8:]
    ...
    for row in after5:
        ...
        if side == "LONG":
            ...
            if close < lower - buffer:
                false_breakout = True
            if low <= upper + buffer and close >= center:
                retest = True
        else:
            if close <= center:
                accept_closes += 1
            if close > upper + buffer:
                false_breakout = True
            if high >= lower - buffer and close <= center:
                retest = True

    # Compare the latest pullback/pause 5m participation to its own prior baseline.
    recent_pause = c5[-3:]
    prior5 = c5[-15:-3]
    recent_vol = (
        sum(float(x.get("volume", 0.0) or 0.0) for x in recent_pause) / len(recent_pause)
        if recent_pause else 0.0
    )
    prior_vol = (
        sum(float(x.get("volume", 0.0) or 0.0) for x in prior5) / len(prior5)
        if prior5 else max(recent_vol, 1.0)
    )
    pullback_volume_ratio = recent_vol / max(prior_vol, 1e-12)
    pullback_volume_cool = pullback_volume_ratio <= 1.05

    price_hold = price >= center if side == "LONG" else price <= center
    acceptance = accept_closes >= LEVEL_RECLAIM_MIN_ACCEPT_CLOSES
    breakout_quality = (
        breakout_vol >= LEVEL_RECLAIM_MIN_BREAKOUT_VOL
        and breakout_body >= LEVEL_RECLAIM_MIN_BREAKOUT_BODY
    )

    state = "BREAKOUT_WAIT_RETEST"
    if false_breakout:
        state = "FALSE_BREAKOUT"
    elif breakout_quality and retest and acceptance and price_hold:
        state = "RETEST_CONFIRMED"
    elif (
        LEVEL_RECLAIM_ALLOW_BREAKOUT_HOLD
        and breakout_quality
        and acceptance
        and price_hold
    ):
        state = "BREAKOUT_HOLD"
    ...
    score = 28.0
    score += min(18.0, float(zone["strength"]) * 0.18)
    score += min(14.0, max(0.0, breakout_vol - 0.70) * 18.0)
    score += min(12.0, breakout_body * 18.0)
    score += 10.0 if acceptance else 0.0
    score += 12.0 if retest else 0.0
    score += 8.0 if pullback_volume_cool else 0.0
    score -= 35.0 if false_breakout else 0.0
    score = max(0.0, min(100.0, score))

    valid = (
        state in {"RETEST_CONFIRMED", "BREAKOUT_HOLD"}
        and score >= LEVEL_RECLAIM_MIN_SCORE
    )
    return {
        "valid": valid,
        "state": state,
...
def active_mover_setup(
    ...
    nearby_flip = find_level_reclaim_zone(c15, c1h, side, price) if LEVEL_RECLAIM_ENABLED else None
    if side == "LONG":
        native_bias_ok = (d3 >= -0.0015 and (d15 >= 0.0015 or d30 >= 0.0030) and price >= vw * 0.992)
        level_bias_ok = bool(
            nearby_flip
            and d3 >= -0.0030
            and d15 >= -0.0015
            and price >= float(nearby_flip["center"]) * 0.992
        )
    else:
        native_bias_ok = (d3 >= -0.0015 and (d15 >= 0.0015 or d30 >= 0.0030) and price <= vw * 1.008)
        level_bias_ok = bool(
            nearby_flip
            and d3 >= -0.0030
            and d15 >= -0.0015
            and price <= float(nearby_flip["center"]) * 1.008
        )
    bias_ok = native_bias_ok or level_bias_ok
    if not bias_ok:
    ...
    return {
        ...
        "paper_validation_origin": reason,
        "level_reclaim_hint": float(nearby_flip["center"]) if nearby_flip else 0.0,
        "level_reclaim_hint_strength": float(nearby_flip["strength"]) if nearby_flip else 0.0,
        "created_at": now_ts(),
    }, "active_watch_ok"
...
def process_active_mover_watches() -> Dict[str, int]:
    ...
    for item in snapshot:
        ...
        native_trigger_ok = bool(reclaim_ok and reaccel_ok)

        # UB-style alternate trigger. Only now, after a real pullback exists,
        # do we pay for the slower 15m/1h context calls.
        level_ctx: Dict[str, Any] = {"valid": False, "state": "DISABLED", "score": 0.0}
        level_trigger_ok = False
        c15: List[Dict[str, float]] = []
        c1h: List[Dict[str, float]] = []
        if LEVEL_RECLAIM_ENABLED:
            c15 = get_klines(symbol, "15m", 100, cache_seconds=15)
            c1h = get_klines(symbol, "1h", 100, cache_seconds=30)
            if c15 and c1h:
                level_ctx = level_reclaim_context(c1, c5, c15, c1h, side)
                level_reaccel_ok = (
                    one_min >= LEVEL_RECLAIM_MIN_REACCEL_1M
                    and three_min > -0.0002
                    and vol1 >= LEVEL_RECLAIM_MIN_VOL1
                    and range1 >= LEVEL_RECLAIM_MIN_RANGE1
                    and candle_ok
                    and structure_ok
                    and chase <= LEVEL_RECLAIM_MAX_CHASE
                    and recovery >= LEVEL_RECLAIM_MIN_RECOVERY
                )
                level_trigger_ok = bool(level_ctx.get("valid") and level_reaccel_ok)

        if not native_trigger_ok and not level_trigger_ok:
            remaining.append(item)
            continue
        ...
        if level_trigger_ok:
            setup["strategy"] = f"PRO_LEVEL_RECLAIM_{side}"
            setup["trade_type"] = f"🧲 LEVEL RECLAIM {side}"
            setup["paper_setup_lane"] = "🧲 LEVEL RECLAIM"
            setup["setup_mode"] = f"V20_3_3_1_LEVEL_RECLAIM_{side}"
            setup["level"] = float(level_ctx.get("level", price) or price)
            setup["level_reclaim_state"] = str(level_ctx.get("state", "UNKNOWN"))
            setup["level_reclaim_score"] = float(level_ctx.get("score", 0.0) or 0.0)
            setup["level_reclaim_zone_low"] = float(level_ctx.get("zone_low", 0.0) or 0.0)
            setup["level_reclaim_zone_high"] = float(level_ctx.get("zone_high", 0.0) or 0.0)
            setup["level_reclaim_breakout_vol"] = float(level_ctx.get("breakout_vol", 0.0) or 0.0)
            setup["level_reclaim_breakout_body"] = float(level_ctx.get("breakout_body", 0.0) or 0.0)
            setup["level_reclaim_accept_closes"] = int(level_ctx.get("accept_closes", 0) or 0)
            setup["level_reclaim_retest"] = bool(level_ctx.get("retest", False))
            setup["level_reclaim_pullback_volume_cool"] = bool(level_ctx.get("pullback_volume_cool", False))
        ...
        if level_trigger_ok:
            setup["paper_validation_origin"] = (
                f"V20.3.3.1 🧲 LEVEL RECLAIM {side}: "
                f"{level_ctx.get('state')} · level {float(level_ctx.get('level',0.0)):.8g} · "
                f"score {float(level_ctx.get('score',0.0)):.0f} · "
                f"breakout Vol x{float(level_ctx.get('breakout_vol',0.0)):.2f} · "
                f"accept closes {int(level_ctx.get('accept_closes',0) or 0)} · "
                f"retest={bool(level_ctx.get('retest',False))} · "
                f"pullback-volume-cool={bool(level_ctx.get('pullback_volume_cool',False))} · "
                f"watch pullback {pullback*100:.2f}% → recovery {recovery*100:.0f}% → "
        ...
        gate_reason = (
            (
                f"LEVEL_RECLAIM_GATE: {level_ctx.get('state')} score={float(level_ctx.get('score',0.0)):.0f} · "
                f"level={float(level_ctx.get('level',0.0)):.8g} · "
                f"breakoutVol x{float(level_ctx.get('breakout_vol',0.0)):.2f} · "
                f"pullback {pullback*100:.2f}% · recovery {recovery*100:.0f}%"
            )
            if level_trigger_ok
            else (
        ...
        # Fix inherited V20.3.3 monitor bug: Trading Intelligence used c15
        # without guaranteeing that 15m candles had been loaded.
        if not c15:
            c15 = get_klines(symbol, "15m", 100, cache_seconds=15)
        if not c15:
            remaining.append(item)
...
def build_diagnostic(scan: Dict[str, Any]) -> str:
    ...
    return (
        f"🧪 Диагностика V20.3.3.1 · 🧲 ACTIVE MOVER + LEVEL RECLAIM + 🚀 SPIKE REGIME\n"
        ...
        f"🧲 ACTIVE MOVER V20.3.3: {_metrics_line(active_metrics)} · "
...
async def scan_loop():
    ...
    send_telegram(
        ...
        f"Mode: PAPER RESEARCH · TWO PROFESSIONAL LANES · REAL MONEY LOCKED.\nЛогика V20.3.3: 🧲 ACTIVE MOVER ищет подтверждённый continuation после pullback/reclaim; 🚀 SPIKE REGIME различает CONTINUATION и EXHAUSTION; candle intelligence оценивает squeeze/overheat/climax; при конфликте режима = NO TRADE.\n"
        ...
        f"\n"
        f"Официальные PAPER-линии: 🧲 ACTIVE MOVER + 🚀 SPIKE REGIME V20.3.3. Legacy Follow-Through/SHADOW отключены.\n"
        ...
        f"🧠 SELF-LEARNING: HARD ENABLED · fresh V20.3.3 cohort · hard-min warm-up {MIN_TRAIN_TRADES} · retrain каждые {RETRAIN_EVERY} · 2-pass validation · canary {ADAPTIVE_INITIAL_LIVE_FRACTION*100:.0f}%.\n"
        ...
        f"Diagnostics V20.3.3: current two-lane summary every 12h (2 times/day); legacy V18/V17 labels removed.\n"
        f"🧲 ACTIVE MOVER + LEVEL RECLAIM: HOT → pullback/pause → native reclaim OR resistance/support flip → re-acceleration → PAPER.\n"        f"🧲 V20.3.3.1 ACTIVE MOVER: true 1m/3m/15m/30m returns + pullback/reclaim/re-acceleration + professional candle intelligence + liquidity + symbol/strategy guards.\n"
        ...
        f"🚀 Важно: UP spike может дать LONG continuation или SHORT exhaustion; DOWN spike — SHORT continuation или LONG exhaustion. Направление сделки выбирается после классификации режима.\n"        f"V20.3.3 SPIKE REGIME: true 1m/3m/5m/15m returns · RUNAWAY guard · symbol quarantine · 3h re-entry lock · strategy circuit breaker · exhaustion reversal/volume-fade/no-new-extreme · candle/squeeze intelligence · adaptive champion/challenger.\n"
