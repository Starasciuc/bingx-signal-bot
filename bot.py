# VERIFIED GITHUB DEPLOY FILE — V21.1 PRO LONG-FIRST · FOLLOW-THROUGH + LEVEL RETEST
...
MIN_RESOLVED_MODEL_ROWS = max(60, int(os.getenv("ADAPTIVE_MIN_RESOLVED_MODEL_ROWS", "60")))
...
MODEL_DATA_POLICY = "v21_1_long_first_resolved_quality_v1"
...
def adaptive_gate(trade: Dict[str, Any]) -> Tuple[bool, str, Optional[float]]:
    ...
    if side != "LONG":
        return True, "V21.1 SHORT is shadow-only; adaptive quality model is LONG-only", None
...
def maybe_retrain(force: bool = False) -> Dict[str, Any]:
    ...
    # V21.1 is a fresh LONG-first experiment. All old versions remain audit-only.
...
APP_NAME = "Professional Adaptive Futures Bot AUTO V21.1 PRO LONG-FIRST · FOLLOW-THROUGH + LEVEL RETEST"
DEPLOY_MARKER = "V21_1_PRO_LONG_FIRST_FOLLOW_THROUGH_LEVEL_RETEST_2026_09_11"
...
STATE_FILE = os.getenv("STATE_FILE", "bot_state_v21_1_long_first.json")
...
TRADER_STYLE_PAPER_REASON = "v21_1_level_retest_long_paper"
EXHAUSTION_PAPER_REASON = "v21_1_impulse_continuation_long_paper"  # compatibility name; exhaustion is disabled
V21_LEVEL_SHORT_SHADOW_REASON = "v21_1_level_retest_short_shadow"
V21_IMPULSE_SHORT_SHADOW_REASON = "v21_1_impulse_continuation_short_shadow"
V21_FOLLOWTHROUGH_FILTER_REASON = "v21_1_followthrough_filter"
...
V21_DAILY_LOSS_LIMIT_R = float(os.getenv("V21_DAILY_LOSS_LIMIT_R", "-4.0"))
V21_CIRCUIT_COOLDOWN_SECONDS = int(os.getenv("V21_CIRCUIT_COOLDOWN_SECONDS", "7200"))
...
def v21_flow_gate(setup: Dict[str, Any]) -> Tuple[bool, str]:
    side = str(setup.get("side", "")).upper(); direction = 1.0 if side == "LONG" else -1.0
    taker = direction * float(setup.get("scalp_taker_imbalance", 0.0) or 0.0)
    cmf = direction * float(setup.get("scalp_cmf", 0.0) or 0.0)
    obv = direction * float(setup.get("scalp_obv_pressure", 0.0) or 0.0)
    hard = taker <= -0.30 and cmf <= -0.08 and obv <= -0.10
    return (not hard), (f"flow conflict taker={taker:+.2f} cmf={cmf:+.2f} obv={obv:+.2f}" if hard else f"flow ok taker={taker:+.2f} cmf={cmf:+.2f} obv={obv:+.2f}")
...
def process_active_mover_watches() -> Dict[str, int]:
    ...
    """
    stats = {"checked": 0, "triggered": 0, "short_shadow": 0, "expired": 0, "rejected": 0, "research_blocked": 0, "mtf_blocked": 0, "mtf_valid": 0, "followthrough_blocked": 0}
    if not ACTIVE_MOVER_ENABLED:
        return stats
    ...
    for item in snapshot:
        ...
        if side == "SHORT":
            trade["paper_validation_origin"] += " · V21.1 SHORT SHADOW ONLY"
...
def process_squeeze_exhaustion_watches() -> Dict[str, int]:
    ...
    stats = {
        ...
        "runaway_watch": 0, "triggered": 0, "short_shadow": 0, "followthrough_blocked": 0, "expired": 0,
...
def build_diagnostic(scan: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"🎯 V21 LEVEL RETEST cohort: {_metrics_line(active_metrics)} · "
...
async def scan_loop():
    ...
    send_telegram(
        ...
        f"🧠 SELF-LEARNING: fresh V21-only cohort · warm-up {MIN_TRAIN_TRADES} · retrain every {RETRAIN_EVERY} · "
