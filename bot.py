# VERIFIED GITHUB DEPLOY FILE — V20.3.2.2 REFERENCE SCALP · BLOGGER CONFLUENCE · PRO TRADING INTELLIGENCE
...
MIN_TRAIN_TRADES = int(os.getenv("ADAPTIVE_MIN_TRAIN_TRADES", "75"))
...
MODEL_DATA_POLICY = "v20_3_2_trading_intelligence_forward_only_v1"
...
def build_feature_dict(trade: Dict[str, Any]) -> Dict[str, float]:
    ...
    features = {
        ...
        # V20.3.2.2 audit-only fields; not promoted to FEATURE_NAMES until forward validation.
        "reference_match": _clip(trade.get("reference_match"), 0.0, 100.0) / 100.0,
        "reference_strong": 1.0 if trade.get("reference_strong") else 0.0,
        "reference_target_scale": _clip(trade.get("reference_target_scale"), 0.8, 1.2, 1.0),
        "reference_reassess_move": _clip(trade.get("reference_reassess_move"), 0.0, 0.15),
    }
...
APP_NAME = "Professional Adaptive Futures Bot AUTO V20.3.2.3 · RENDER SAFE START + REFERENCE SCALP + BLOGGER CONFLUENCE + PRO TRADING INTELLIGENCE"
DEPLOY_MARKER = "V20_3_2_3_RENDER_SAFE_REFERENCE_SCALP_2026_09_08"
...
TRADER_STYLE_PAPER_REASON = "active_mover_paper_v20_3_2"
EXHAUSTION_PAPER_REASON = "spike_regime_paper_v20_3_2"
...
SPIKE_ADAPTIVE_BLOCK_REASON = "spike_adaptive_block_v20_3_2"
SPIKE_STRATEGY_GUARD_REASON = "spike_strategy_guard_v20_3_2"
ACTIVE_MOVER_GUARD_REASON = "active_mover_strategy_guard_v20_3_2"
...
# --- V20.3.2.2 REFERENCE SCALP + BLOGGER CONFLUENCE LAYER ---
# Research-only layer. It does NOT replace the proven V20.3.2 detectors.
# It adds four independent, observable confirmations:
#   WADE      = trend + two-legged pullback + EMA/key-point signal bar
#   WIELAND   = VWAP bias + EMA fan + flag/re-break continuation
#   TBM       = VWAP + candle-volume POC proxy + reaction/aggression at level
#   LIQUIDITY = sweep/reclaim of a recent high/low (portable part of liquidity-map logic)
# Proprietary options/gamma feeds are intentionally NOT simulated.
BLOGGER_CONFLUENCE_ENABLED = os.getenv("BLOGGER_CONFLUENCE_ENABLED", "true").lower() == "true"
BLOGGER_MIN_ENTRY_SCORE = float(os.getenv("BLOGGER_MIN_ENTRY_SCORE", "48"))
BLOGGER_STRONG_LANE_SCORE = float(os.getenv("BLOGGER_STRONG_LANE_SCORE", "68"))
BLOGGER_MAX_HARD_CONFLICTS = int(os.getenv("BLOGGER_MAX_HARD_CONFLICTS", "2"))
BLOGGER_SCORE_BONUS_MAX = float(os.getenv("BLOGGER_SCORE_BONUS_MAX", "7"))
BLOGGER_KEYPOINT_MAX_ATR = float(os.getenv("BLOGGER_KEYPOINT_MAX_ATR", "0.85"))
BLOGGER_LEVEL_MAX_ATR = float(os.getenv("BLOGGER_LEVEL_MAX_ATR", "1.00"))
BLOGGER_SWEEP_TOLERANCE_ATR = float(os.getenv("BLOGGER_SWEEP_TOLERANCE_ATR", "0.10"))

# --- V20.3.2.2 REFERENCE SCALP PATTERN ---
# Calibrated from two user-supplied successful examples:
# H LONG  : ~0.99 / 1.59 / 2.19 / 3.00 / 4.39%, reassessment ~8.79% against entry
# UB SHORT: ~0.81 / 1.40 / 2.00 / 3.01 / 4.00%, reassessment ~8.18% against entry
# Shared template ≈ 0.90 / 1.50 / 2.10 / 3.00 / 4.20%.
# It is a geometry/quality reference, NOT a standalone signal. The ~8.5% zone
# is NEVER an automatic averaging order: only a fresh independent A+ setup may re-enter.
REFERENCE_SCALP_ENABLED = os.getenv("REFERENCE_SCALP_ENABLED", "true").lower() == "true"
REFERENCE_SCALP_STRONG_SCORE = float(os.getenv("REFERENCE_SCALP_STRONG_SCORE", "70"))
REFERENCE_SCALP_SCORE_BONUS_MAX = float(os.getenv("REFERENCE_SCALP_SCORE_BONUS_MAX", "4"))
REFERENCE_TP1_MOVE = float(os.getenv("REFERENCE_TP1_MOVE", "0.0090"))
REFERENCE_TP2_MOVE = float(os.getenv("REFERENCE_TP2_MOVE", "0.0150"))
REFERENCE_TP3_MOVE = float(os.getenv("REFERENCE_TP3_MOVE", "0.0210"))
REFERENCE_TP4_MOVE = float(os.getenv("REFERENCE_TP4_MOVE", "0.0300"))
REFERENCE_TP5_MOVE = float(os.getenv("REFERENCE_TP5_MOVE", "0.0420"))
...
def reference_scalp_context(
    setup: Dict[str, Any],
    c1: List[Dict[str, float]],
    c5: List[Dict[str, float]],
) -> Dict[str, Any]:
    """Measure similarity to the H LONG + UB SHORT successful scalp geometry.

    This never creates a trade by itself and never bypasses safety/execution gates.
    A strong match can add a small quality bonus and refine the TP ladder.
    """
    side = str(setup.get("side", "")).upper()
    entry = float(setup.get("entry", 0.0) or 0.0)
    defaults = (
        REFERENCE_TP1_MOVE, REFERENCE_TP2_MOVE, REFERENCE_TP3_MOVE,
        REFERENCE_TP4_MOVE, REFERENCE_TP5_MOVE,
    )
    if not REFERENCE_SCALP_ENABLED or side not in {"LONG", "SHORT"} or entry <= 0 or len(c5) < 16:
        return {
            "score": 0.0, "strong": False, "target_scale": 1.0,
            "target_moves": defaults, "reassess_move": REFERENCE_REASSESS_MOVE,
            "reassess_price": 0.0, "reason": "reference pattern unavailable",
        }

    direction = 1.0 if side == "LONG" else -1.0
    a5 = atr(c5, 14)
    atr_pct = a5 / max(entry, 1e-12)
    raw_scale = 0.90 + 0.10 * (atr_pct / max(REFERENCE_ATR_BASE, 1e-9))
    scale = _clip(raw_scale, REFERENCE_TARGET_SCALE_MIN, REFERENCE_TARGET_SCALE_MAX, 1.0)
    target_moves = tuple(move * scale for move in defaults)

    score = 18.0
    reasons: List[str] = []

    blogger = _clip(setup.get("blogger_score"), 0.0, 100.0)
    score += blogger * 0.22
    if blogger >= BLOGGER_STRONG_LANE_SCORE:
        reasons.append(f"blogger {blogger:.0f}")

    intel = _clip(setup.get("intel_setup_quality"), 0.0, 100.0)
    score += intel * 0.18
    if intel >= 65:
        reasons.append(f"intel {intel:.0f}")

    vol1 = _clip(setup.get("vol1"), 0.0, 8.0)
    range1 = _clip(setup.get("range1"), 0.0, 8.0)
    score += min(9.0, vol1 / 1.20 * 9.0)
    score += min(7.0, range1 / 1.40 * 7.0)
    if vol1 >= 0.65:
        reasons.append(f"Vol1 x{vol1:.2f}")
    if range1 >= 0.75:
        reasons.append(f"Range1 x{range1:.2f}")

    directional_3m = direction * float(setup.get("ch3m_1m", 0.0) or 0.0)
    if directional_3m > 0:
        score += min(9.0, directional_3m / 0.012 * 9.0)
        reasons.append(f"3m aligned {directional_3m*100:.2f}%")
    else:
    ...
    if 0.0020 <= pullback <= 0.0200:
        score += 7.0
        reasons.append(f"pullback {pullback*100:.2f}%")
    elif pullback > 0.035:
        score -= 6.0
    if recovery >= 0.50:
        score += min(5.0, recovery * 5.0)

    distance_atr = _clip(setup.get("intel_distance_atr"), 0.0, 20.0)
    climax = _clip(setup.get("intel_climax"), 0.0, 100.0)
    if distance_atr <= 3.5 and climax < 82:
        score += 5.0
    elif distance_atr > 5.0 or climax >= 90:
        score -= 8.0

    recent = c5[-24:]
    recent_hi = max(float(x["high"]) for x in recent)
    recent_lo = min(float(x["low"]) for x in recent)
    recent_range = (recent_hi - recent_lo) / max(entry, 1e-12)
    viability = recent_range / max(target_moves[-1], 1e-9)
    if viability >= 0.85:
        score += 8.0
        reasons.append(f"TP5 range viable {viability:.1f}x")
    elif viability >= 0.55:
        score += 4.0
    else:
        score -= 5.0

    conflicts = int(setup.get("blogger_conflicts", 0) or 0)
    score -= min(12.0, conflicts * 4.0)
    score = _clip(score, 0.0, 100.0)
    strong = score >= REFERENCE_SCALP_STRONG_SCORE
    reassess_price = entry * (1.0 - REFERENCE_REASSESS_MOVE) if side == "LONG" else entry * (1.0 + REFERENCE_REASSESS_MOVE)
    reason = f"reference {score:.0f}/100 {'STRONG' if strong else 'observe'} · " + (", ".join(reasons[:6]) if reasons else "no strong similarities")
    return {
        "score": score, "strong": strong, "target_scale": scale,
        "target_moves": target_moves, "reassess_move": REFERENCE_REASSESS_MOVE,
        "reassess_price": reassess_price, "atr_pct": atr_pct,
        "recent_range": recent_range, "reason": reason,
    }
...
def _attach_reference_context(setup: Dict[str, Any], context: Dict[str, Any]) -> None:
    ...
    setup["reference_target_scale"] = float(context.get("target_scale", 1.0) or 1.0)
    setup["reference_target_moves"] = list(context.get("target_moves") or ())
    setup["reference_reassess_move"] = float(context.get("reassess_move", REFERENCE_REASSESS_MOVE) or REFERENCE_REASSESS_MOVE)
    setup["reference_reassess_price"] = float(context.get("reassess_price", 0.0) or 0.0)
    setup["reference_reason"] = str(context.get("reason", ""))


def _reference_target_moves(
    setup: Dict[str, Any],
    fallback: Tuple[float, float, float, float, float],
    blend: float,
    ...
    floors = (0.0065, 0.0115, 0.0180, 0.0260, 0.0350)
    caps = (0.0105, 0.0165, 0.0220, 0.0320, 0.0445)
    clean = tuple(_clip(x, lo, hi, old) for x, lo, hi, old in zip(blended, floors, caps, fallback))
    if not all(clean[i] < clean[i + 1] for i in range(4)):
        return fallback
    return clean
...
def calculate_active_mover_trade(
    ...
    target_moves = _reference_target_moves(
        setup,
        (ACTIVE_TP1_MOVE, ACTIVE_TP2_MOVE, ACTIVE_TP3_MOVE, ACTIVE_TP4_MOVE, ACTIVE_TP5_MOVE),
        blend=0.40,
    )
    if side == "LONG":
        sl = entry * (1 - sl_move)
        tp1, tp2, tp3, tp4, tp5 = [entry * (1 + move) for move in target_moves]
    else:
        sl = entry * (1 + sl_move)
        tp1, tp2, tp3, tp4, tp5 = [entry * (1 - move) for move in target_moves]
    ...
    trade.update({
        ...
        "target_profile": "REFERENCE_BLEND" if bool(setup.get("reference_strong")) else "ACTIVE_NATIVE",
...
def _aggregate_fixed_bars(candles: List[Dict[str, float]], group: int) -> List[Dict[str, float]]:
    """Aggregate time-ordered candles into fixed groups (used for a 3m proxy)."""
    if group <= 1:
        return list(candles)
    rows = list(candles)
    ...
    for i in range(0, len(rows), group):
        ...
        out.append({
            "time": float(chunk[0].get("time", 0.0) or 0.0),
...
def _candle_volume_poc_proxy(candles: List[Dict[str, float]], bins: int = 24) -> float:
    """Approximate a rolling volume POC from candle typical-price volume.

    BingX kline data does not expose exchange-grade volume-at-price. We therefore
    use this only as a soft level-of-interest proxy, never as a hard claim that it
    equals a true futures Volume Profile POC.
    """
    rows = list(candles[-72:])
    if not rows:
        return 0.0
    ...
    bins = max(8, min(64, int(bins)))
    width = (hi - lo) / bins
    bucket = [0.0] * bins
    for row in rows:
        typical = (float(row["high"]) + float(row["low"]) + float(row["close"])) / 3.0
        idx = int((typical - lo) / max(width, 1e-12))
        idx = max(0, min(bins - 1, idx))
        bucket[idx] += max(0.0, float(row.get("volume", 0.0) or 0.0))
    best = max(range(bins), key=lambda i: bucket[i])
    return lo + (best + 0.5) * width


def _countertrend_leg_count(candles: List[Dict[str, float]], side: str, lookback: int = 12) -> int:
    """Count distinct counter-trend pushes separated by a directional bounce."""
    rows = list(candles[-max(6, lookback):])
    if len(rows) < 5:
        return 0
    direction = 1.0 if side == "LONG" else -1.0
    legs = 0
    ...
    for prev, cur in zip(rows, rows[1:]):
        move = direction * (float(cur["close"]) - float(prev["close"]))
        if move < 0:
            if not in_counter:
                legs += 1
                in_counter = True
...
def blogger_strategy_context(
    ...
    """Portable confluence layer inspired by four public futures trading styles.

    This is intentionally a *confluence* layer, not a claim to reproduce any
    trader's proprietary system. Only observable price/volume logic is used.
    """
    side = str(side).upper()
    if side not in {"LONG", "SHORT"} or len(c1) < 35 or len(c5) < 35 or len(c15) < 25:
        return {
            "score": 0.0, "primary": "NONE", "passed": False,
            "confirmations": 0, "conflicts": 0,
            "wade": 0.0, "wieland": 0.0, "tbm": 0.0, "liquidity": 0.0,
            "reason": "blogger context warming",
        }
    ...
    loc = close_location(last)
    signal_bar = bool(
        (side == "LONG" and float(last["close"]) > float(last["open"]) and loc >= 0.58)
        or (side == "SHORT" and float(last["close"]) < float(last["open"]) and loc <= 0.42)
    )
    vol1 = volume_ratio(c1, 20)
    range1 = candle_range_ratio(c1, 20)

    # Thomas Wade style: trend, key entry point, two-legged pullback, signal bar.
    e21_5 = ema(closes(c5[-60:]), 21)
    e21_15 = ema(closes(c15[-60:]), 21)
    e50_15 = ema(closes(c15[-80:]), 50)
    htf_ok = direction * (e21_15 - e50_15) > 0
    if c1h and len(c1h) >= 25:
        e21_1h = ema(closes(c1h[-60:]), 21)
        e50_1h = ema(closes(c1h[-80:]), 50)
        htf_ok = htf_ok and direction * (e21_1h - e50_1h) >= -0.05 * a5
    keypoint_distance = abs(price - e21_5) / a5
    legs = _countertrend_leg_count(c1, side, 14)
    wade = 0.0
    wade += 28.0 if htf_ok else 0.0
    wade += 26.0 if keypoint_distance <= BLOGGER_KEYPOINT_MAX_ATR else max(0.0, 16.0 - keypoint_distance * 6.0)
    wade += 24.0 if legs >= 2 else (10.0 if legs == 1 else 0.0)
    wade += 22.0 if signal_bar else 0.0
    wade = max(0.0, min(100.0, wade))

    # Patrick Wieland style: VWAP bias + 8/21/51 EMA fan + flag/re-break.
    c3 = _aggregate_fixed_bars(c1[-90:], 3)
    wieland = 0.0
    if len(c3) >= 20:
        c3_close = closes(c3)
        e8_3 = ema(c3_close, 8)
        e21_3 = ema(c3_close, 21)
        e51_3 = ema(c3_close, min(51, len(c3_close)))
        vw3 = vwap(c3, min(30, len(c3)))
        fan_ok = (e8_3 > e21_3 > e51_3) if side == "LONG" else (e8_3 < e21_3 < e51_3)
        vwap_ok = (price >= vw3) if side == "LONG" else (price <= vw3)
        ema15_ok = (price >= e21_15) if side == "LONG" else (price <= e21_15)
        flag_rows = c3[-5:-1]
        flag_span = (max(float(x["high"]) for x in flag_rows) - min(float(x["low"]) for x in flag_rows)) / max(price, 1e-12)
        recent_atr_pct = max(a5 / price, 1e-6)
        compact_flag = flag_span <= recent_atr_pct * 3.2
        rebreak = (
            float(c3[-1]["close"]) > float(c3[-2]["high"])
            if side == "LONG"
            else float(c3[-1]["close"]) < float(c3[-2]["low"])
        )
        wieland += 26.0 if vwap_ok else 0.0
        wieland += 26.0 if fan_ok else 0.0
        wieland += 18.0 if ema15_ok else 0.0
        wieland += 15.0 if compact_flag else 0.0
        wieland += 15.0 if rebreak else (6.0 if signal_bar else 0.0)
    wieland = max(0.0, min(100.0, wieland))

    # Trades by Matt style: VWAP + volume-profile POC proxy + aggression at level.
    vw5 = vwap(c5, min(48, len(c5)))
    poc = _candle_volume_poc_proxy(c5, 24)
    nearest_level = vw5 if abs(price - vw5) <= abs(price - poc) else poc
    level_distance = abs(price - nearest_level) / a5
    crossed_level = bool(
        (side == "LONG" and float(prev["close"]) <= nearest_level <= float(last["close"]))
        or (side == "SHORT" and float(prev["close"]) >= nearest_level >= float(last["close"]))
    )
    held_level = bool(
        (side == "LONG" and price >= nearest_level and float(last["low"]) <= nearest_level * 1.0025)
        or (side == "SHORT" and price <= nearest_level and float(last["high"]) >= nearest_level * 0.9975)
    )
    aggression = signal_bar and vol1 >= 0.80 and range1 >= 0.75
    tbm = 0.0
    tbm += 26.0 if level_distance <= BLOGGER_LEVEL_MAX_ATR else max(0.0, 14.0 - level_distance * 5.0)
    tbm += 24.0 if crossed_level else (14.0 if held_level else 0.0)
    tbm += 24.0 if aggression else (10.0 if signal_bar else 0.0)
    tbm += 14.0 if ((price >= vw5) if side == "LONG" else (price <= vw5)) else 0.0
    tbm += 12.0 if ((price >= poc) if side == "LONG" else (price <= poc)) else 0.0
    tbm = max(0.0, min(100.0, tbm))

    # Portable liquidity-map component: sweep of a prior local extreme + reclaim.
    prior = c1[-24:-3]
    recent = c1[-3:]
    prior_low = min(float(x["low"]) for x in prior)
    prior_high = max(float(x["high"]) for x in prior)
    sweep_tol = a5 * BLOGGER_SWEEP_TOLERANCE_ATR
    if side == "LONG":
        swept = min(float(x["low"]) for x in recent) <= prior_low + sweep_tol
        reclaimed = price > prior_low and float(last["close"]) > float(prev["close"])
        rejection = (float(last["close"]) - float(last["low"])) / max(float(last["high"]) - float(last["low"]), 1e-12)
    else:
        swept = max(float(x["high"]) for x in recent) >= prior_high - sweep_tol
        reclaimed = price < prior_high and float(last["close"]) < float(prev["close"])
        rejection = (float(last["high"]) - float(last["close"])) / max(float(last["high"]) - float(last["low"]), 1e-12)
    liquidity = 0.0
    liquidity += 40.0 if swept else 0.0
    liquidity += 32.0 if reclaimed else 0.0
    liquidity += 18.0 if rejection >= 0.55 else 0.0
    liquidity += 10.0 if vol1 >= 0.80 else 0.0
    liquidity = max(0.0, min(100.0, liquidity))

    lanes = {"WADE": wade, "WIELAND": wieland, "TBM": tbm, "LIQUIDITY": liquidity}
    ordered = sorted(lanes.items(), key=lambda item: item[1], reverse=True)
    top_name, top_score = ordered[0]
    second_score = ordered[1][1]
    # One very clean setup is enough; agreement from a second style raises confidence.
    score = min(100.0, top_score * 0.78 + second_score * 0.22)
    confirmations = sum(1 for value in lanes.values() if value >= BLOGGER_STRONG_LANE_SCORE)

    conflicts = 0
    # Genuine directional conflicts only; missing a setup is not a conflict.
    if direction * (e21_15 - e50_15) < 0:
        conflicts += 1
    if (side == "LONG" and price < vw5 - 0.75 * a5) or (side == "SHORT" and price > vw5 + 0.75 * a5):
        conflicts += 1
    if signal_bar is False and vol1 < 0.65:
        conflicts += 1

    passed = bool(score >= BLOGGER_MIN_ENTRY_SCORE or top_score >= BLOGGER_STRONG_LANE_SCORE)
    reason = (
        f"blogger confluence {score:.0f}/100 · primary {top_name} {top_score:.0f} · "
        f"Wade {wade:.0f} / Wieland {wieland:.0f} / TBM {tbm:.0f} / Liquidity {liquidity:.0f} · "
        f"conflicts {conflicts} · EMA21dist {keypoint_distance:.2f}ATR · POCproxy {poc:.8g}"
    )
    return {
        "score": score,
        "primary": top_name,
...
def _attach_blogger_context(setup: Dict[str, Any], context: Dict[str, Any]) -> None:
    setup["blogger_score"] = float(context.get("score", 0.0) or 0.0)
    setup["blogger_primary"] = str(context.get("primary", "NONE"))
    setup["blogger_passed"] = bool(context.get("passed", False))
    setup["blogger_confirmations"] = int(context.get("confirmations", 0) or 0)
    setup["blogger_conflicts"] = int(context.get("conflicts", 0) or 0)
    setup["blogger_wade"] = float(context.get("wade", 0.0) or 0.0)
    setup["blogger_wieland"] = float(context.get("wieland", 0.0) or 0.0)
    setup["blogger_tbm"] = float(context.get("tbm", 0.0) or 0.0)
    setup["blogger_liquidity"] = float(context.get("liquidity", 0.0) or 0.0)
    setup["blogger_reason"] = str(context.get("reason", ""))


def blogger_hard_conflict(context: Dict[str, Any]) -> bool:
    """Block only when several independent conditions actively disagree."""
    if not BLOGGER_CONFLUENCE_ENABLED:
        return False
    return bool(
        int(context.get("conflicts", 0) or 0) >= BLOGGER_MAX_HARD_CONFLICTS
        and float(context.get("score", 0.0) or 0.0) < BLOGGER_MIN_ENTRY_SCORE
    )
...
def process_active_mover_watches() -> Dict[str, int]:
    ...
    for item in snapshot:
        ...
        # V20.3.2.1 FIX: V20.3.2 used c15 later in this function without
        # loading it here. That could raise NameError after a watch reached trigger.
        c15 = get_klines(symbol, "15m", 80, cache_seconds=12)
        c1h = get_klines(symbol, "1h", 60, cache_seconds=25)
        if not c1 or not c5 or not c15 or not c1h or ref <= 0:
        ...
        blogger_ctx = blogger_strategy_context(c1, c5, c15, c1h, side) if BLOGGER_CONFLUENCE_ENABLED else {
            "score": 100.0, "primary": "DISABLED", "passed": True, "confirmations": 0, "conflicts": 0,
            "wade": 0.0, "wieland": 0.0, "tbm": 0.0, "liquidity": 0.0, "reason": "disabled"
        }
        _attach_blogger_context(setup, blogger_ctx)
        if blogger_hard_conflict(blogger_ctx):
            stats["rejected"] += 1
            continue
        # Do not replace V20.3.2's trigger. The layer is a quality bonus and an
        # active-conflict veto only when several independent observations disagree.
        setup["score"] = min(100, int(float(setup.get("score", 88) or 88) + min(
            BLOGGER_SCORE_BONUS_MAX, float(blogger_ctx.get("score", 0.0) or 0.0) / 100.0 * BLOGGER_SCORE_BONUS_MAX
        )))
        gate_reason += f" · {blogger_ctx.get('reason', '')}"

        reference_ctx = reference_scalp_context(setup, c1, c5)
        _attach_reference_context(setup, reference_ctx)
        if REFERENCE_SCALP_ENABLED:
...
def calculate_exhaustion_trade(setup: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    target_moves = _reference_target_moves(
        setup,
        (EXHAUST_TP1_MOVE, EXHAUST_TP2_MOVE, EXHAUST_TP3_MOVE, EXHAUST_TP4_MOVE, EXHAUST_TP5_MOVE),
        blend=0.75,
    )
    if side == "SHORT":
        sl = entry * (1 + sl_move)
        tps = [entry * (1 - move) for move in target_moves]
    else:
        sl = entry * (1 - sl_move)
        tps = [entry * (1 + move) for move in target_moves]
    ...
    trade.update({
        ...
        "risk_mult": FAST_RISK_MULT,
        "roi_tp1": target_moves[0] * LEVERAGE * 100,
        "target_profile": "REFERENCE_BLEND" if bool(setup.get("reference_strong")) else "SPIKE_NATIVE",
        "target_moves": list(target_moves),
        "roi_sl": sl_move * LEVERAGE * 100,
        "status": "active",
        "tp1_hit": False, "tp2_hit": False, "tp3_hit": False, "tp4_hit": False, "tp5_hit": False,
...
def process_squeeze_exhaustion_watches() -> Dict[str, int]:
    ...
    stats = {
        ...
        "adaptive_errors": 0, "exhaust_quality_wait": 0, "blogger_conflict_blocked": 0,
    }
    if not SQUEEZE_4H_ENABLED:
        return stats
    ...
    for item in snapshot:
        ...
        blogger_ctx = blogger_strategy_context(c1, c5, c15, c1h, trade_side) if BLOGGER_CONFLUENCE_ENABLED else {
            "score": 100.0, "primary": "DISABLED", "passed": True, "confirmations": 0, "conflicts": 0,
            "wade": 0.0, "wieland": 0.0, "tbm": 0.0, "liquidity": 0.0, "reason": "disabled"
        }
        if blogger_hard_conflict(blogger_ctx):
            stats["blogger_conflict_blocked"] += 1
            remaining.append(item)
            continue
        ...
        _attach_blogger_context(setup, blogger_ctx)
        setup["score"] = min(100, int(float(setup.get("score", 90) or 90) + min(
            BLOGGER_SCORE_BONUS_MAX, float(blogger_ctx.get("score", 0.0) or 0.0) / 100.0 * BLOGGER_SCORE_BONUS_MAX
        )))
        setup["paper_validation_origin"] = (
            ...
            f"BLOGGER: {blogger_ctx.get('reason', '')}."
        )
...
def build_signal_message(s: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Reference pattern: {float(s.get('reference_match', 0.0) or 0.0):.0f}/100 · {s.get('target_profile', 'native')} · "
        f"reassessment {format_price(s.get('reference_reassess_price'))} (NO AUTO-AVERAGING)\n\n"
        f"📌 Логика:\n{s['reason']}\n"
...
def build_paper_signal_message(s: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Blogger confluence: {float(s.get('blogger_score', 0.0) or 0.0):.0f}/100 · "
        f"primary {s.get('blogger_primary', 'NONE')} · "
        f"Wade {float(s.get('blogger_wade', 0.0) or 0.0):.0f} / "
        f"Wieland {float(s.get('blogger_wieland', 0.0) or 0.0):.0f} / "
        f"TBM {float(s.get('blogger_tbm', 0.0) or 0.0):.0f} / "
        f"Liquidity {float(s.get('blogger_liquidity', 0.0) or 0.0):.0f}\n"
        f"Reference pattern: {float(s.get('reference_match', 0.0) or 0.0):.0f}/100 · "
        f"{'STRONG' if s.get('reference_strong') else 'observe'} · targets {s.get('target_profile', 'native')}\n"
        f"Reassessment zone: {format_price(s.get('reference_reassess_price'))} · "
        f"NO AUTO-AVERAGING; only a fresh independent A+ setup may re-enter\n"
...
def build_diagnostic(scan: Dict[str, Any]) -> str:
    ...
    return (
        f"🧪 Диагностика V20.3.2.2 REFERENCE SCALP + BLOGGER CONFLUENCE · 🧲 ACTIVE MOVER + 🚀 SPIKE REGIME\n"
        ...
        f"🧲 ACTIVE MOVER V20.3.2: {_metrics_line(active_metrics)} · "
        f"n={int(active_metrics.get('n', 0) or 0)}\n"
        f"🚀 SPIKE REGIME V20.3.2: {_metrics_line(spike_metrics)} · "
...
async def scan_loop():
    ...
    send_telegram(
        ...
        f"Mode: PAPER RESEARCH · V20.3.2 CORE + BLOGGER CONFLUENCE + REFERENCE SCALP · REAL MONEY LOCKED.\n"
        ...
        f"Официальные PAPER-линии: 🧲 ACTIVE MOVER + 🚀 SPIKE REGIME V20.3.2.3. Legacy Follow-Through/SHADOW отключены.\n"
        ...
        f"🧠 SELF-LEARNING: HARD ENABLED · fresh V20.3.2.3 cohort · warm-up {MIN_TRAIN_TRADES} · retrain каждые {RETRAIN_EVERY} · 2-pass validation · canary {ADAPTIVE_INITIAL_LIVE_FRACTION*100:.0f}%.\n"
        ...
        f"🎯 BLOGGER CONFLUENCE: Wade second-entry/key-point + Wieland VWAP/EMA/flag + "
        f"Trades by Matt VWAP/volume-level reaction + portable liquidity sweep/reclaim. "
        f"Layer does not replace V20.3.2; only boosts aligned setups and blocks strong multi-factor conflicts.\n"
        f"📐 REFERENCE SCALP: H LONG + UB SHORT geometry · match 0–100 · strong matches refine targets toward "
        f"~0.9/1.5/2.1/3.0/4.2% with mild ATR scaling. ~8.5% is reassessment only — NEVER blind averaging.\n"
        f"⚠️ No proprietary gamma/options feed is simulated; TBM POC is explicitly a candle-volume proxy.\n"
        f"Diagnostics V20.3.2.3: current two-lane summary every 12h (2 times/day); no legacy V18/V17 labels.\n"
        f"🧲 ACTIVE MOVER: HOT → quiet WATCH → pullback/reclaim → re-acceleration → PAPER.\n"        f"🧲 V20.3: ACTIVE MOVER uses corrected true 1m/3m/15m/30m returns + native pullback/reclaim/re-acceleration + liquidity + symbol/strategy guards.\n"
        ...
        f"🚀 Важно: UP spike может дать LONG continuation или SHORT exhaustion; DOWN spike — SHORT continuation или LONG exhaustion. Направление сделки выбирается после классификации режима.\n"        f"V20.3: true 1m/3m/5m/15m returns · RUNAWAY guard · symbol quarantine · 3h re-entry lock · strategy circuit breaker · exhaustion reversal/volume-fade/no-new-extreme · adaptive champion/challenger.\n"
...
async def _send_immediate_boot_notice() -> None:
    """Send a minimal startup notice without being able to fail ASGI startup."""
    try:
        await asyncio.to_thread(
            send_telegram,
            f"🟢 BOOT · PAPER\nСборка: {APP_NAME}\nDeploy: {DEPLOY_MARKER}\n"
            "HTTP-приложение поднято; запускаю state/DB/scanner workers.",
        )
    except Exception as exc:
        try:
            print(f"BOOT TELEGRAM ERROR: {exc!r}", flush=True)
        except Exception:
            pass
...
async def startup_event():
    ...
    # Render-visible boot marker. If this line appears in logs, bot.py imported
    # and FastAPI entered startup successfully.
    try:
        print(f"BOOT START {APP_NAME} | {DEPLOY_MARKER}", flush=True)
    except Exception:
        pass

    # Fire the Telegram boot notice BEFORE DB/scanner initialization. It is
    # deliberately non-blocking so Telegram/network problems cannot fail Render.
    try:
        boot_task = asyncio.create_task(_send_immediate_boot_notice(), name="render-safe:boot-notice")
        BACKGROUND_TASKS.append(boot_task)
    except Exception as exc:
        try:
            print(f"BOOT NOTICE SCHEDULE ERROR: {exc!r}", flush=True)
        except Exception:
            pass
