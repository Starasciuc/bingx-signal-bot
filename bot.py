...
APP_NAME = "Professional Adaptive Futures Bot V20.4 PRO CONTINUATION"
DEPLOY_MARKER = "V20_4_PRO_CONTINUATION_CANDLE_ENGINE_2026_09_03"
...
class TelegramClient:
    ...
    async def send(self, text: str) -> bool:
        if not self.enabled:
            LOG.info("Telegram disabled; message follows:\n%s", text)
            return False
        ...
        try:
            async with self.session.post(url, json=payload) as response:
                ...
                if response.status >= 400 or not body.get("ok", False):
                    raise RuntimeError(
                        f"Telegram HTTP {response.status}: {str(body)[:240]}"
                    )
            return True
        except (aiohttp.ClientError, asyncio.TimeoutError, RuntimeError) as error:
...
class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802 - required by BaseHTTPRequestHandler
        if self.path not in {"/", "/health"}:
            ...
            self.end_headers()
            return
        payload = json.dumps({**HEALTH, "status": "ok", "time": utc_text()}).encode(
            "utf-8"
        )
