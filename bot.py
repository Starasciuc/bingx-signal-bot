# VERIFIED GITHUB DEPLOY FILE — V20.4.0 SAFETY REPAIR · FRESH FORWARD COHORT
...
DB_PATH = os.getenv("ADAPTIVE_DB_PATH", "adaptive_bot_v20_4_0.sqlite3")
# A model with dozens of features must not be promoted from a tiny sample.
# These are hard safety floors: environment variables may raise, but not lower,
# the amount of independent evidence required for model influence.
MIN_TRAIN_TRADES = max(500, int(os.getenv("ADAPTIVE_MIN_TRAIN_TRADES", "500")))
...
MIN_VALIDATION_ROWS = max(75, int(os.getenv("ADAPTIVE_MIN_VALIDATION_ROWS", "75")))
MIN_CALIBRATION_ROWS = max(75, int(os.getenv("ADAPTIVE_MIN_CALIBRATION_ROWS", "75")))
MIN_POSITIVE_ROWS = max(40, int(os.getenv("ADAPTIVE_MIN_POSITIVE_ROWS", "40")))
MIN_NEGATIVE_ROWS = max(40, int(os.getenv("ADAPTIVE_MIN_NEGATIVE_ROWS", "40")))
...
MIN_SELECTED_TEST_ROWS = max(30, int(os.getenv("ADAPTIVE_MIN_SELECTED_TEST_ROWS", "30")))
MIN_SELECTED_LIVE_TEST_ROWS = max(30, int(os.getenv("ADAPTIVE_MIN_SELECTED_LIVE_TEST_ROWS", "30")))
...
ADAPTIVE_TARGET_SUCCESS_RATE = max(
    0.50, float(os.getenv("ADAPTIVE_TARGET_SUCCESS_RATE", "0.50"))
)
ADAPTIVE_CONFIRMATION_PASSES = max(
    3, int(os.getenv("ADAPTIVE_CONFIRMATION_PASSES", "3"))
)
ADAPTIVE_CONFIRMATION_SELECTED = max(
    100, int(os.getenv("ADAPTIVE_CONFIRMATION_SELECTED", "100"))
)
...
MODEL_DATA_POLICY = "v20_4_0_safety_repair_fresh_forward_only_v1"
...
ROUND_TRIP_COST_MOVE = max(
    0.0020, float(os.getenv("ROUND_TRIP_COST_MOVE", "0.0020"))
)
...
def _connect() -> sqlite3.Connection:
    ...
    # Live-admission evidence must survive abrupt process/host failure.
...
def format_training_attempt_message(report: Dict[str, Any]) -> str:
    ...
    lines = [
        title,
        f"Данных модели (V20.4 fresh SPIKE candidates): {closed_count}",
...
APP_NAME = "Professional Adaptive Futures Bot V20.4.0 SAFETY REPAIR"
DEPLOY_MARKER = "V20_4_0_SAFETY_REPAIR_FRESH_COHORT_2026_09_05"
...
STATE_FILE = os.getenv("STATE_FILE", "bot_state_v20_4_0_safety_repair.json")
LEVERAGE = max(1, min(10, int(os.getenv("LEVERAGE", "10"))))
...
AUTO_SCAN_SECONDS = max(30, int(os.getenv("AUTO_SCAN_SECONDS", "30")))
AUTO_TRACK_SECONDS = max(5, int(os.getenv("AUTO_TRACK_SECONDS", "5")))
REQUEST_TIMEOUT = float(os.getenv("REQUEST_TIMEOUT", "8"))
API_RETRIES = int(os.getenv("API_RETRIES", "3"))
API_THROTTLE_SECONDS = float(os.getenv("API_THROTTLE_SECONDS", "0.04"))
# Global sliding-window limiter shared by all scan workers. It deliberately
# stays configurable and conservative instead of assuming a venue limit.
API_MAX_REQUESTS_PER_10S = max(
    10, min(90, int(os.getenv("API_MAX_REQUESTS_PER_10S", "80")))
)
MAX_CONTRACTS = min(300, max(50, int(os.getenv("MAX_CONTRACTS", "300"))))
MAX_ANALYZE_SYMBOLS = min(
    80, max(20, int(os.getenv("MAX_ANALYZE_SYMBOLS", "60")))
)
HOT_SYMBOLS_TO_ANALYZE = min(
    24, max(8, int(os.getenv("HOT_SYMBOLS_TO_ANALYZE", "18")))
)
MIN_HOT_CANDIDATES = min(
    HOT_SYMBOLS_TO_ANALYZE,
    max(8, int(os.getenv("MIN_HOT_CANDIDATES", "16"))),
)
HOT_SCAN_WORKERS = min(4, max(1, int(os.getenv("HOT_SCAN_WORKERS", "4"))))
DEEP_SCAN_WORKERS = min(4, max(1, int(os.getenv("DEEP_SCAN_WORKERS", "4"))))
...
MAX_LIVE_SIGNALS_24H = max(
    1, min(3, int(os.getenv("MAX_LIVE_SIGNALS_24H", "3")))
)
MAX_LIVE_SIGNALS_PER_SIDE_24H = max(
    1, min(2, int(os.getenv("MAX_LIVE_SIGNALS_PER_SIDE_24H", "2")))
)
MIN_LIVE_SIGNAL_SPACING_SECONDS = max(
    3600, int(os.getenv("MIN_LIVE_SIGNAL_SPACING_SECONDS", "3600"))
)
MAX_ADAPTIVE_CANARY_LIVE_24H = max(
    1, min(2, int(os.getenv("MAX_ADAPTIVE_CANARY_LIVE_24H", "2")))
)
...
TRADER_STYLE_PAPER_REASON = "active_mover_paper_v20_4_0"
EXHAUSTION_PAPER_REASON = "spike_regime_paper_v20_4_0"
...
SPIKE_ADAPTIVE_BLOCK_REASON = "spike_adaptive_block_v20_4_0"
SPIKE_STRATEGY_GUARD_REASON = "spike_strategy_guard_v20_4_0"
ACTIVE_MOVER_GUARD_REASON = "active_mover_strategy_guard_v20_4_0"
...
def is_official_user_paper(signal: Dict[str, Any]) -> bool:
    return official_paper_reason(signal) in OFFICIAL_USER_PAPER_REASONS


def official_paper_reason(signal: Dict[str, Any]) -> str:
    return str(
...
REAL_MONEY_LIVE_REASON = "official_micro_live_v20_4_0"
...
REAL_MONEY_ACK_PHRASE = "I_ACCEPT_REAL_MONEY_RISK_V20_4_0"
REAL_MONEY_ACK = os.getenv("REAL_MONEY_ACK", "")
REAL_MONEY_MIN_FORWARD = max(
    300, int(os.getenv("REAL_MONEY_MIN_FORWARD", "300"))
)
REAL_MONEY_MIN_UNIQUE_SYMBOLS = max(
    75, int(os.getenv("REAL_MONEY_MIN_UNIQUE_SYMBOLS", "75"))
)
...
REAL_MONEY_RECENT_WINDOW = max(
    60, int(os.getenv("REAL_MONEY_RECENT_WINDOW", "60"))
)
REAL_MONEY_MIN_SIDE_FORWARD = max(
    100, int(os.getenv("REAL_MONEY_MIN_SIDE_FORWARD", "100"))
)
REAL_MONEY_SIDE_RECENT_WINDOW = max(
    30, int(os.getenv("REAL_MONEY_SIDE_RECENT_WINDOW", "30"))
)
REAL_MONEY_MIN_SIDE_EXPECTANCY_R = float(
    os.getenv("REAL_MONEY_MIN_SIDE_EXPECTANCY_R", "0.10")
)
REAL_MONEY_MIN_LANE_SIDE_FORWARD = max(
    50, int(os.getenv("REAL_MONEY_MIN_LANE_SIDE_FORWARD", "50"))
)
REAL_MONEY_LANE_SIDE_RECENT_WINDOW = max(
    20, int(os.getenv("REAL_MONEY_LANE_SIDE_RECENT_WINDOW", "20"))
)
REAL_MONEY_MIN_STRATEGY_SIDE_FORWARD = max(
    40, int(os.getenv("REAL_MONEY_MIN_STRATEGY_SIDE_FORWARD", "40"))
)
REAL_MONEY_STRATEGY_SIDE_RECENT_WINDOW = max(
    20, int(os.getenv("REAL_MONEY_STRATEGY_SIDE_RECENT_WINDOW", "20"))
)
# Position sizing is informational: this service still never submits orders.
ACCOUNT_EQUITY_USDT = max(0.0, float(os.getenv("ACCOUNT_EQUITY_USDT", "0")))
ACCOUNT_RISK_PCT = min(
    0.0010, max(0.0, float(os.getenv("ACCOUNT_RISK_PCT", "0.0005")))
)
MAX_NOTIONAL_TO_EQUITY = min(
    2.0, max(0.1, float(os.getenv("MAX_NOTIONAL_TO_EQUITY", "1.0")))
)
MICRO_LIVE_RISK_MULT = min(
    0.02, max(0.0, float(os.getenv("MICRO_LIVE_RISK_MULT", "0.02")))
)
...
# V20.4 hard-disables the unaudited legacy route. Only the two official lanes
# can create PAPER candidates or ever become eligible for a live alert.
LEGACY_SIGNAL_PATH_ENABLED = False
...
SHADOW_MAX_ACTIVE = min(8, max(2, int(os.getenv("SHADOW_MAX_ACTIVE", "8"))))
SHADOW_PAPER_RESERVED_SLOTS = SHADOW_MAX_ACTIVE
...
ACTIVE_MOVER_SOFT_EXPIRE_MINUTES = int(os.getenv("ACTIVE_MOVER_SOFT_EXPIRE_MINUTES", "6"))
ACTIVE_MOVER_HARD_EXPIRE_MINUTES = int(os.getenv("ACTIVE_MOVER_HARD_EXPIRE_MINUTES", "30"))
ACTIVE_MOVER_MIN_PROGRESS_AT_SOFT = float(os.getenv("ACTIVE_MOVER_MIN_PROGRESS_AT_SOFT", "0.15"))
ACTIVE_MOVER_MAX_ACTIVE = min(
    3, max(1, int(os.getenv("ACTIVE_MOVER_MAX_ACTIVE", "3")))
)
...
EXHAUST_SOFT_EXPIRE_MINUTES = int(os.getenv("SPIKE_SOFT_EXPIRE_MINUTES", "8"))
EXHAUST_HARD_EXPIRE_MINUTES = int(os.getenv("SPIKE_HARD_EXPIRE_MINUTES", "45"))
...
EXECUTION_PRICE_CACHE: Dict[str, Tuple[float, Dict[str, Any]]] = {}
...
LIVE_DISPATCH_LOCK = threading.Lock()
API_RATE_LOCK = threading.Lock()
API_REQUEST_TIMES: deque = deque()
...
def default_state() -> Dict[str, Any]:
    return {
        ...
        "last_scan": {},
        "last_diag_ts": 0,
        "state_recovery_quarantine": False,
        "last_error": "",
    }


def load_state() -> Dict[str, Any]:
    global STATE_RECOVERY_OK
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            base.update(data)
        # V20.4 starts a clean execution cohort. Old open/pending signals were
        # produced with the off-by-one return and broken execution gates, so
        # carrying them forward would contaminate the repaired statistics.
        if loaded_schema < 8:
            base["active_signals"] = []
            base["pending_signals"] = []
            base["shadow_signals"] = []
            base["active_mover_watch"] = []
            base["squeeze_4h_watch"] = []
        ...
        base.setdefault("state_recovery_quarantine", False)
        ...
        fallback["state_recovery_quarantine"] = True
        fallback["last_error"] = f"state recovery failed: {sanitize_error(repr(exc))}"
        return fallback


def save_state() -> None:
    global STATE_PERSISTENCE_OK
    try:
...
def persistent_storage_ready() -> Tuple[bool, str]:
    """Fail closed for live alerts unless state and model storage are durable."""
    if not STATE_RECOVERY_OK:
        return False, "state recovery failed; manual review required"
    if bool(STATE.get("state_recovery_quarantine", False)):
        return False, "state recovery quarantine requires manual review"
    if not STATE_PERSISTENCE_OK:
        return False, "last state write failed"
    state_path = Path(STATE_FILE)
    db_path = Path(DB_PATH)
    if not state_path.is_absolute() or not db_path.is_absolute():
        return False, "STATE_FILE and ADAPTIVE_DB_PATH must be absolute"
    for path in (state_path, db_path):
        parent = path.parent
        if not parent.exists() or not os.access(parent, os.W_OK):
            return False, f"storage directory is not writable: {parent}"
    return True, "durable storage configured"
...
def sanitize_error(error: Any) -> str:
    """Prevent credentials from being persisted or returned by diagnostics."""
    value = str(error or "")
...
def _telegram_record_failure(error: str, attempts_used: int, mode: str) -> None:
    with STATE_IO_LOCK:
        ...
        delivery["last_error"] = sanitize_error(
            error or "unknown Telegram failure"
        )
        delivery["last_mode"] = mode
...
def _telegram_post_text(
    ...
    prefix = "" if last_retryable else "NON_RETRYABLE: "
    return False, prefix + sanitize_error(last_error), attempts_used
    ...
    with STATE_IO_LOCK:
        ...
        for item in outbox:
            if str(item.get("id", "")) == message_id:
                item["last_error"] = sanitize_error(
                    error or item.get("last_error", "")
                )
...
def send_telegram(text: str) -> bool:
    ...
    if str(error).startswith("NON_RETRYABLE:"):
        with STATE_IO_LOCK:
            delivery = _telegram_delivery_state()
            delivery["dead_lettered"] = int(
                delivery.get("dead_lettered", 0) or 0
            ) + 1
            delivery["last_mode"] = "non_retryable_rejected"
            save_state()
        return False
    if replaceable:
        with STATE_IO_LOCK:
        ...
        for _ in range(TELEGRAM_OUTBOX_BATCH):
            ...
            if str(error).startswith("NON_RETRYABLE:"):
                with STATE_IO_LOCK:
                    ...
                    _telegram_record_failure(error, attempts_used, "dead_letter")
                continue
            ...
            with STATE_IO_LOCK:
                ...
                if (
                    ...
                    current_outbox[0]["last_error"] = sanitize_error(error)
                    backoff = min(
...
def telegram_delivery_summary() -> str:
    ...
    return (
        ...
        f"dead-letter={int(delivery.get('dead_lettered', 0) or 0)} · "
...
def micro_live_circuit_breaker() -> Dict[str, Any]:
    ...
    with _LOCK, _connect() as conn:
        rows = conn.execute(
            "SELECT id, result, pnl_r, symbol, side, strategy, decision_reason "
            "FROM adaptive_trades "
            "WHERE COALESCE(decision_reason, '')=? "
            "ORDER BY closed_at ASC, id ASC",
...
def real_money_readiness() -> Dict[str, Any]:
    ...
    by_lane_side: Dict[str, Dict[str, Any]] = {}
    for lane in sorted(OFFICIAL_USER_PAPER_REASONS):
        for side in ("LONG", "SHORT"):
            bucket_rows = [
                row
                for row in rows
                if str(row["decision_reason"] or "") == lane
                and str(row["side"] or "").upper() == side
            ]
            bucket_recent_window = max(1, REAL_MONEY_LANE_SIDE_RECENT_WINDOW)
            bucket_recent_rows = bucket_rows[-bucket_recent_window:]
            bucket_metrics = _outcome_metrics(bucket_rows)
            bucket_recent = _outcome_metrics(bucket_recent_rows)
            bucket_checks = {
                "enough_lane_side_forward": len(bucket_rows)
                >= REAL_MONEY_MIN_LANE_SIDE_FORWARD,
                "lane_side_tp3_majority": float(
                    bucket_metrics.get("success_rate", 0.0) or 0.0
                )
                > REAL_MONEY_MIN_SUCCESS_RATE,
                "lane_side_expectancy": float(
                    bucket_metrics.get("expectancy_r", 0.0) or 0.0
                )
                >= REAL_MONEY_MIN_SIDE_EXPECTANCY_R,
                "lane_side_recent_complete": len(bucket_recent_rows)
                >= bucket_recent_window,
                "lane_side_recent_tp3_majority": float(
                    bucket_recent.get("success_rate", 0.0) or 0.0
                )
                > REAL_MONEY_MIN_SUCCESS_RATE,
                "lane_side_recent_expectancy": float(
                    bucket_recent.get("expectancy_r", 0.0) or 0.0
                )
                > 0.0,
            }
            key = f"{lane}:{side}"
            by_lane_side[key] = {
                "ready": all(bucket_checks.values()),
                "checks": bucket_checks,
                "cumulative": bucket_metrics,
                "recent": bucket_recent,
                "required": REAL_MONEY_MIN_LANE_SIDE_FORWARD,
                "recent_window": bucket_recent_window,
            }
    ...
    for strategy in strategies:
        for side in ("LONG", "SHORT"):
            bucket_rows = [
                row
                for row in rows
                if str(row["strategy"] or "") == strategy
                and str(row["side"] or "").upper() == side
            ]
            if not bucket_rows:
                continue
            bucket_recent_window = max(
                1, REAL_MONEY_STRATEGY_SIDE_RECENT_WINDOW
            )
            bucket_recent_rows = bucket_rows[-bucket_recent_window:]
            bucket_metrics = _outcome_metrics(bucket_rows)
            bucket_recent = _outcome_metrics(bucket_recent_rows)
            bucket_checks = {
                "enough_strategy_side_forward": len(bucket_rows)
                >= REAL_MONEY_MIN_STRATEGY_SIDE_FORWARD,
                "strategy_side_tp3_majority": float(
                    bucket_metrics.get("success_rate", 0.0) or 0.0
                )
                > REAL_MONEY_MIN_SUCCESS_RATE,
                "strategy_side_expectancy": float(
                    bucket_metrics.get("expectancy_r", 0.0) or 0.0
                )
                >= REAL_MONEY_MIN_SIDE_EXPECTANCY_R,
                "strategy_side_recent_complete": len(bucket_recent_rows)
                >= bucket_recent_window,
                "strategy_side_recent_tp3_majority": float(
                    bucket_recent.get("success_rate", 0.0) or 0.0
                )
                > REAL_MONEY_MIN_SUCCESS_RATE,
                "strategy_side_recent_expectancy": float(
                    bucket_recent.get("expectancy_r", 0.0) or 0.0
                )
                > 0.0,
            }
            key = f"{strategy}:{side}"
            by_strategy_side[key] = {
                "ready": all(bucket_checks.values()),
                "checks": bucket_checks,
                "cumulative": bucket_metrics,
                "recent": bucket_recent,
                "required": REAL_MONEY_MIN_STRATEGY_SIDE_FORWARD,
                "recent_window": bucket_recent_window,
            }
    checks = {
    ...
    storage_ok, storage_reason = persistent_storage_ready()
    acknowledgement_ok = secrets.compare_digest(
        str(REAL_MONEY_ACK or ""), REAL_MONEY_ACK_PHRASE
    )
    any_lane_side_ready = any(
        bool(item.get("ready")) for item in by_lane_side.values()
    )
    any_strategy_side_ready = any(
        bool(item.get("ready")) for item in by_strategy_side.values()
    )
    live_enabled = bool(
        ready
        and any_lane_side_ready
        and any_strategy_side_ready
        and REAL_MONEY_SIGNALS
        and not TEST_MODE
        and acknowledgement_ok
        and ACCOUNT_EQUITY_USDT > 0
        and ACCOUNT_RISK_PCT > 0
        and WEB_CONCURRENCY == 1
        and len(ADMIN_KEY) >= 24
        and storage_ok
    )
    return {
        "ready": ready,
        ...
        "account_equity_configured": ACCOUNT_EQUITY_USDT > 0,
        "storage_ok": storage_ok,
        "storage_reason": storage_reason,
        "single_worker": WEB_CONCURRENCY == 1,
        "admin_key_configured": len(ADMIN_KEY) >= 24,
        "checks": checks,
...
def live_authorization(
    signal: Dict[str, Any], lane_reason: Optional[str] = None
) -> Tuple[bool, str]:
    """Single fail-closed authorization gate for every possible live alert."""
    lane = str(lane_reason or official_paper_reason(signal))
    side = str(signal.get("side", "")).upper()
    failures: List[str] = []
    if TEST_MODE:
        failures.append("TEST_MODE=true")
    if not REAL_MONEY_SIGNALS:
        failures.append("REAL_MONEY_SIGNALS=false")
    if not secrets.compare_digest(str(REAL_MONEY_ACK or ""), REAL_MONEY_ACK_PHRASE):
        failures.append("REAL_MONEY_ACK mismatch")
    if ACCOUNT_EQUITY_USDT <= 0:
        failures.append("ACCOUNT_EQUITY_USDT is not configured")
    if ACCOUNT_RISK_PCT <= 0:
        failures.append("ACCOUNT_RISK_PCT must be positive")
    if WEB_CONCURRENCY != 1:
        failures.append("WEB_CONCURRENCY must equal 1")
    if len(ADMIN_KEY) < 24:
        failures.append("ADMIN_KEY must contain at least 24 characters")
    if lane not in OFFICIAL_USER_PAPER_REASONS:
        failures.append("candidate is not an official fresh PAPER lane")
    if side not in {"LONG", "SHORT"}:
        failures.append("invalid side")

    storage_ok, storage_reason = persistent_storage_ready()
    if not storage_ok:
        failures.append(storage_reason)

    try:
        readiness = real_money_readiness()
        if not bool(readiness.get("ready")):
            failures.append("global fresh-cohort readiness failed")
        side_state = readiness.get("by_side", {}).get(side, {})
        if not bool(side_state.get("ready")):
            failures.append(f"{side or 'UNKNOWN'} evidence gate failed")
        lane_side_key = f"{lane}:{side}"
        lane_side_state = readiness.get("by_lane_side", {}).get(
            lane_side_key, {}
        )
        if not bool(lane_side_state.get("ready")):
            failures.append("strategy-lane/side evidence gate failed")
        strategy_side_key = f"{str(signal.get('strategy', ''))}:{side}"
        strategy_side_state = readiness.get("by_strategy_side", {}).get(
            strategy_side_key, {}
        )
        if not bool(strategy_side_state.get("ready")):
            failures.append("exact strategy/side evidence gate failed")
        if not bool(
            readiness.get("micro_live_circuit", {}).get("allowed", False)
        ):
            failures.append("micro-live circuit breaker is open")
    except Exception as exc:
        failures.append(f"readiness error: {sanitize_error(repr(exc))}")

    return (not failures), ("authorized" if not failures else "; ".join(failures))


def attach_position_sizing(signal: Dict[str, Any]) -> None:
    """Attach a capped risk plan; never place or modify an exchange order."""
    entry = float(signal.get("entry", 0.0) or 0.0)
    stop = float(signal.get("sl", entry) or entry)
    if ACCOUNT_EQUITY_USDT <= 0 or entry <= 0 or stop <= 0:
        return
    stop_move = abs(entry - stop) / entry
    effective_risk_move = stop_move + ROUND_TRIP_COST_MOVE
    if effective_risk_move <= 0:
        return
    risk_usdt = ACCOUNT_EQUITY_USDT * ACCOUNT_RISK_PCT
    notional = min(
        risk_usdt / effective_risk_move,
        ACCOUNT_EQUITY_USDT * MAX_NOTIONAL_TO_EQUITY,
    )
    signal["account_risk_pct"] = ACCOUNT_RISK_PCT
    signal["risk_budget_usdt"] = risk_usdt
    signal["suggested_notional_usdt"] = max(0.0, notional)
    signal["suggested_margin_usdt"] = max(0.0, notional / max(1, LEVERAGE))
    signal["estimated_cost_move"] = ROUND_TRIP_COST_MOVE
...
def maybe_send_auto_backup() -> Dict[str, Any]:
    ...
    V20.4 keeps the `paper0` fix: milestones are based on the two official
    ...
    try:
        ...
        caption = (
            f"🧠 V20.4 clean backup · official {official_count} · "
...
def _wait_for_api_slot() -> None:
    """Bound aggregate REST traffic from every scan/tracker thread."""
    window_seconds = 10.0
    while True:
        wait_for = 0.0
        current = time.monotonic()
        with API_RATE_LOCK:
            while API_REQUEST_TIMES and current - API_REQUEST_TIMES[0] >= window_seconds:
                API_REQUEST_TIMES.popleft()
            if len(API_REQUEST_TIMES) < API_MAX_REQUESTS_PER_10S:
                API_REQUEST_TIMES.append(current)
                return
            wait_for = window_seconds - (current - API_REQUEST_TIMES[0]) + 0.01
        ...
        try:
            ...
            if r.status_code in (429, 500, 502, 503, 504):
                ...
                try:
                    ...
                    retry_after = 0.0
                time.sleep(
                    max(min(retry_after, 10.0), 0.25 * (attempt + 1))
                )
                continue
            if not r.ok:
                last_err = f"HTTP {r.status_code} {path}"
            ...
            if not isinstance(payload, dict):
                last_err = f"non-object JSON response {path}"
                continue
            venue_code = payload.get("code")
            if venue_code not in (None, 0, "0"):
                last_err = (
                    f"venue code {sanitize_error(venue_code)} {path}: "
                    f"{sanitize_error(payload.get('msg') or payload.get('message'))}"
                )
                continue
            return payload
        except Exception as e:
            last_err = f"get_json {path}: {sanitize_error(repr(e))}"
...
def percent_change(candles: List[Dict[str, float]], bars: int) -> float:
    """Return the close-to-close move across exactly ``bars`` intervals."""
    bars = max(1, int(bars))
    if len(candles) <= bars:
        return 0.0
    a = candles[-bars - 1]["close"]
    b = candles[-1]["close"]
...
def v17_2_dual_paper_setup(
    ...
    price = float(c1[-1]["close"])
    last = c1[-1]
    # True interval returns use the repaired exact-bars helper.
...
def prepare_active_mover_execution(
    setup: Dict[str, Any],
    symbol: str,
    c1: List[Dict[str, float]],
    c5: List[Dict[str, float]],
) -> Tuple[bool, str]:
    """Attach executable entry, turnover, spread and depth before TP/SL math."""
    liquidity = candle_liquidity_snapshot(symbol, c1, c5)
    quote60 = float(liquidity.get("quote_60m", 0.0) or 0.0)
    if not bool(liquidity.get("ok")):
        return False, f"liquidity: {liquidity.get('reason', 'unknown')}"
    if quote60 < ACTIVE_MOVER_MIN_QUOTE_60M:
        return False, f"turn60 {quote60:.0f} < {ACTIVE_MOVER_MIN_QUOTE_60M:.0f}"
    ...
    executable = float(
        book.get("ask" if side == "LONG" else "bid", 0.0) or 0.0
    )
    if executable <= 0:
        return False, "bad executable entry"
    setup["entry"] = executable
    setup["book_spread_bps"] = spread
    setup["book_depth_usdt"] = depth
    setup["liquidity_quote_60m"] = quote60
    setup["liquidity_rank_percentile"] = float(
        LIQUIDITY_SCAN_CACHE.get(normalize_symbol(symbol), {}).get(
            "rank_percentile", 0.0
        )
        or 0.0
    )
    accepted, reason = active_mover_execution_ok(setup)
    if not accepted:
        return False, reason
    return True, (
        f"book ok: spread {spread:.1f}bps, depth {depth:.0f}, "
        f"turn60 {quote60:.0f}"
    )
...
def process_active_mover_watches() -> Dict[str, int]:
    ...
    for item in snapshot:
        ...
        gate_reason += (
            f" · PRO INTEL {setup['intel_state']} squeeze={setup['intel_squeeze']:.0f} "
            f"overheat={setup['intel_overheat']:.0f} climax={setup['intel_climax']:.0f}"
        )

        execution_ok, execution_reason = prepare_active_mover_execution(
            setup, symbol, c1, c5
        )
        if not execution_ok:
            stats["rejected"] += 1
        ...
        if not exec_ok:
            remaining.append(item)
            continue
        exec_reason = f"{execution_reason}; {exec_reason}"
        ...
        dispatch_result = dispatch_official_trade(
            trade, TRADER_STYLE_PAPER_REASON
        )
        if dispatch_result in {"paper", "live"}:
            stats["triggered"] += 1
            if dispatch_result == "paper":
                active_paper += 1
...
def spike_bar_return(candles: List[Dict[str, float]], bars: int) -> float:
    """Backward-compatible name for the corrected interval-return helper."""
    return percent_change(candles, bars)
...
def process_squeeze_exhaustion_watches() -> Dict[str, int]:
    ...
    stats = {
        ...
        "runaway_watch": 0, "triggered": 0, "live": 0, "expired": 0,
    ...
    for item in snapshot:
        ...
        dispatch_result = dispatch_official_trade(
            trade, EXHAUSTION_PAPER_REASON
        )
        if dispatch_result in {"paper", "live"}:
            set_spike_regime_reentry_lock(trade)
            stats["triggered"] += 1
            if dispatch_result == "live":
                stats["live"] += 1
            continue
...
def analyze_symbol(
    ...
    # The remainder of this function is retained only for historical audit.
    # V20.4 never returns a legacy candidate into the generic live sender.
    if not LEGACY_SIGNAL_PATH_ENABLED:
        blocks["legacy_signal_path_disabled"] = blocks.get(
            "legacy_signal_path_disabled", 0
        ) + 1
        return None
...
def build_signal_message(s: Dict[str, Any]) -> str:
    ...
    live_header = (
        "🛡 ОГРАНИЧЕННЫЙ MICRO-LIVE ALERT — все fail-closed проверки пройдены\n"
        "⚠️ Это сигнал, не автоматическая биржевая заявка.\n"
    )
    sizing_text = ""
    if float(s.get("suggested_notional_usdt", 0.0) or 0.0) > 0:
        sizing_text = (
            f"План риска: {float(s.get('account_risk_pct', 0.0))*100:.3f}% счёта "
            f"≈ {float(s.get('risk_budget_usdt', 0.0)):.2f} USDT; "
            f"notional ≤ {float(s.get('suggested_notional_usdt', 0.0)):.2f} USDT; "
            f"margin x{LEVERAGE} ≈ {float(s.get('suggested_margin_usdt', 0.0)):.2f} USDT\n"
        )
    return (
        ...
        f"Риск: multiplier x{s['risk_mult']:.2f}; жёсткий предел риска 0.10% счёта\n"
        f"{sizing_text}"
...
def build_paper_result_message(
    ...
    badge = "🚀 SPIKE REGIME" if is_exhaust else ("🧲🟣 ACTIVE MOVER" if is_active else "⚡🟡 LEGACY FOLLOW-THROUGH")
    labels = {
        "profit": "✅ TP3+",
        "sl": "❌ STOP LOSS",
        "expired": "⏱ EXPIRED",
    ...
    stats_name = "SPIKE REGIME V20.4" if is_exhaust else ("ACTIVE MOVER V20.4" if is_active else "LEGACY FOLLOW-THROUGH")
    return (
...
def _diagnostic_block_label(key: str) -> str:
    """Map legacy internal counter keys to current V20.4 user-facing names."""
    mapping = {
...
def build_diagnostic(scan: Dict[str, Any]) -> str:
    ...
    return (
        f"🧪 Диагностика V20.4 SAFETY REPAIR · 🧲 ACTIVE MOVER + 🚀 SPIKE REGIME\n"
        ...
        f"🧲 ACTIVE MOVER V20.4: {_metrics_line(active_metrics)} · "
        f"n={int(active_metrics.get('n', 0) or 0)}\n"
        f"🚀 SPIKE REGIME V20.4: {_metrics_line(spike_metrics)} · "
...
def dispatch_official_trade(
    candidate: Dict[str, Any], lane_reason: str
) -> str:
    """Route one official setup through the same fail-closed live boundary.

    Returns ``live``, ``paper`` or ``rejected``. Capacity/cooldown failures
    always fall back to the official PAPER cohort; they never bypass tracking.
    """
    trade = dict(candidate)
    trade["paper_validation_lane"] = lane_reason
    trade["paper_validation_only"] = True
    attach_position_sizing(trade)

    with LIVE_DISPATCH_LOCK:
        authorized, authorization_reason = live_authorization(
            trade, lane_reason
        )
        budget_reason = ""
        if authorized:
            cooldown_allowed, cooldown_reason = cooldown_ok(
                str(trade.get("symbol", "")), str(trade.get("strategy", ""))
            )
            open_risk = sum(
                1
                for signal in STATE.setdefault("active_signals", [])
                if not signal.get(PROFIT_TARGET_KEY + "_hit")
                and not signal.get("stats_recorded")
            )
            budget = live_signal_budget_24h()
            side = str(trade.get("side", "")).upper()
            if not cooldown_allowed:
                budget_reason = cooldown_reason
            elif open_risk >= max(0, MAX_ACTIVE_SIGNALS):
                budget_reason = "maximum simultaneous live alerts reached"
            elif int(budget.get("remaining", 0) or 0) <= 0:
                budget_reason = "24h live alert cap reached"
            elif int(budget.get("spacing_left", 0) or 0) > 0:
                budget_reason = "minimum live-alert spacing is active"
            elif (
                MAX_LIVE_SIGNALS_PER_SIDE_24H > 0
                and int(budget.get(side.lower(), 0) or 0)
                >= MAX_LIVE_SIGNALS_PER_SIDE_24H
            ):
                budget_reason = f"{side} 24h cap reached"

        if authorized and not budget_reason:
            trade["paper_validation_only"] = False
            trade.pop("shadow_reason", None)
            trade["adaptive_reason"] = REAL_MONEY_LIVE_REASON
            trade["live_authorization_reason"] = authorization_reason
            trade["risk_mult"] = min(
                float(trade.get("risk_mult", MICRO_LIVE_RISK_MULT) or 0.0),
                MICRO_LIVE_RISK_MULT,
            )
            add_active_signal(trade)
            send_telegram(build_signal_message(trade))
            return "live"

        fallback_reason = authorization_reason
        if budget_reason:
            fallback_reason = f"{fallback_reason}; {budget_reason}"
        trade["paper_validation_origin"] = (
            f"{trade.get('paper_validation_origin', 'official forward setup')} · "
            f"LIVE LOCK: {fallback_reason}"
        )
        if add_shadow_signal(trade, lane_reason):
...
def _run_scan_impl(manual: bool = False) -> Dict[str, Any]:
    ...
    for paper_candidate in paper_ready:
        if set_confirmed_paper_signal(paper_candidate):
            ...
            paper_sent += 1
    ready_candidates = []
    for candidate in possible_live:
        live_ok, live_reason = live_authorization(candidate)
        if live_ok:
            ready_candidates.append(candidate)
            continue
        blocks["central_live_authorization_block"] = blocks.get(
            "central_live_authorization_block", 0
        ) + 1
        if is_official_user_paper(candidate):
            lane_reason = official_paper_reason(candidate)
            candidate["paper_validation_only"] = True
            candidate["paper_validation_origin"] = (
                f"{candidate.get('paper_validation_origin', 'official setup')} · "
                f"LIVE LOCK: {live_reason}"
            )
            if add_shadow_signal(candidate, lane_reason):
    ...
    for s in selected_live:
        final_live_ok, final_live_reason = live_authorization(s)
        if not final_live_ok:
            blocks["final_live_authorization_block"] = blocks.get(
                "final_live_authorization_block", 0
            ) + 1
            if is_official_user_paper(s):
                lane_reason = official_paper_reason(s)
                s["paper_validation_only"] = True
                s["paper_validation_origin"] = (
                    f"{s.get('paper_validation_origin', 'official setup')} · "
                    f"FINAL LIVE LOCK: {final_live_reason}"
                )
                if add_shadow_signal(s, lane_reason):
                    send_telegram(build_paper_signal_message(s))
                    paper_sent += 1
            continue
        attach_position_sizing(s)
        s["adaptive_reason"] = REAL_MONEY_LIVE_REASON
        add_active_signal(s)
...
def current_price(symbol: str, side: str = "") -> Optional[float]:
    """Return the currently executable exit side, not a candle-close proxy."""
    normalized = normalize_symbol(symbol)
    cached = EXECUTION_PRICE_CACHE.get(normalized)
    if cached and time.monotonic() - cached[0] < 1.0:
        book = cached[1]
    else:
        book = execution_book_snapshot(normalized)
        if bool(book.get("ok")):
            EXECUTION_PRICE_CACHE[normalized] = (time.monotonic(), dict(book))
    if not bool(book.get("ok")):
        return None
    normalized_side = str(side or "").upper()
    if normalized_side == "LONG":
        return float(book.get("bid", 0.0) or 0.0) or None
    if normalized_side == "SHORT":
        return float(book.get("ask", 0.0) or 0.0) or None
    bid = float(book.get("bid", 0.0) or 0.0)
    ask = float(book.get("ask", 0.0) or 0.0)
    return (bid + ask) / 2.0 if bid > 0 and ask > bid else None


def conservative_completed_candle_price(
    signal: Dict[str, Any], executable_price: float
) -> Tuple[float, bool]:
    """Apply unseen completed 1m bars with adverse-first ambiguity handling.

    The entry candle is ignored unless it opened at/after the recorded entry,
    avoiding pre-entry highs/lows. If TP3 and SL occur inside the same unseen
    candle, the stop is chosen. This intentionally avoids optimistic backfill.
    """
    candles = get_klines(
        str(signal.get("symbol", "")), "1m", 80, cache_seconds=1
    )
    if not candles or len(candles) < 2:
        return executable_price, False
    side = str(signal.get("side", "")).upper()
    created_at = float(signal.get("created_at", now_ts()) or now_ts())
    last_seen = float(signal.get("last_completed_candle_time", 0.0) or 0.0)
    changed = False

    for candle in candles[:-1]:
        candle_time = _candle_time_seconds(candle)
        if candle_time <= last_seen or candle_time < created_at:
            continue
        signal["last_completed_candle_time"] = candle_time
        last_seen = candle_time
        changed = True
        high = float(candle.get("high", executable_price) or executable_price)
        low = float(candle.get("low", executable_price) or executable_price)
        spread_move = max(
            0.0, float(signal.get("book_spread_bps", 0.0) or 0.0) / 10_000.0
        )
        # Convert candle prints into a conservative executable-side estimate.
        if side == "LONG":
            high *= 1.0 - spread_move
            low *= 1.0 - spread_move
        else:
            high *= 1.0 + spread_move
            low *= 1.0 + spread_move
        # Excursions use both extremes; result ordering remains conservative.
        update_excursion(signal, high)
        update_excursion(signal, low)

        protected = signal.get("protected_sl")
        effective_sl = float(
            protected if protected is not None else signal.get("sl", 0.0)
        )
        stop_touched = (
            low <= effective_sl if side == "LONG" else high >= effective_sl
        )
        tp3 = float(signal.get(PROFIT_TARGET_KEY, 0.0) or 0.0)
        tp3_touched = high >= tp3 if side == "LONG" else low <= tp3
        if stop_touched:
            # Includes same-bar SL+TP3 ambiguity: adverse outcome wins.
            return effective_sl, True

        for key in ("tp1", "tp2"):
            target = float(signal.get(key, 0.0) or 0.0)
            touched = high >= target if side == "LONG" else low <= target
            if touched and not signal.get(f"{key}_hit"):
        ...
        if (
            ...
            entry = float(signal.get("entry", executable_price) or executable_price)
            candidate_stop = (
                entry * (1.0 + ROUND_TRIP_COST_MOVE)
                if side == "LONG"
                else entry * (1.0 - ROUND_TRIP_COST_MOVE)
            )
            signal["protected_sl"] = (
                max(float(signal.get("sl", candidate_stop)), candidate_stop)
                if side == "LONG"
                else min(float(signal.get("sl", candidate_stop)), candidate_stop)
            )
            signal["tp1_protected"] = True
            changed = True

        if signal.get("tp2_hit") and not signal.get("tp2_protected"):
            tp1 = float(signal.get("tp1", executable_price) or executable_price)
            candidate_stop = (
                tp1 * (1.0 - ROUND_TRIP_COST_MOVE)
                if side == "LONG"
                else tp1 * (1.0 + ROUND_TRIP_COST_MOVE)
            )
            current_stop = float(
                signal.get("protected_sl", signal.get("sl", candidate_stop))
                or candidate_stop
            )
            signal["protected_sl"] = (
        ...
        # A newly armed protective stop touched somewhere in the same candle is
        # treated as an exit because the within-candle order is unknowable.
        newly_protected = signal.get("protected_sl")
        if newly_protected is not None:
            protective = float(newly_protected)
            protective_touched = (
                low <= protective if side == "LONG" else high >= protective
            )
    ...
    return executable_price, changed
...
def track_shadow_signals() -> bool:
    with STATE_IO_LOCK:
        shadows = list(STATE.setdefault("shadow_signals", []))
    if not shadows:
        return False
    ...
    for signal in shadows:
        ...
        price = current_price(signal["symbol"], signal.get("side", ""))
        if price is None:
            remaining.append(signal)
            continue

        price, bar_changed = conservative_completed_candle_price(signal, price)
        changed = changed or bar_changed
    ...
    if changed:
        with STATE_IO_LOCK:
            current = list(STATE.setdefault("shadow_signals", []))
            current_ids = {
                str(item.get("signal_id", ""))
                for item in current
                if item.get("signal_id")
            ...
            surviving_originals = [
                ...
                or str(item.get("signal_id")) in current_ids
            ]
            newly_added = [
                item
                for item in current
                if not item.get("signal_id")
                or str(item.get("signal_id")) not in original_ids
            ]
            STATE["shadow_signals"] = surviving_originals + newly_added
    return changed


def _track_active_signals_impl() -> None:
    with STATE_IO_LOCK:
        active = list(STATE.setdefault("active_signals", []))
    original_ids = {
        str(item.get("signal_id", ""))
        for item in active
        if item.get("signal_id")
    ...
    if changed:
        with STATE_IO_LOCK:
            current = list(STATE.setdefault("active_signals", []))
            current_ids = {
                str(item.get("signal_id", ""))
                for item in current
                if item.get("signal_id")
            ...
            STATE["active_signals"] = surviving_originals + newly_added
        save_state()
...
async def scan_loop():
    ...
    send_telegram(
        ...
        f"Mode: PAPER BY DEFAULT · TWO OFFICIAL LANES · LIVE FAIL-CLOSED.\n"
        f"Логика V20.4: 🧲 ACTIVE MOVER ждёт pullback/reclaim/re-acceleration; "
        f"🚀 SPIKE REGIME различает continuation и exhaustion. Конфликт режима = NO TRADE.\n"
        f"Исполнение: вход по ASK/BID, выход по исполняемой стороне книги; "
        f"неоднозначная свеча SL+TP учитывается как SL.\n"
        ...
        f"Официальные PAPER-линии: 🧲 ACTIVE MOVER + 🚀 SPIKE REGIME V20.4. Legacy-маршрут жёстко отключён.\n"
        ...
        f"🧠 SELF-LEARNING: SHADOW={SHADOW_ONLY} · fresh V20.4 cohort · hard-min warm-up {MIN_TRAIN_TRADES} · retrain каждые {RETRAIN_EVERY} · {ADAPTIVE_CONFIRMATION_PASSES}-pass validation.\n"
        f"🧠 TRADING INTELLIGENCE: candle anatomy + wick rejection + EMA/VWAP/ATR displacement + RSI context + volume/range expansion + squeeze proxy + overheat/climax + regime conflict = NO TRADE.\n"
        f"Diagnostics V20.4: current two-lane summary every 12h.\n"
        f"🧲 ACTIVE MOVER: HOT → quiet WATCH → pullback/reclaim → re-acceleration → execution gate.\n"
        ...
        f"🚀 Важно: UP spike может дать LONG continuation или SHORT exhaustion; DOWN spike — SHORT continuation или LONG exhaustion. Направление выбирается после классификации режима.\n"
        ...
        f"Forward cohort: минимум {REAL_MONEY_MIN_FORWARD} закрытых исходов, "
        f"{REAL_MONEY_MIN_UNIQUE_SYMBOLS} уникальных символов и "
        f"{REAL_MONEY_MIN_LANE_SIDE_FORWARD} на lane/side, "
        f"{REAL_MONEY_MIN_STRATEGY_SIDE_FORWARD} на точную strategy/side.\n"
        ...
        f"Evidence, TEST_MODE=false, explicit ACK, equity and durable storage are mandatory. "
        f"This code never places exchange orders.\n"
        ...
        f"Eligible SPIKE model data={model_data_count}/{MIN_TRAIN_TRADES}.\n"
        f"Official two-lane cohort: {_metrics_line(official_two_lane_metrics())}.\n"
        f"{watch_audit_summary()}.\n"
...
def root():
    return HTMLResponse(
        f"<h3>{APP_NAME}</h3>"
        f"<p>{DEPLOY_MARKER}</p>"
        f"<p>Use /health /version /auto-status /readiness /stats /adaptive-report "
        f"/source-audit /watch-audit /adaptive-retrain /adaptive-events "
        f"/export-data /telegram-status. POST /scan, /adaptive-retrain, "
        f"/telegram-backup and /test-telegram require X-Admin-Key.</p>"
    )
...
def manual_scan(
    ...
    if not admin_authorized(x_admin_key):
        return JSONResponse(
            {"ok": False, "error": "valid X-Admin-Key header required"},
            status_code=403,
        )
