# VERIFIED GITHUB DEPLOY FILE — V20.0 PRO SPIKE REGIME · CONTINUATION + EXHAUSTION
...
APP_NAME = "Professional Adaptive Futures Bot AUTO V20.0 PRO SPIKE REGIME + ACTIVE MOVER"
DEPLOY_MARKER = "V20_0_PRO_SPIKE_REGIME_CONTINUATION_EXHAUSTION_2026_08_30"
...
EXHAUSTION_PAPER_REASON = "spike_regime_paper_v20_0"
...
def process_squeeze_exhaustion_watches() -> Dict[str, int]:
    ...
    for item in snapshot:
        ...
        if cont_score >= SPIKE_CONT_MIN_SCORE and not failed_reclaim:
            regime = "CONTINUATION"
            trade_side = "LONG" if impulse_dir > 0 else "SHORT"
            stats["continuation"] += 1

        # Exhaustion takes priority once the broken structure has failed reclaim.
        if failed_reclaim and fade_score >= SPIKE_FADE_MIN_SCORE:
            regime = "EXHAUSTION"
            trade_side = "SHORT" if impulse_dir > 0 else "LONG"
            stats["exhaustion"] += 1
        ...
        setup["paper_validation_origin"] = (
            ...
            f"cont {cont_score}/8, fade {fade_score}/8, failed reclaim={failed_reclaim}, "
...
async def scan_loop():
    ...
    send_telegram(
        ...
        f"Официальные PAPER-линии: 🧲 ACTIVE MOVER + 🚀 SPIKE REGIME V20.0. Legacy Follow-Through/SHADOW отключены.\n"
